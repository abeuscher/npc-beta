...
class ImportDonationsPage extends Page
{
                            ...
                            Forms\Components\Section::make('Source')
                                ->schema([
                                    Forms\Components\TextInput::make('session_label')
                                        ->label('Session label')
                                        ->default(fn () => 'Donations Import on ' . now()->format('F j, Y \a\t g:i A'))
                                        ->required()
                                        ->maxLength(255),
                                    ...
                                    Forms\Components\Grid::make(5)->schema([
                                        ...
                                        Forms\Components\Select::make('import_source_id')
                                            ...
                                            ->helperText('Select to enable re-import matching and load saved donations mapping.')
    ...
    private function processUploadedFile(): void
    {
        ...
        $savedFieldMap       = $savedSource?->donations_field_map ?? [];
        $savedCustomFieldMap = $savedSource?->donations_custom_field_map ?? [];
        ...
        if ($savedSource && ! empty($savedFieldMap)) {
            ...
            foreach ($this->parsedHeaders as $header) {
                ...
                if (isset($savedCustomFieldMap[$normalized])) {
                    ...
                    $columnMap["col_{$n}"]        = '__custom_donation__';
            ...
            $this->data['contact_match_key'] = $savedSource->donations_contact_match_key ?: $this->deriveContactMatchKey($columnMap);
    ...
    private function getColumnMappingSchema(): array
    {
        ...
        if ($this->usedSavedMapping) {
            ...
            $schema[] = Forms\Components\Placeholder::make('saved_mapping_banner')
                ->label('')
                ->content(new \Illuminate\Support\HtmlString(
                    "<p class='text-sm text-gray-500'>Using saved donations mapping from <strong>{$name}</strong>. Adjust any that are wrong; your overrides do not mutate the source.</p>"
                ));
        }
    ...
    private function columnMappingRowSchema(string $header, array $groupedOptions): array
    {
        ...
        $customSubForm = Forms\Components\Grid::make(3)
            ...
            ->visible(fn (Forms\Get $get) => $get($key) === '__custom_donation__');
    ...
    private function contactMatchKeyOptions(Forms\Get $get): array
    {
        ...
        foreach ($this->parsedHeaders as $header) {
            ...
            if (is_string($dest) && str_starts_with($dest, 'contact:')) {
                $label = DonationImportFieldRegistry::flatFields()[$dest] ?? $dest;
                $options[$dest] = $label;
            }
        }
    ...
    private function getPreviewSchema(): array
    {
        ...
        $map    = $this->data['column_map'] ?? [];
        $labels = DonationImportFieldRegistry::flatFields();
    ...
    {
        $blocking = ImportSession::where('model_type', 'donation')
        ...
        if ($blocking) {
            Notification::make()
                ...
                ->body('A previous donations import is awaiting review. Approve or roll it back before starting a new one.')
                ->danger()
                ->send();
