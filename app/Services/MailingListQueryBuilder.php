...
class MailingListQueryBuilder
{
    ...
    private static function applyFilter(Builder $query, MailingListFilter $filter): void
    {
        ...
        match ($operator) {
            ...
            'includes'    => $query->whereHas('tags', fn ($q) => $q->where('type', 'contact')->where('name', $value)),
            'not_includes'=> $query->whereDoesntHave('tags', fn ($q) => $q->where('type', 'contact')->where('name', $value)),
