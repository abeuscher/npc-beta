...
>
> *Worked example (397):* Forms — the eighth run, the second compressed into carve AND packaging AND extraction in one session (the 396 shape), and the fifth migration-owning, testsuite-carrying run. Touchpoints found: **no asset-path strings** (WebForm declares no assets — step 3's rewrite and gate 4 both recorded n/a, the sync-row proof standing in); two Dockerfile COPY lines (added at the same-session package step, removed here); the `Forms` phpunit testsuite (8 files, 50 tests); **one path read inside the plugin's own traveling tests** (the schema twin's migrator-path assertion — the 395 `loadMigrationsFrom(__DIR__)`-symlink nuance again: it asserts the in-repo path until extraction, re-pointed vendor-relative in the extracted copy at step 3); the core twin fixtures' migrator reaches were **vendor-relative from birth** (the 393/395 compression nuance matured — written that way at the carve, zero fixture re-points at any later step); two comment lines (`routes/web.php`, the boundary-guard docblock) + `docs/app-reference.md` (2 rows) / `docs/runbooks/permission-matrix.md` (1 link). The plugin's first-ever Blade component (`<x-public-form>`, registered unprefixed via `Blade::anonymousComponentPath()`) needed nothing at extraction — the registration is `__DIR__`-relative and location-independent by construction. No procedure deltas — the runbook ran as written.
>
> *Worked example (398):* Products — the ninth run, the third compressed into carve AND packaging AND extraction in one session, and the sixth migration-owning, testsuite-carrying run. Touchpoints found: two asset-path strings (ProductCarousel scss+js; ProductDisplay declares none; no cross-plugin asset input); two Dockerfile COPY lines (added at the same-session package step, removed here); the `Products` phpunit testsuite (10 files, 54 tests); **one path read inside the plugin's own traveling tests** (the schema twin's migrator-path assertion — the 395 `loadMigrationsFrom(__DIR__)`-symlink nuance again, re-pointed vendor-relative in the extracted copy at step 3); the core twin fixtures' migrator reaches were **vendor-relative from birth** (the matured nuance held); two core test file reads landed at the carve on the in-repo path and re-pointed here (`AppearanceConfigRenameTest`'s template read + `WidgetColorTokenConsumptionTest`'s two line-keyed baseline entries — a **line-keyed SCSS baseline follows the file's path**, the 388 nuance); one `routes/web.php` comment + the boundary-guard docblock + `docs/app-reference.md` (3 rows) / `docs/runbooks/permission-matrix.md` (1 link) / `docs/schema/` ownership headers (4 + README). No procedure deltas — the runbook ran as written.
>
...
**Do NOT write a namespace boundary guard for the vertical.** `PluginConformanceTest` already scans core for every namespace in the distribution manifest, so the plugin is covered by the commit that adds its manifest entry — writing a per-plugin one duplicates it and puts the cost back on every future extraction. Eight such files accumulated before the kit replaced them, four of them describing themselves in their own comments as a previous one copied verbatim; the habit is easy to fall back into, which is why this is stated as a prohibition rather than a preference.

Write a per-plugin guard only for a ban that is **genuinely specific to this vertical** and has no generic form — the standing example is Payments, whose third-party SDK and config-namespace bans would need an exception list in the kit, which is the kit's own rule for what does not belong in it. If you find yourself reaching for an exception list, that is the signal the check is per-plugin.

Carry the vertical's "what core legitimately keeps" note into the table in `PluginConformanceTest`'s header rather than into a new file. See `docs/adr/0009-plugin-tests-in-three-tiers.md`.
...
>
> *Worked example (397):* the `Forms` testsuite (8 files) re-pointed to `vendor/nonprofitcrm/forms/tests` — 50 tests, identical counts from the vendor path.
>
> *Worked example (398):* the `Products` testsuite (10 files) re-pointed to `vendor/nonprofitcrm/products/tests` — 54 tests, identical counts from the vendor path.
