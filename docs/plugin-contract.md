...
- **Installed** = in the image: listed in `config/plugins.php`, the superset and **sole ordering authority** (amended at session 382, superseding the earlier "Stage B replaces the config list with package auto-discovery" posture): packages declare no `extra.laravel.providers` and Laravel auto-discovery is deliberately unused, because the remove-the-line guarantee, the registries-before-plugins binding order, and plugin-boot-before-core-routes ordering are all load-bearing and all keyed on the config list — auto-discovery would move activation to composer edits and surrender ordering to installation order. The provider remains the single entry point at every stage.
