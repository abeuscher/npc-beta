...
- **Deterministic install order: core dump → enabled plugins' migrations → seeders.** A fresh `migrate` loads the dump, then runs each enabled plugin's pending migrations (their rows append to the shared `migrations` table); enabling a plugin later runs its migrations then. Playwright's e2e `migrate:fresh --seed` and CI cover the order on every push.
