...
M4. The toolbar MUST NOT take focus away from the editor except via the link popover's URL field (D4a).

M5. The toolbar MUST NOT collapse, shift, or alter the Quill selection on any interaction other than D4a (link popover capture/restore).
...
### 6.5 `refreshPreview` suppression

A13 states that no preview-HTML replacement may swap the active widget's preview HTML while editing is in progress. The existing `store.inlineActiveWidgetId` flag is checked today only by the post-save echo path; other refresh paths (the per-region `needsConfig` watcher, direct programmatic calls to `store.refreshPreview`) do not consult it. To make A13's claim literally true across every path, the suppression is **centralized inside `refreshPreview` itself**:

- A single guard at the entry of `useRefreshPreview`'s `refreshPreview(widgetId)`: `if (widgetId === store.inlineActiveWidgetId) return`. Additive to existing logic; no caller changes are required and every refresh path automatically respects the suppression.
- The legitimate reconciling refresh that fires after teardown is unaffected: `endInlineEdit` clears `inlineActiveWidgetId` *before* calling `refreshPreview(widgetId)`, so the guard does not block it.
- Combined with A14's removal-detection observer, the corner-ghost bug is structurally impossible: A13 (centralised suppression) prevents wholesale replacement from being initiated; A14 (host-element removal observer) detects any replacement that somehow bypasses A13 and clears the handle synchronously.
