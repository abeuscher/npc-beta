...
it('keeps the billing/account code free of Stripe SDK/config references (renders the pushed document alone)', function () {
    ...
    // Lives in the Payments plugin since session 380 (arc P4); extracted to
    // its own repo at session 387 (arc D1).
    expect(file_get_contents(base_path('vendor/nonprofitcrm/payments/Services/StripeCheckoutService.php')))
        ->toMatch($banned);
});
