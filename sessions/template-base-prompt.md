...
- **Questions are not instructions.** When the user asks a question, the correct response is an *answer*, then a wait. Do not interpret a question as an implicit directive to take action. The user takes responsibility for explicit instructions; questions are an invitation to a discussion or to clarify. Reading questions as veiled commands is a learned bias from working under passive-aggressive management — this user does not communicate that way, and assuming otherwise produces friction. Heuristic: *if you can answer in words and stop, do that; only act when the user has given an explicit instruction.*
- **Human-facing output follows the reporting rules in `CLAUDE.md` (the "Reporting to the human" section).** That governs everything you write for me to read: the Open Gate orientation, the testing-ready announcement, the Phase 1 digest, and session logs. The dense process register in this template is how you *operate* — it is not how you *report*. Switch registers at those boundaries.
...
- **When implementation is complete**, run the fast test suite: `docker compose exec app php artisan test --exclude-group=slow` (or `./dev test`). Fix any failures before proceeding. If the session prompt specifies slow test groups to run, run those separately as well. For scoped design/drift work the inner-loop signal is `./dev test:design` (`php artisan test --group=design`) — ~10× faster; iterate on it, then run the full fast suite once before announcing completion.
...
- **Playwright is the CI signal — do not run it locally as a close-gate.** The local close-gate is `./dev test` (the fast Pest suite). Playwright runs on every CI push against the isolated e2e stack and is non-gating by design; the user should not pay machine time running it locally just to close a session. Local Playwright runs are valid only when **actively developing a Playwright spec under iteration** — once that iteration is complete, either the spec ships as a permanent regression artifact (a standing spec deliberately added to the suite) or it is torn down at session close. **Specs created mid-session purely to verify a one-off objective during development are torn down at close, not committed.** Heuristic: *if a future session would derive zero value from re-running this spec, delete it; if the spec is a standing guard, keep it and document its purpose in the session log.*
...
- **Digest the housekeeping capture buffer.** Read `sessions/housekeeping-incoming.md`. If it carries any items beyond its header (captured mid-session via `npm run logbug`), for each one do a quick validity check against current code (already fixed? duplicate of an existing inbox item? references code that no longer exists?) and **surface the digest to the user now, before close** — list each captured item with its capture stamp and your read on whether it's still valid. The actual fold-into-the-inbox happens in Phase 2; this step is the user-facing review. If the buffer holds only its header, say so in one line and move on.

Then stop. The user reviews the work, the next session's drafts, and the capture-buffer digest, and initiates close when ready. No merge, push, or deploy happens at this stage unless the user explicitly requests it (e.g. for deploy-server testing or structural changes that affect CI).
...
- **Archive the previous session**: move all files matching `sessions/(NNN-1). *.md` into `sessions/archived/`. This includes the previous session's base prompt, session prompt, and log. Skip silently if those files don't exist or have already been archived (e.g., the user moved them manually). The current session's files stay in `sessions/` until the *next* session's close. **Also sweep any closed `A###` (agentic) session files** (`sessions/A###. *.md`) whose work is complete — they don't match the numeric `(NNN-1)` pattern, so the per-close numeric sweep never catches them and they otherwise pile up at the top level. Move a completed `A###` set to `sessions/archived/` at the first numeric close after that agentic session lands.
- **Fold the capture buffer into the inbox.** For each item the Phase 1 digest cleared with the user, append it (keeping its `[VERSION · date]` capture stamp) to `sessions/housekeeping-inbox.md` under `## Inbox`, then clear `sessions/housekeeping-incoming.md` back to its header only. Items the user flagged as already-fixed or invalid are dropped, not folded. The capture stamp travels with the item so a future inbox walk can see its age and re-verify the premise.
- **Commit**: stage all changed files (including the log, updated outlines/track docs, the completed-sessions index, the inbox + cleared incoming buffer, and any files moved into `sessions/archived/`), commit on the current `session-NNN/N` branch, and notify the user. Do not push — the user pushes (or doesn't) on their own cadence. Never push to main, never force-push, never merge to main yourself (see `CLAUDE.md` Git Workflow for the full forbidden list).
