...
- **Commit and push**: stage all changed files (including the log, updated outlines/track docs, the completed-sessions index, and any files moved into `sessions/archived/`), commit on the current `session-NNN/N` branch, push the branch to its same-named remote, and notify the user. The user reviews and merges to main; never push to main, never force-push, never merge to main yourself (see `CLAUDE.md` Git Workflow for the full forbidden list).
