...
Copy this file to `173. base-prompt.md` at the start of each session. Replace `173` with the session number and update the session title.

---

We are about to begin a new session: **173. Widget Manifest & Metadata**.
...
3. Open `sessions/173. Widget Manifest & Metadata.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/173. Widget Manifest & Metadata — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
