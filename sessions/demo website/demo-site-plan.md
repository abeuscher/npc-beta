...
| **C** | **Event landing pages — Simple & Standard** ✅ *(session 326)* | Every event gets an LP at creation via a **Simple / Standard** toggle. **Simple** (free events only): a normal page seeded with hero + event-details (image optional, no reg form). **Standard** (forced once ticketed): hero + event details + registration card; the fuller v1 LP per §1b (About folded in, Share, Related events, `Event` schema/OG). LP is a plain `Page` linked via `events.landing_page_id`, editable in the builder; **deletion guarded via the existing `PageObserver` block-with-counts pattern** (no event left LP-less). Relink the mini-calendar + `EventsListing` rows to the LP URL and strip the dead-end fallback. `data-tour` anchors. | A |
| **B** | **Events — listing upgrade** ✅ *(session 327)* | As built (scope grew via owner iteration): two **orthogonal layout toggles** — `side_by_side_rows` (item layout) + `group_by_day` (grouping) — beside the unchanged grid/carousel; a layout-independent **featured-event hero**; a tag-based **Event-Type filter** dropdown (no Date-Range — cut); the **art-fallback chain** (§1a); Free/Sold-Out badges; `ItemList` JSON-LD; styleable day headings (`{{day.*}}` tokens); `data-tour` anchors. Also shipped the operator **`sold_out` event flag** (schema). Rows link to the LP. | C (every event has a resolvable LP URL) |
| **D** | **Per-page permission + locked landing** ✅ *(session 328)* | **Shipped as a `locked` flag + `edit_locked_pages` permission** (pivoted from `restricted_roles`; a `status` value would break `published()` visibility — see the 328 prompt's "Design decision"). `pages.locked` migration + `PagePolicy::update()`/`delete()` + page-builder save-API enforcement (every write route) + a permission-gated "Published & Locked" toggle on pages **and posts** + a lock badge in both lists + `demo:reset` locks **every** page idempotently; demo role's `navigation_menu` trimmed to view-only. Also closed the Filament single/bulk delete bypass on both PageResource/PostResource. §2a's `restricted_roles` design is **superseded**. | — (independent) |
| **329** | **Demo upload lockdown** *(hardening, inserted before E)* | Block new media uploads for the `demo` role at the Livewire temp-upload + page-builder upload chokepoints (existing-media reuse stays open). Closes the one elevated risk — public hosting of arbitrary image content. | — (independent) |
| **E** | **Demo node assembly** ✅ *(session 336 — reshaped)* | **Reshaped at the run-up to 336 from "seed showcase pages via `demo:reset`" to "wire the restore-from-blob primitive."** Built the demo-mode-gated **`demo:restore`** command (restores a locally-pushed baseline blob: `db:wipe --drop-views --drop-types` → `gunzip`\|`psql` the dump → restore the `storage/app/public` media tree → fix `base_url` from the node's `.env`), proven by a real local round-trip: the **full backup** round-trips everything the demo needs — site-identity `SiteSetting`s, the logo file, and `appearance_background_image` media all carry wholesale (the background-image bug is in the selective portability-bundle path, which the demo does **not** use). `demo:reset` retained for local/dev; the node's reset cron points at `demo:restore`. **Faker/`--no-dev` blocker (§6) is moot** under restore-from-blob (Faker only runs in the authoring env at snapshot time; the node only restores). Header/footer duplication (§5) now rides the blob too (no longer seeded from code). FM owns provisioning/cron/blob-push + baseline alignment. | A, C, B, D, 329 |
| **F** | **Portal pages — sign-in / dashboard / account styling** ✅ *(session 337)* | Styled the three surfaces into one branded visual language **and consolidated the portal onto `/members`** (auth stays at `/system`): retired the duplicate `system/account` dashboard (now a redirect alias) + the split `edit-account`/`change-password` pages, combined both account forms onto `/members/account`, rewired the `portal` nav, and made `layouts.portal` load the widget asset bundle so the dashboard `BarChart` renders. `portal.*` tour anchors reserved. See the 337 log. | E |

**Order:** A ✅ → **C (326) ✅** → **B (327) ✅** → **D (328) ✅** → **329 (upload lockdown) ✅** → **E (336) ✅** → **F (337) ✅** → **driver.js tour (338) ✅**. The driver.js tour framework (the marketing-ready gate) **shipped at session 338** — a multi-page guided walkthrough of the demo CRM in the Filament admin (interactive contact→record→ledger flow, demo-safe Roles/Import showcases that respect the `demo` lockdown, a dashboard launcher). **Demo v1 is complete.** See the 338 log.

**Count:** ~6 sessions for demo v1. E+F stayed separate (E reshaped to restore-wiring, F is the portal styling pass).

### Downstream follow-ons (hooks reserved now, not in v1 sequence)

- **Driver.js tour build** — ✅ **shipped at session 338** (the marketing-ready gate; demo v1 complete). The reserved `events-index.*` / `event-lp.*` / `portal.*` anchors stay for a later **public-site tour** (the admin tour did not consume them). A reusable **in-app contextual-help tour** was scoped this session — see the Post-Beta backlog stub in `session-outlines.md`.
