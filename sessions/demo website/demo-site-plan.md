...
| **D** | **Per-page permission + locked landing** *(session 328)* | **Pivoted from `restricted_roles` to a `locked` flag + `edit_locked_pages` permission** (a `status` value would break `published()` visibility; see the 328 prompt's "Design decision"). Migration + `PagePolicy` + page-builder save-API enforcement + a permission-gated lock toggle ("Published & Locked") + `demo:reset` locks the demo site's content pages; plus trim the demo role's `navigation_menu` to view-only. §2a's `restricted_roles` design is superseded. | — (independent) |
...
**Order:** A ✅ → **C (326) ✅** → **B (327) ✅** → **D (328)** → **329 (upload lockdown)** → E → F (D still unblocked, can interleave; E gates on everything as it assembles the curated pages).
