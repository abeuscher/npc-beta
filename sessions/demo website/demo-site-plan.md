...
| **F** | **Portal pages — sign-in / dashboard / account styling** *(session 337)* | Style three already-built-but-unstyled portal surfaces — `PortalLogin` (sign-in card), a member dashboard (hero + `BarChart` + `TextBlock`, illustrative content), and `PortalContactEdit`/`PortalChangePassword` (account edit) — into presentable pages with a consistent hero + look-and-feel; reserve `portal.*` tour anchors. Presentation only; the page-builder rendering of the seeded `system/*` pages already works. | E |

**Order:** A ✅ → **C (326) ✅** → **B (327) ✅** → **D (328) ✅** → **329 (upload lockdown) ✅** → **E (336) ✅** → **F (337)**. The driver.js tour framework (the marketing-ready gate) follows F.
