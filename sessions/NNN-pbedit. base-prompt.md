...
7. Read the leverage surfaces before touching them:
   ...
   - `resources/js/page-builder-vue/components/fields/RepeaterField.vue` + `RepeaterRowField.vue`, and `app/Widgets/PricingChart/PricingChartDefinition.php` (+ `template.blade.php`) — the arbitrary-repeated-units surface that Phase 2b collapses to a structural add/remove/reorder table once text moves inline.
