...
- **Discoverability is progressive and selection-scoped — part of Phase 2, not optional polish.** Editability is never revealed page-wide; selecting a widget arms its editable regions, and the unselected preview stays pixel-clean (preserves WYSIWYG / honest visual review). Empty slots show muted schema-`label` ghost text ("Add {label}…"); non-empty regions reuse the dormant `.inline-editable` hover/focus CSS (`public/css/admin.css:677–692`) + a `text` cursor; the whole slot (incl. padding) is a generous click target. The session prompt's "Phase 2 — discoverability spec" is canonical for this.
