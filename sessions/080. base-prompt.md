We are about to begin a new session: **080. System Email Preview Wizard**.
...
2. Open `sessions/080. System Email Preview Wizard.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/080. System Email Preview Wizard — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit to a feature branch named `session-080`, and notify the user. Do not push — the user will push and merge when ready.
