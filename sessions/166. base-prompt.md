...
We are about to begin a new session: **166. Appearance Controls — Docs Finalization & Widget Spacing Harmonization**.
...
3. Open `sessions/166. Appearance Controls — Docs Finalization & Widget Spacing Harmonization.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/166. Appearance Controls — Docs Finalization & Widget Spacing Harmonization — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Archive the previous session**: move all files matching `sessions/165. *.md` into `sessions/archived/`. This includes session 165's base prompt, session prompt, and log. Skip silently if those files don't exist or have already been archived. Session 166's own files stay in `sessions/` until the next session's close.
- **Commit**: stage all changed files (including the log, updated outlines, and any files moved into `sessions/archived/`), commit on the current `session-166/N` branch, and notify the user. Do not push — the user will push and merge when ready.
