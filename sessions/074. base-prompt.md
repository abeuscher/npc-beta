We are about to begin a new session: **074. Products & Checkout**.
...
2. Open `sessions/074. Products & Checkout.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/074. Products & Checkout — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
- **Update session-outlines.md**: move this session's title into the Completed Sessions table. Review remaining stubs and update if this session's work affects them. Note that the Stripe Checkout session creation pattern established here is the template for event ticketing and membership payment in future sessions.
- **Commit**: stage all changed files (including the log and updated outlines), commit to a feature branch named `session-074`, and notify the user. Do not push — the user will push and merge when ready.
