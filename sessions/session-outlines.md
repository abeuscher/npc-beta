...
- **Launch (replaces "Beta One" as the release target — session 369 replan)** — release = three launch gates passed: **Gate 1**, the public site + demo look professional to paid traffic (essentially passed; one floating session remains — demo tighten-up from designer feedback + the owner's final page/component review, combined); **Gate 2**, safe to hold one real client's money and donor PII (donations end-to-end with automatic tax receipt, event ticketing including the $0 path, backup/restore proven, Privacy + ToS live); **Gate 3**, the owner believes it's secure (the Security Hardening track, internal-review bar). **Nothing goes live until Gates 2 and 3 both pass.** The ratified schedule (16 sessions + 1 floating, ~30–40 session budget, end-of-July target) lives in `release-plan.md` § Launch schedule; the plain-English steering copy is `sessions/launch-plan-summary.md`. **Superseded at session 376:** the launch schedule is formally **parked** (owner pivot at 374, ratified at 376; per-position dispositions recorded in `release-plan.md` § Launch schedule). The active spine is now the **Plugin Architecture arc** — canonical plan `sessions/plugin-architecture-plan.md`, arc summary in `release-plan.md` § Plugin Architecture arc. **Reframed out at session 385 (owner ruling):** there is no launch target — no security review or marketing is planned; the finish line is the **end-state architecture** (plan doc § Stage D — full decomposition), followed by the niche-distribution follow-on arc (§ Stage E: marketing sites + config packages per vertical). The gates and their freight stand as history and resurrect only if a real client appears, at which point the project pivots to serving them.
- **Security Hardening track (Gate 3)** *(promoted at the 369 replan from the post-Beta "Security Audit Ready" milestone named at 358)* — the self-driven enumerate-and-fix pass is now **launch-gating**, decomposed into five sessions S1–S5 (perimeter/CSP · fleet-endpoint second lock · XSS containment + exposure · two adversarial passes absorbing the portal-scoping and permission-matrix E2E suites). Internal review is the release bar; a paid external audit is explicitly not the bar (post-launch, owner-procured if ever). Track doc: `sessions/tracks/security-hardening.md`. **S4/S5 parked at session 376** with the launch suspension; the findings register and residuals stand intact for resurrection. **Reframed out at session 385** (with the launch itself): S4/S5 leave the forward path entirely — no security review is planned; the register + track doc remain the resurrection map if a real client appears.
...
### Editor↔Public parity — homogenize widget hydration + layer the admin reset *(post-Beta — architectural; surfaced 333. Track 1 hydration ✅ 355; Track 2 admin-reset interim-fixed 354, clean Tailwind-v4 path still gated on Filament v4)*

The page-builder canvas renders **public** widget HTML/CSS/JS inside a **Filament admin** document, so two cascades and two JS lifecycles share one `window`. Every editor↔public parity bug traces to that one fact. Session 333 fixed the symptoms (the uneven-grid CSS, the pager rendering Swiper's default dots instead of the designed pager) and stood up a standing parity harness, but the *cause* is structural: the editor re-implements widget **hydration** (HTML → correctly-layered CSS + libs loaded + Alpine initialized) as a bespoke second copy of what the public site does, and the copies drift. (Session 340 brought the *public* chrome render to parity; **session 354** closed the *editor* chrome-render instance — header/footer wrapper parity — and took the Track-2 interim below; **session 355** homogenized the widget hydration — the JS-lifecycle half — into one shared `hydrate()` both surfaces call, retiring the bespoke editor copy.) Two independent tracks, ~3 sessions + a risk buffer:

- **Homogenize widget hydration (~1–2 sessions, knowable).** **→ ✅ Resolved at session 355.** Consolidated into one shared `resources/js/shared/hydrate.ts` both surfaces call — public `hydrate(document, { firstInit: true })` (→ `Alpine.start()`), editor `hydrate(scopeEl, { libs })` (→ the destroy/`initTree` re-init dance) — with the bespoke `useLibraryLoader.ts` retired. The shared module imports none of the libs (operates over the `window.*` globals), so neither build graph duplicates Alpine/Swiper (build emitted a standalone `hydrate-*` chunk). `AdminPanelProvider`'s head-hook lib emission stayed surface-owned (the admin-page equivalent of `public.js`'s bundling — Livewire slots need libs present synchronously). Verified against the green 333 + 354 parity net (19/19); fast Pest 2930/0 (no delta — client-side). See the 355 log.
...
### In-app contextual tours via the help system *(post-Beta — foundation BUILT at session 362: the single-area tour engine)*

A per-view guided tour as fast in-app onboarding for the tool the user is currently in — distinct from the *marketing* tours (demo walkthroughs launched from the dashboard). Each tour is **tied to a single view** and run from the **help system**, which is already per-view: the `?` slide-over resolves a help article for the current route (`HelpArticleService::forRoute`), so a tour is just one more per-view asset, and the slide-over (or a button beside the `?`) is the natural "Take a tour of this page" launch home.

**Session 362 settled the engine question (gate decision: the tour redesign IS this feature's foundation).** The 338 multi-page walkthrough — cross-page navigation, `localStorage` resume, the page-group `controller.js` + `state.js` layer, the role-gated showcase fallbacks — is **retired**; in its place is exactly the engine this feature needs: `resources/js/admin/tour/` drives an ordered step list within one area (`registerTours`/`startTour(id)`), with the owned **`data-tour` anchor convention**, the `anchors.js` primitives (`resolveAnchor` incl. nav/navGroup lookups, `waitForElement`, `waitForStableRect` settle-before-position, Alpine-safe `ensureNavVisible`), entry-first background anchor resolution (fast first paint), and a one-shot sessionStorage launch flag as the only cross-page surface. The three demo tours (dashboard/crm/cms defs under `tours/`) are just registered tour objects — a contextual tour is one more `{id, steps}` def plus a launch surface.

What remains for this feature: (1) the **help-system launch surface** (slide-over button → `startTour(<view-tour-id>)`); (2) per-view step scripts — keep each tour **inside one view** (the 362 engine supports a contained hop via the launch flag, but a contextual tour shouldn't need it); (3) **sanitize the step copy** if it is ever authored in editable help content — driver renders popover text as HTML, so author-editable tour text is a stored-XSS surface (the 338 caveat; today every word is static and ours). Step-script home (help-article frontmatter vs a companion definition) is a sprint-time design call; the pages to tour are named at scoping, not now.

---

## Active tracks

- **Plugin Architecture** — *active — THE current spine (opened session 376)* — decompose the app into a thin core + contracted plugins, per-niche distributions as manifests. Canonical plan: `sessions/plugin-architecture-plan.md` (that doc is the track doc; no separate track file). Arc P1–P11 in three stages, ~9–15 sessions total. **Stage A, in-repo boundaries:** P1 ✅ (session 377) pilot shipped — LogoGarden extracted to `plugins/LogoGarden/`, self-registering via `config/plugins.php` + core `PluginServiceProvider`, `docs/plugin-contract.md` **v0.1.0** live (4 surfaces PROVEN, 9 DECLARED), thumbnail resolution generalized (`WidgetDefinition::thumbnailDir()`), guards extended to `plugins/*`, fast Pest 3116/0 → **P2 ✅ (session 378)** widget boundary real across all 41 — the twelve out-of-contract templates (plus a thirteenth the guard surfaced: the shared buttons partial, promoted to core `<x-widget-buttons>`) retrofitted onto declared contracts; `dataContracts()` named multi-contract seam live (single-contract widgets untouched); new fail-closed arms `fund` / `membership_tier` / `navigation_menu` / `portal_member` (non-leak tested) + `SOURCE_SERVICE` (super-admin gate in-arm); `site_name` page-context token; standing guard `WidgetTemplateBoundaryTest` bans template reaches with **zero allowlist**; `WidgetDefinition::baseDir()` rider; contract doc **v0.2.0**; fast Pest 3144/0 → **P3 ✅ (session 379)** admin-shell sockets — `->routes()` closure decomposed into eleven per-feature `routes/admin/` files with **exact route-list parity** (274 routes byte-identical, verified twice); plugin admin socket live (`app/Plugins/` — `AdminContribution` + `PluginAdminRegistry`, consumed by the panel provider; plugin in-panel routes force-wrapped in `Authenticate`; permissions swept idempotently by `PermissionSeeder`, granted to no shipped role); proven by the SocketProbe test fixture incl. the remove-the-line mirror; standing guard `PluginAdminCssGuardTest` enforces the core-owned admin theme; contract doc **v0.3.0**, surfaces 3/4 DECLARED-with-mechanics per honesty rule; fast Pest 3156/0 → **P4 ✅ (380)** Payments foundation module — Stripe rails carved into `plugins/Payments/`, the first shipping non-widget plugin; core `CapabilityRegistry` (surface 13 **PROVEN** — five vertical guards + the dashboard integration widget consume `enabled('payments')`, fixing its never-shows-connected bug) + core-owned `CheckoutProvider`/`PaymentModeProvider` contracts so nothing in `app/` references the plugin (standing guard `PaymentModuleBoundaryTest`, zero allowlist); route-list parity 274/274; vanish proven via the strip-the-line `PaymentsRemovedTestCase` (checkouts degrade to not-configured, free/$0 paths untouched); `laravel/cashier` removed (unused; two dead vendor routes dropped), `stripe/stripe-php` direct; ProductPriceObserver backfill absorbed; contract doc **v0.4.0**; fast Pest 3189/0 → **P5 ✅ (381)** Events vertical module — the first domain vertical carved into `plugins/Events/` (controllers/routes, quantities service, observers, three mailables, reminders command + schedule, policy, `EventResource` + four pages, both import wizard pages, seven widgets nested under `Widgets/`, the events importer); **surfaces 3/4 PROVEN** (first shipping admin-page plugin; extracted-vertical permission nuance — the `event` permissions stay core-seeded), **surface 8 PROVEN** (core `ImporterContribution`/`ImporterRegistry`; hub, CSV templates, fixture runner, demo sources resolve through it), **surface 10 PROVEN events-slice** (core `CheckoutSettled` dispatched by Payments' webhook branch; the plugin's listener owns fulfillment; no-listener = 200, rows stay pending); route parity 272/272 after every phase; presence matrix executable (enabled twin / `EventsRemovedTestCase` vanish mirror with data kept / Events-without-Payments free-comp-paid); standing guard `EventModuleBoundaryTest` (zero allowlist); landing-page factory lifted to core (`EventLandingPageFactory`); models/schema stay core with the events-owned annotation recorded in plan doc § 6.7 (the P7 redraw input); contract doc **v0.5.0**; fast Pest 3226/0 → **P6 ✅ (382)** First Composer Package & Migration Squash — Stage B entered: LogoGarden is the first true composer package (`nonprofitcrm/logo-garden`, own composer.json with explicit version 0.1.0, root path repository + bound `^0.1` require, committed lock, `composer validate --strict` clean, Dockerfile manifests-before-source COPY fix in both stages proven locally + in CI's image-building job); **no Laravel auto-discovery** — `config/plugins.php` confirmed sole activation + ordering authority through Stage B (contract surface 1 amended; standing guard `PluginPackageGuardTest` enforces no-auto-discovery + lock-pinning + the accepted root PSR-4 overlap, globbed so future packages are covered free); the absorbed **T1 squash** landed (three live migrations → regenerated dump, zero test fallout, 348-procedure identity check clean + re-proven cold in CI; events tables stay in the dump until P7); contract doc **v0.6.0** (new Package shape (Stage B) section); route parity 272/272; fast Pest 3229/0 → **P7 ✅ (383)** Events Vertical as a Package & Squash Boundary Redraw — Events is the **second composer package** (`nonprofitcrm/events` 0.1.0, P6 mechanics verbatim, per-plugin Dockerfile manifest lines, in-image proof local + CI); **the squash boundary redrawn**: the three events tables left the core dump for plugin-owned byte-faithful create-table migrations (raw DDL from the dump, `loadMigrationsFrom`), deterministic install order core dump → enabled plugins' migrations → seeders, both per-composition identity checks clean — full composition **byte-identical** modulo migrations rows; events-absent = **the repo's first fresh-install-without-a-vertical**, which surfaced + fixed the one unconditional core read of a plugin-owned table (`PageContextTokens` → type-gated per the established pattern); vanish semantics settled **disabled ≠ uninstalled** (fixture registers the migration path directly, proven on a pristine DB); **test-monolith dismantling begun** — 29 events-slice files → `plugins/Events/tests/` as the third phpunit testsuite (`Events`), 3-deletion logged reduction riding along; new standing guard pins plugin tables out of the core dump; models/factories stay core (unscheduled, composition-safety consequence recorded); route parity 272/272; contract doc **v0.7.0** (surface 5 PROVEN); fast Pest **3229 → 3227/0** → **P8 ✅ (384)** Per-Install Activation & Workspace Assembly — **Stage B closed**: the tier-1 flag-flip path is real — env-backed `PLUGINS_DISABLED` (comma-separated derived kebab handles) through new `config/plugin-activation.php`, subtracted by `PluginServiceProvider`'s filter at the exact strip-the-line seam the removal fixtures prove (**runtime-disabled ≡ the proven mirrors by construction**; `config/plugins.php` untouched as installed superset + sole ordering authority; unknown handles fail the bootstrap loudly); workspace assembly proven as the **superset image** (owner-accepted reading, Open Gate): one locally built image ran the full composition (migrations 117, widgets 41) and `PLUGINS_DISABLED=events` (migrations 114, widgets 34, POST 405/JSON 404) and the **enable-later flip end to end** (flag off → restart → `migrate` applies the 3 plugin migrations); tier-1 runbook recorded; +21 tests incl. LogoGarden's first vanish coverage; `.dockerignore` rider (stray local `.env.production` no longer baked into local image builds); contract doc **v0.8.0** (surface 1 two-layer rewrite + flip runbook); route parity 272/272; fast Pest **3227 → 3248/0** → **P9 ✅ (385)** One Plugin to Its Own Repo & the Extraction Runbook — **Stage C-lite entered**: LogoGarden made the round trip to the owner-created public `abeuscher/crm-plugin--logo-garden` repo (naming precedent `crm-plugin--{kebab-handle}`) — composer VCS repository + `v0.1.0` tag with the explicit version field dropped (the path-repo determinism device inverts under tags), vendor-relative asset paths with the surface-11 bundle-content gate run and passed, guard coverage extended over `vendor/nonprofitcrm/*` via the shared `extractedPluginPackageDirs()` helper (six scanning guards + `PluginPackageGuardTest`'s tagged-VCS pinning), Dockerfile manifest-COPY cleanup, in-image proof local + CI-cold, activation proven location-independent (flag vanish tests unchanged); **`docs/plugin-extraction-runbook.md` shipped** as the repeatable procedure; contract **v0.9.0**; fast Pest 3249/0; route parity 272/272 → **P10 ✅ (386)** Distribution Manifest & Central Build — **Stage C-lite closed**: `distribution.json` at the repo root is the plugin-set + ordering authority (per-entry provider FQCN, honest source kind `folder`/`path`/`vcs`, package + bound constraint where one exists; array order = load order); `config/plugins.php` is its **generated artifact** (`plugins:manifest-sync`, single reader `App\Plugins\DistributionManifest`, fail-loud validation, byte-stable output — same three FQCNs, same order; remove-the-entry ≡ remove-the-line); composer.json + Dockerfile stay hand-maintained, pinned by standing guard `DistributionManifestGuardTest` (regeneration idempotence — verified to fail on a hand edit; per-source-kind lock pinning + repository/require alignment; no unmanifested surfaces: lock packages, plugins/ dirs, Dockerfile COPY lines, phpunit testsuites); `PluginPackageGuardTest`'s `EXTRACTED_PLUGIN_PACKAGES` const replaced by a manifest read; **composition-by-manifest proven by verified run** (committed manifest = full composition — migrations 117, widgets 41, events surface live; Events-entry-removed manifest = the 384 events-disabled facts — migrations 114, widgets 34, POST 405/JSON 404 — by the manifest edit alone, Events code still in the image inert = the recorded Stage E boundary); deploy.yml untouched (zero diff); contract doc **v0.10.0**; route parity 272/272; fast Pest **3249 → 3263/0** → **D1 ✅ (387)** Payments Packaged & Extracted — **Stage D opened**: Payments walked its manifest source kind `folder` → `path` (Stage B mechanics — package `nonprofitcrm/payments` 0.1.0, the 386 same-commit guard interlock held) → `vcs` (`github.com/abeuscher/crm-plugin--payments`, public, tag `v0.1.0`, lock-pinned to the exact tag commit) via the runbook's second run — **first extracted foundation plugin** (the surface-13 capability seam held: nothing outside the plugin edited beyond three test file-read re-points + two comment mentions) and **first dependency-carrying package** (`stripe/stripe-php ^16.6` package-owned, root's direct line dropped same-commit, lock resolution identical at v16.6.0 transitive — the recorded pattern in contract § Package shape); guards needed zero manual edits (both sets derive from the manifest); gate 4 recorded n/a (no assets); in-image proof clean local + CI; runbook deltas folded (COPY-line order per manifest path-entry order; composer-before-artisan at step 5; no-assets gate wording); contract doc **v0.11.0**; route parity 272/272; fast Pest 3263/0 (baseline exact) → **D2 ✅ (388)** Events Extracted — the known-edges runbook run (third extraction): `nonprofitcrm/events` to the owner-created public `crm-plugin--events` (tag `v0.1.0`, manifest `path` → `vcs`, lock at the exact tag commit) — **first migration-owning extraction** (the removal/flag fixtures re-point their migrator reaches to `vendor/nonprofitcrm/events/database/migrations`; disabled ≠ uninstalled survives with assertions unchanged), **first testsuite-carrying extraction** (phpunit `Events` suite re-pointed to the vendor path — same 29 files, 236 tests, identical counts under paratest + CI; the § 6.3 central-build disposition made real; test membership now a two-posture rule), **gate 4 live at scale** (five vendor-relative asset rewrites in three definitions; bundle content proven incl. the EventsListing→BlogPager core cross-input and swiper), and **`plugins/` emptied until D3's carve** (logged drift: three guards needed the tolerate-empty relaxation, not the surveyed one — folded into runbook step 8; root PSR-4 stays until D12); in-image proof = all three plugin packages fetched from GitHub inside `docker build`, `plugins/` absent; contract doc **v0.12.0**; route parity 272/272; fast Pest 3263/0 (387 baseline exact) → **D3 389 ✅** Donations Carved — the block opener: behavioral surface into `plugins/Donations/` (refills `plugins/`, fourth manifest entry), **surface 7 PROVEN** (core `EmailTemplateRegistry`, the two donation handles its first contribution; defaults() resolves core array → registry → empty, seeder loops the registry), **surface 10 donations-slice PROVEN — the first cross-repo inversion** (`crm-plugin--payments` v0.2.0 inverts all three donation-reaching branches to `CheckoutSettled` / new core `InvoiceSettled` / `InvoicePaymentFailed`; three plugin listeners own the bodies verbatim; require/constraint `^0.2` same-commit; rider `crm-plugin--events` v0.1.1 — listener metadata guard + removal-mirror exclusivity assertion generalized, the self-filter rule now normative), new `Donations` testsuite (9 moves + 1 split, 0 deletions), twins/mirrors/flag/boundary guard landed, contract **0.13.0**, routes 272/272 byte-identical, fast Pest 3263 → **3309/0** → **390 ✅** Donations Packaged — `nonprofitcrm/donations` is the fourth path-repo package (P6 mechanics verbatim, manifest `folder` → `path`, in-image proof clean) and the four donation tables left the core dump for plugin-owned byte-faithful migrations (the second squash-boundary redraw — largest FK web, the RESTRICT-FK-as-behavior + sequences-travel notes; **both identity checks verified clean**; fixtures gained migrator paths; setup-checklist fund row route-gated, the one composition-safety find); ADRs 0001–0006 shipped (`docs/adr/`); contract **0.14.0**; routes 272/272; fast Pest 3309 → **3314/0** → **391 ✅** Donations Extracted — the D3 block closed: `nonprofitcrm/donations` to owner-created public `crm-plugin--donations` (tag `v0.1.0`, manifest `path` → `vcs`, version field dropped, lock at the exact tag commit) — second migration-owning + testsuite-carrying extraction (fixtures + the 14-file `Donations` suite re-pointed to the vendor path, identical counts), gate 4 proven on the one DonationForm asset, in-image proof clean (all four plugin packages VCS-resolved, `plugins/` absent, zero Dockerfile plugin COPY lines), zero new guard relaxations, no runbook deltas; contract **0.15.0**; routes 272; fast Pest **3314/0** (390 baseline exact) → **392 ✅** Memberships Carved — the D4 block opener: the smallest vertical yet into `plugins/Memberships/` (checkout + observer/policy + three admin resources incl. the Contact-model MemberResource + importer + MembershipStatus/PricingChart, fifth manifest entry refilling `plugins/`); **surface 10 memberships slice PROVEN — the second cross-repo inversion** (`crm-plugin--payments` v0.3.0: checkout branch → third `CheckoutSettled` dispatcher, renewal branch a pure branch-delete since `InvoiceSettled` was already dispatch-always; two plugin listeners; no failure listener — never handled, preservation cuts both ways); new `Memberships` testsuite (6 files/65 tests); routes 272/272 byte-identical; fast Pest 3314 → **3349/0**; contract doc **v0.16.0** → **393 ✅** Memberships Packaged & Extracted — **the D4 block closed in one combined session** (the D1 compression precedent): `nonprofitcrm/memberships` fifth path-repo package (P6 mechanics verbatim) → **the third squash-boundary redraw** (the two membership tables to plugin-owned byte-faithful raw-DDL migrations — smallest yet: zero inbound core FKs, **no sequences** (first with none), `contact_id` RESTRICT-as-behavior covered by the existing `CascadeDeleteTest` net; **both identity checks verified clean**; composition-safety gated on `Route::has('membership.checkout')` — the resolver arm, the PortalSignup template, SignupController tier handling, and the check's one find: `DatabaseSeeder`'s unconditional `MembershipTierSeeder` call; twin fixtures' migrator paths landed vendor-relative directly, the same-session compression nuance) → **the fifth runbook run** to owner-created public `crm-plugin--memberships` (tag `v0.1.0`, manifest `path` → `vcs`, version field dropped, lock at the exact tag commit) — third migration-owning + testsuite-carrying extraction (the `Memberships` suite, 8 files/71 tests, identical counts from `vendor/nonprofitcrm/memberships/tests`), gate 4 proven (pricing-chart rules from the vendor path), in-image proof clean (**five packages VCS-resolved**, `plugins/` absent), **zero new guard relaxations, no runbook deltas**; routes 272/272 byte-identical; fast Pest 3349 → **3355/0**; contract doc **v0.17.0** → **394 ✅** Member Portal Carved — the D5 block opener: the portal surface into `plugins/MemberPortal/` (sixth manifest entry refilling `plugins/`; six widgets, six controllers + all 17 routes byte-identical, the `PortalEmailChange` mailable + three views), the first plugin-owned **auth stack** (guard/provider/broker config-injected at `register()`, the `portal.auth` alias at `boot()` — the first plugin-owned middleware alias; `config/auth.php` + `bootstrap/app.php` keep no portal entries), the **`member-portal` capability** (surface 13's second, presence-is-enablement) gating the **five** core `auth('portal')` reads (logged drift — the survey named two; an undefined guard throws: PageContext, PageController's member branch → 404, ActivityLogger, the collision check, the `portal_member` arm), the **Events v0.1.2 rider** (`portal.events.register` registers only when the capability is present — vanishes, never 500s, portal-absent; lock-pinned same-commit), and the logged mailable disposition (`PortalEmailVerification` + `PortalPasswordReset` stay core as model-adjacent — their senders are `PortalAccount`'s framework hooks, § 6.7); new `MemberPortal` testsuite (4 files/37 tests — PortalAuthTest moved + the free-signup block split); routes 272/272; fast Pest 3355 → **3384/0**; contract doc **v0.18.0** → **395 NEXT** Member Portal Packaged & Extracted (**the D5 closer, one combined session** — the D4 compression precedent; prompts drafted with the redraw input front-loaded: `portal_accounts` → `portal_password_reset_tokens`, **`portal_accounts_id_seq` travels** — the second sequence-carrying redraw, one outbound SET-NULL FK, zero inbound core FKs; the portal-*table* composition-safety survey — ContactResource/EditContact reads, the topology seeders, DemoPortalMemberSeeder — capability-gated; one Open-Gate item: owner-created `crm-plugin--member-portal`, blocks the extraction phase only; contract → 0.19.0). **Stage B, packages in one workspace — ✅ complete:** P6 first composer path-repo package + the absorbed T1 squash (boundary redraw deferred to P7) → P7 vertical→package with own migrations/tests + the squash-boundary redraw and per-composition fresh-install check (test-monolith dismantling begins) → P8 the per-install activation layer + superset-image workspace assembly. **Stage C-lite, separate repos proven once — ✅ complete:** P9 ✅ one plugin to its own repo + **the extraction runbook** → P10 ✅ the distribution manifest + composition-by-manifest proven (deploy.yml untouched; the health-payload plugin manifest stays the future FM coordination item per protocol). **Stage D, full decomposition (owner-committed at the 385 reframe — the finish line; ACTIVE as of 386 close):** every plugin a package in its own `crm-plugin--*` repo over a thin core — **D1 ✅ (387)** Payments packaged+extracted → **D2 ✅ (388)** Events extracted → **D3 (389–391) ✅** Donations (carve 389, package 390, extract 391 — block closed) → **D4 (392–393) ✅** Memberships (carve 392, package + extract combined 393 — block closed) → **D5 (394–395)** Member Portal (carve ✅ 394; package + redraw + extract combined at 395 NEXT) → D6–D8 the remaining verticals carved/packaged/extracted (Blog/Posts, Forms, Products-disposition) → D9 integrations → D10 the two widget packs (`content-widgets` ~13, `admin-widgets` ~4; Nav stays core; vertical widgets ride with verticals) → D11 test redistribution + per-plugin CI → D12 endgame (model moves, root PSR-4 retirement, core-slimming audit, contract 1.0.0). ~15–21 sessions; ladder + settled decisions in plan doc § Stage D. **Stage E, the follow-on arc:** niche distributions — marketing sites + config/seed/vocabulary packs per vertical (P11's layer promoted from optional tail to the whole next arc; planned when Stage D is far enough along). Execution posture (owner directive, 376): binary success criteria, behavior-preserving, minimal UAT, decisions front-loaded into session prompts.
- **Security Hardening** — *active (opened at the 369 launch replan) — this track IS Gate 3 of the launch* — five sessions; **S1 ✅ (session 370)**, **S3 ✅ (session 371)**, **S2 ✅ (session 372)**, S4/S5 pending (schedule order S1 → S3 → S2 → S4 → S5): **S1 ✅** perimeter headers + enforced public CSP (admin Report-Only) + self-hosted Quill + prod-hardening defaults + the Stripe live-key generator guard + the demo-role seeding gate + an admin-editable CSP host allow-list, **S3 ✅ (session 371)** XSS write-path containment + the accidental-exposure audit (absorbed release-plan #32c Path A — fixed the event-registration `withoutEvents` sanitizer bypass + a SiteSetting save-time backstop, neutralized CSV/XLSX formula-injection, proved member-page scoping, closed the write-time `allowedSlots` hole + added publish/collection warnings; **no field-level sensitivity tagging** per owner decision), **S2 ✅ (session 372)** app-layer second lock on the five FM `/api/*` endpoints (the track's one boundary-touching session — additive contract bump **v2.6.0 → v2.7.0** shipped: a `VerifyFleetAgent` per-install shared-secret `X-Fleet-Gate-Key` gate behind the mTLS termination, TLS-independent so it survives an nginx regression the single per-location `if` would not, inert until provisioned, worst-first blob + recover; shared-secret chosen over cert-fingerprint forwarding; `/api/logs` PII resolved as an accepted residual), **S4** adversarial pass over the public write surface + the portal-scoping E2E suite (absorbs F2; the form-spam/CAPTCHA inbox item lands here), **S5** permission-matrix E2E + demo-server hardening (absorbs F3; assembles the Gate-3 findings register for the owner walkthrough). Bar: self-driven internal review — every enumerated item fixed or consciously accepted. **Residuals on the register:** S1 — admin CSP Report-Only (not enforced), analytics hosts managed via the admin allow-list; S3 — help-article content unsanitized (dev-authored via HelpSync), page head/body snippets raw by design, email scalar `{{tokens}}` unescaped (no custom-field token path), WidgetPreset no independent sanitizer (all four accepted by design); S2 — `/api/logs` returns raw Laravel log lines verbatim (accepted: now behind two locks, logging-hygiene-at-write-site is the real control, not lossy read-time redaction). Evidence base: the session-368 security surface map, folded into the track doc as Appendix A. Track doc: `sessions/tracks/security-hardening.md`. **S4/S5 parked at session 376** with the launch suspension (owner ruling; per-position dispositions in `release-plan.md` § Launch schedule); the findings register and residuals stand intact for resurrection. **Reframed out at session 385** with the launch itself — off the forward path; resurrects only on a real client.
...
- **Client Billing & Account** — *CRM lane complete — CB1 ✅ (366, boundary-touching, v2.6.0) + CB2 ✅ (367, non-boundary) shipped; CB3 ❌ withdrawn (368, false premise); FM-B1…B5 FM-side pending* — how client organizations pay, see their account, and get suspended / offboarded. Stripe holds the money + risk; **Fleet Manager owns billing centrally** (polls Stripe daily, derives the suspension state machine + clocks, pushes state to nodes); each node gets a **read-only "My Account" window** + a hard **suspension gate**. **No vendor-Stripe anything CRM-side, by design** — money logic is FM-repo-only; the node enforces a flag and renders a pushed document. **CB1 (366, boundary-touching) shipped the node half:** contract **v2.5.0 → v2.6.0** (additive) — the `SUSPENSION_STATE` flag + one enforcement middleware (`admin_locked` → 403 admin lock with the public site / donations / member portal staying up; `site_off` → 503 public maintenance with FM `/api/*` up; absent = none; unrecognized fails safe to none), the display-only billing-state document reader (backup-excluded pushed JSON, null-object on missing/malformed), and a never-red / excluded-from-worst-of `suspension` `/api/health` subcheck. **CB2 (367, non-boundary) shipped the account window:** a read-only "Account" Filament page rendering exclusively from the pushed document (plan, plain-English status badge, next invoice + line items, read-only billing contact, Stripe-hosted portal hand-off, staleness footer), self-hiding where no document exists; a prominent on-page pre-lock banner plus a slim panel-wide strip (both gated on the new `manage_account` permission, which is seeded but granted to **no shipped role** — super-admin-only by design); and the two convention-drift guard cases that make the two-Stripes separation mechanical. **Remaining CRM-side: nothing.** CB3 (the demo-conversion cleanup command) was **withdrawn at session 368 on a false premise** — it assumed a prospect's server runs in demo mode and is flipped to production at conversion, leaving a demo login account and blanket-locked pages to sweep up. A prospect's node is an ordinary production node and has neither. 368 corrected the track doc's § Design decision 4 (adding a § Vocabulary table fixing **"demo" = the shared public sandbox, and nothing else**) and shrank FM's conversion sequence from seven steps to four; no code changed. **FM-side (separate repo):** FM-B1 Stripe sync → FM-B2 push + verify → FM-B3 operator UI → FM-B4 trial + conversion (**re-read its scope — it shrank at 368**) → FM-B5 first-conversion drill. **FM-B2 is what lights up CB2's Account page on real nodes** — until FM pushes a billing-state document, the page is inert and hidden everywhere, by design. Gate `first-customer` (commercial readiness — **not** Beta-1-blocking). Full design, session decomposition, open questions, and FM handoff: `sessions/tracks/client-billing-and-account.md`.
...
- **Test Audit** — *Cycle 2 ✅ closed (364) — next trigger ≈ 414 or forcing function* — lifted at A003 close. Recurring 50-session cadence; single audit session by default, carve-out shape if dimensions overflow. Four audit dimensions: D1 Playwright spec discipline, D2 Spec-claim integrity, D3 Mutation testing slice (existing `docs/testing/mutation-audits.md`), D4 Pest file relevance + fast/slow refresh. **Cycle 1 closed at A005** (D1 + D2: Playwright suite 28 → 16 specs, s296 `[test-integrity]` backlog drained, `docs/testing/playwright-discipline.md` shipped). **Cycle 2 closed at 364** (D3 + D4: Fleet-controller mutation slice — 326 mutants, MSI 100%, 10 `// guards:` markers, 0 deletions; D4 walk of the 60 oldest files — 53 keep / 7 update / 0 remove, the s363 flake fixed, 8 tests reclassified slow→fast; localization scoping — footprint NOT small, parked; ~290 newer files + the knockout-guard deferred to Cycle 3). Prior partial audits noted in the track doc as precursors: s094 (Pest audit), s241 (first mutation slice). Track doc: `sessions/tracks/test-audit.md`.
...
- **Last boundary-touching session in this repo:** session 372 (additive — bumped **2.6.0 → 2.7.0**; the Security Hardening track's **S2** (Gate 3), the track's **one** boundary-touching session — adds an **app-layer second gate** behind the nginx mTLS termination on all five FM `/api/*` endpoints, worst-first (`/api/backup/blob`, `/api/admin/recover`). A **per-install shared secret** (`FLEET_GATE_SECRET`), sent as an `X-Fleet-Gate-Key` header, checked constant-time in a new `VerifyFleetAgent` middleware — **independent of the TLS layer** (not cert-derived), so an nginx regression / misconfig / location-shadow that lets an un-certed request through still fails the app gate. **Backstops mTLS, does not replace it** — two locks, both must hold when enforced. **Absent secret = gate inert = exactly v2.6.0 behaviour**, so the bump is additive / no-flag-day on every running node (same shape as `SUSPENSION_STATE` absent = none). Enforced failure = app-layer JSON `401 {error: fleet_gate_unauthorized}`, distinct from nginx's plain-HTML `403`/`400` mTLS failures so FM can tell "passed mTLS, failed the second lock" apart. `HealthController::CONTRACT_VERSION` `2.6.0 → 2.7.0` via the shared `HasContractVersion` trait (`BackupController` + `RecoveryController` follow); subcheck keys + response shapes unchanged. `/api/logs` raw-log PII tension resolved as a **consciously-accepted residual** for the Gate-3 findings register — now double-locked, and logging-hygiene-at-the-write-site is the real control, not lossy read-time redaction. **FM-side absorption pending** via the Two-Repo Coordination Protocol; **order is load-bearing** — FM ships the change that **sends `X-Fleet-Gate-Key` on every request first** (harmless while the node secret is unset and the gate inert), **then pushes `FLEET_GATE_SECRET`** to the node over its existing single-key `.env` config-push channel to switch enforcement on (reverse order locks FM out for one poll cycle). One secret per install, never shared; no schema, no migration, no new inbound surface). **Prior boundary-touching sessions in this repo:** session 366 (additive — bumped **2.5.0 → 2.6.0**; the **node half of client billing** — draft entry CB1 of the Client Billing & Account track. Three additions, all inert until FM starts pushing: (1) the **`SUSPENSION_STATE` env flag** (`none` / `admin_locked` / `site_off`, **absent = none**), pushed via FM's existing single-key config-push channel, read node-side into `fleet.suspension.state` and enforced by one middleware — `admin_locked` renders a `403` suspension notice across the whole admin surface (panel, login, in-panel API groups) while public pages, donation/event/membership checkout, the member portal, backups, the scheduler, and all five FM `/api/*` endpoints stay up; `site_off` renders a `503` maintenance notice on public + admin while the FM `/api/*` endpoints stay up; an unrecognized value **fails safe to none** and logs; (2) the **billing-state document** — a display-only JSON file (`schema_version` 1: plan, status, next invoice + line items, billing contact, Stripe-hosted portal URL, suspension state/reason/since/grace-ends, trial) FM pushes over its **existing SSH provisioning channel** (not HTTP — no new inbound surface), atomic temp+rename, at `storage/app/private/fleet/billing-state.json`, the **backup-excluded** fleet-metadata dir, untouched by `demo:restore`; read node-side through a null-object reader (missing/malformed/unknown-schema → "no billing state", never influences enforcement); reason codes `delinquent`/`trial_expired`/`canceled`/`manual` select the lock-screen wording; (3) a new **`suspension` `/api/health` subcheck** — `value` `{state, billing_state_as_of}`, the enforced state + the document's `as_of`, FM's read-back that a push took effect; `green` when none / `yellow` otherwise, **never red**, **excluded from the worst-of overall status** exactly like `data_hygiene`; state string + timestamp only, no PII. **Enforcement rides the flag; display rides the document** — the gate locks correctly with no document present. **No vendor-Stripe anything exists CRM-side, by design.** `HealthController::CONTRACT_VERSION` `2.5.0 → 2.6.0` (via the shared `HasContractVersion` trait; `BackupController` follows). Eight subcheck keys now. **FM-side absorption pending at FM-B2** — refresh the cached contract copy to v2.6.0, then write the document + push the flag. CB2 (node "My Account" page + `manage_account` permission + convention-drift guards) and CB3 (demo-conversion cleanup command) are later CRM sessions). **Superseded in part at session 368 (documentation correction, no contract surface change, no version bump):** CB2 shipped at 367; **CB3 was withdrawn on a false premise** — it assumed a prospect's node runs in demo mode and is flipped to production at conversion, and a prospect's node is an ordinary production node. **FM-B4 must not build a reset-cron removal, an `APP_ENV` flip, a node-side cleanup command, or an `is_demo` write-once relaxation**; conversion pushes nothing to the node but the billing-state document. CRM-side billing work is complete at CB1 + CB2. Prior boundary-touching sessions: session 360 (additive — bumped **2.4.0 → 2.5.0**; new **`POST /api/admin/recover`** node endpoint — the FM-triggered reset for a locked-out admin (operator-mediated **admin account recovery**, promoting the session-304 "admin lockout has no recovery" flag that session 359's mandatory admin TOTP 2FA made urgent). JSON `{email, actions}` with composable `reset_2fa` / `reset_password`; always-200 `status` envelope `{contract_version, status, email, actions_applied, temporary_password, recovered_at, message}`; `reset_password` returns a one-time `temporary_password` — the **one documented exception** to "no secrets in any response," transiting once over mTLS, only the hash stored. Rides the existing nginx mTLS gate (new `location = /api/admin/recover` block in both `default.conf` + `prod.conf`), `throttle:6,1` (sensitive — matches `/api/backup/trigger`); **no app-layer identity check** — identity verified out-of-band against an operator-held external-vault PIN, no recovery secret stored in CRM or FM (procedure-not-control trade-off, accepted for the solo-operator model). Recovers the protected oldest super-admin too (`isProtected()` delete-guard deliberately bypassed — reset ≠ delete). Every reset audited via the app's existing `ActivityLogger` → `activity_logs` (subject = target `User`, event `admin_recovery`, meta `{actions, path}`), so it shows in that user's activity timeline — no new table/schema. A node-local `admin:recover` break-glass artisan command (demo-mode hard-gated) ships alongside for a node cut off from FM — not part of the contract surface. **FM-side absorption pending** — build the operator-facing control-panel UI that calls the endpoint. En passant, corrected the vestigial Spatie `config/activitylog.php` `table_name` default `activity_log` → `activity_logs`), 353 (additive — bumped **2.3.0 → 2.4.0**; new **count-only `data_hygiene` `/api/health` subcheck** sourced from session 352's `DataHygieneAudit::counts()` — `value` is the four-category non-PII breakdown `{orphan_event_pages, scrub_records, orphan_media_dirs, dead_owner_media}`; **informational, never red**, soft-yellow above a generous total threshold (default 100), **excluded from the worst-of overall status** so benign cruft never drags node health; counts read through a ~10-min `Cache::remember` to stay cheap on the polled endpoint; **count-only privacy boundary** — no raw rows/records/paths ever cross the wire, the `--deep` records mode stays node-local; v2.3.0 consumers ignore the unknown key and keep working. **FM-side absorption pending** — consume the subcheck + build the per-node maintenance/auditable toggle, handed off via `sessions/data-hygiene-handoff-from-crm.md`), 268 (additive — bumped 2.2.0 → 2.3.0; new `GET /api/backup/blob` endpoint streams the freshest backup zip from the resolved source disk; `Content-Type: application/zip`, `Content-Disposition: attachment; filename="<spatie-blob-filename>"`, `Content-Length: <bytes>`, `Cache-Control: no-store`; mTLS gate extended to the new location in both `default.conf` and `prod.conf`; per-location `fastcgi_read_timeout 600;`; `throttle:60,1`; disk-fallback rule has two layers — Layer A preference puts `local` first if present, Layer B falls through on empty across the resolved order; 404 `no_backup_available` envelope when all configured disks are empty; 500 `backup_destinations_not_configured` when `BACKUP_DISKS` resolves empty; 500 `backup_disk_error` for synchronous storage exceptions); 263 (additive `/api/backup/trigger` endpoint; bumped 2.1.0 → 2.2.0); 253 (documentation revision under v2.1.0 — no contract surface change, no version bump); 251 (additive `/api/logs` endpoint; bumped 2.0.0 → 2.1.0). **Session 291 (documentation revision under v2.3.0 — no contract surface change, no version bump):** the `/api/health` `version` field's description + examples were corrected to the build-stamped pre-1.0 `0.<session>.<iteration>` form (e.g. `0.291.1`); FM consumes this string for per-client upgrade verification (before→after, rollback). The build pipeline now reads the repo-root `VERSION`, bakes it as `APP_VERSION`, and immutably tags the published GHCR image with it (never overwrites; `latest` still moves). Response shape, status enums, and auth are unchanged — FM picks up the corrected field description on its next WebFetch refresh; no FM-side consumer-code change forced. **Session 335 (documentation revision under v2.3.0 — no contract surface change, no version bump):** records the **demo-node reset coordination** decision (new § Demo-node reset coordination in the contract doc) — the demo node resets by restoring a curated baseline blob (`RestoreFromBlob`) rather than the local `demo:reset` reseed; **FM** owns the loop (provisions the node, writes the reset cron, **pushes** the blob via its provisioning channel so the node needs no outbound egress), **CRM** provides a `isDemoMode()`-hard-gated `demo:restore` command that restores the pushed blob + fixes env-specific values from `.env`. The demo node's `IMAGE_TAG` pin is lifted (upgrades ride the daily reset window). CRM-side `demo:restore` implementation lands in the demo session following 335; no HTTP surface change, contract stays v2.3.0. **Session 336 (CRM-side implementation — non-boundary, contract stays v2.3.0):** built the `demo:restore` command the 335 revision referenced (hard-gated to `isDemoMode()`; restores a locally-pushed blob's DB + media tree; fixes `base_url` from the node's `.env`); a real local round-trip proved the full-backup path round-trips faithfully. No HTTP surface change. **FM-side pickup:** demo-node reset coordination — FM owns provisioning/cron/blob-push + baseline alignment. **Session 341 (deploy-infra — non-boundary, contract stays v2.3.0; no `/api/*` or `CONTRACT_VERSION` change):** added an idempotent `widgets:sync` artisan command and wired it into every CRM-side deploy path after `migrate --force` (`deploy.yml`, `deploy-demo.yml`, the droplet upgrade runbook) so pure-schema widget changes (a new/changed inspector control with no migration) actually reach an upgraded server — the page builder + public render read widget fields from `widget_types.config_schema`, only (re)written by `WidgetRegistry::sync()`, which the deploy path never ran. **FM-side action required (no contract change):** wherever FM's node-upgrade orchestration runs `migrate --force` on a node outside `deploy.yml`, it must also run `php artisan widgets:sync` immediately after, or the bug reproduces on FM-driven upgrades. Manual unstick for an already-stale node: `docker exec nonprofitcrm_app php artisan widgets:sync`.
- **Fleet Manager repo state (corrected at CRM session 365):** FM is at **session 048 complete** (session 050 drafted) — the FM-side absorption backlog this block used to track session-by-session is long shipped. **Absorbed FM-side:** v2.0.0 mTLS (FM 012); v2.1.0 log-fetch (FM 013); v2.2.0 backup-trigger — contract-cache refresh + `BackupTriggerClient` + "Trigger backup now" Filament action (FM 020); v2.3.0 blob endpoint — `BackupBlobClient` + operator restore-to-fresh-node affordance (FM 021 + 022, plus operator same-node restore at FM 038 beyond original scope); the 253 Security-Posture and 291/335 documentation revisions rode along on the same refreshes. **Pending FM-side:** v2.4.0 (`data_hygiene` subcheck + maintenance toggle) is FM session **050**, drafted; v2.5.0 (`/api/admin/recover` control-panel UI) is pending. Ground truth for FM state: FM `sessions/completed-sessions.md`. Per-version status is in the "pending boundary changes" entries below.
- **Pending boundary changes:**
  - **FM v2.7.0 absorption pending — app-layer second gate on the five FM `/api/*` endpoints (`X-Fleet-Gate-Key` header + per-install `FLEET_GATE_SECRET`).** CRM-side **shipped at session 372** (additive v2.6.0 → v2.7.0, Security Hardening track S2): a `VerifyFleetAgent` middleware behind the nginx mTLS termination requires a constant-time-checked per-install shared-secret header on all five endpoints; **absent node secret = gate inert = exactly v2.6.0 behaviour** (additive, no flag day), enforced failure = JSON `401 {error: fleet_gate_unauthorized}` distinct from nginx's plain-HTML `403`/`400`. Secret is TLS-independent (not cert-derived) — survives an nginx regression the single per-location `if` would not. **FM-side absorption** via the Two-Repo Coordination Protocol, **order load-bearing to avoid a flag day**: (1) refresh the local contract cache to v2.7.0; (2) ship the HTTP-client change that **sends `X-Fleet-Gate-Key` on every request to all five endpoints first** (harmless while the node secret is unset and the gate inert); (3) generate a unique-per-install `FLEET_GATE_SECRET` and **push it to the node** over the existing single-key `.env` config-push channel (the `SUSPENSION_STATE` machinery) to switch enforcement on. The reverse order (push secret before sending the header) locks FM out for one poll cycle. One secret per install, never shared; treat it as a rotatable credential (rotate like the cert, via the same channel — no contract change). No new CRM-side action required.
  - **FM v2.6.0 absorption pending — client billing node half (`SUSPENSION_STATE` flag + billing-state document + `suspension` subcheck).** CRM-side **shipped at session 366** (additive v2.5.0 → v2.6.0, draft entry CB1 of the Client Billing & Account track): the node enforces the pushed `SUSPENSION_STATE` flag (`none`/`admin_locked`/`site_off`, absent = none, unrecognized fails safe to none) via one middleware, reads a display-only billing-state JSON document from `storage/app/private/fleet/billing-state.json` (backup-excluded, atomic-written, null-object on missing/malformed), and reports the enforced state + document `as_of` via a new never-red, excluded-from-worst-of `suspension` `/api/health` subcheck. **No vendor-Stripe anything CRM-side, by design** — money logic stays FM-repo-only. **FM-side absorption is session FM-B2** via the Two-Repo Coordination Protocol: refresh the local contract cache to v2.6.0, then **write the billing-state document + push the `SUSPENSION_STATE` flag** over the existing SSH provisioning + single-key config-push channels, and verify via the read-back subcheck (auto-lock at grace/trial expiry, auto-unlock on payment recovery). FM-B1 (Stripe sync + billing model + clocks) is a prerequisite and can precede this CRM session. Track design: `sessions/tracks/client-billing-and-account.md` (Appendix B carries the FM-side session outlines).
  - **FM v2.5.0 absorption pending — operator-mediated admin account recovery (`POST /api/admin/recover`).** CRM-side **shipped at session 360** (additive v2.4.0 → v2.5.0): the FM-triggered reset for a locked-out admin (`reset_2fa` / `reset_password`, always-200 `status` envelope, `reset_password` returns a one-time `temporary_password`, mTLS-gated, `throttle:6,1`, no app-layer identity check — identity verified out-of-band against an operator-held external-vault PIN, no recovery secret stored anywhere). **FM-side absorption pending** via the Two-Repo Coordination Protocol: refresh the local contract cache to v2.5.0, build a recovery HTTP client (mirror the `BackupTriggerClient` pattern), and add the operator-facing **admin-recovery control-panel UI** on the FM-side ClientResource view (the operator checks the external-vault PIN, then triggers the reset; the UI must treat the returned `temporary_password` as a show-once secret, never logged). No further CRM-side action required.
  - **FM v2.4.0 absorption pending — count-only `data_hygiene` subcheck + per-node maintenance toggle (Fleet Data Hygiene track).** CRM-side **shipped at session 353** (additive v2.3.0 → v2.4.0): the `data_hygiene` `/api/health` subcheck carries **aggregate counts only** (`{orphan_event_pages, scrub_records, orphan_media_dirs, dead_owner_media}`) — never raw node data, by the deliberate privacy boundary (FM must not be *built* to read customer data even though mTLS makes it possible; deep audit stays node-local + consent-gated). Informational, never red, excluded from the worst-of overall status. **FM-side absorption is FM session 050 (drafted as of CRM 365)** via the Two-Repo Coordination Protocol: refresh the local contract cache to v2.4.0, consume the new subcheck (object `value`, informational/excluded-from-worst-of semantics), and build the manual, user-editable per-node **"maintenance/auditable" toggle** gating any data-touching op (deep audit, cleanup, remediation). Handoff: `sessions/data-hygiene-handoff-from-crm.md` (FM repo). Design + boundary: `sessions/tracks/fleet-data-hygiene.md` (and agent memory `project-fleet-data-audit-privacy-boundary`). Phase 3 (bounded remediation) is future.
  - **FM v2.3.0 absorption ✅ complete both sides (FM 021 + 022).** CRM-side authoring shipped at session 268; FM-side shipped — FM 021 refreshed the local contract cache and built `BackupBlobClient`, FM 022 shipped the operator-facing restore-to-fresh-node affordance (plus operator same-node restore at FM 038 beyond original scope). **A2(c)'s live end-to-end was blocked since FM 042 (2026-06-05) on a CRM-side cross-node role-grant bug** — a CRM backup's `pg_dump` carried `GRANT … TO <source node's read-only role>`, so restoring on a different node aborted on the missing role. **Fixed CRM-side at session 365** (dump-side `--no-privileges --no-owner`); future blobs are role-portable, old blobs get re-recorded.
  - **FM v2.2.0 absorption ✅ complete both sides (FM 020).** CRM-side authoring shipped at session 263; FM-side shipped at FM 020 (contract-cache refresh + `BackupTriggerClient` + "Trigger backup now" Filament action). Manual-test sign-off (open since FM 020) rides the post-deploy verification sequence recorded in the FM-repo handoff `a2-runbook-handoff-from-crm.md` (deferred at 365 by owner ruling — no mid-session deploy).
...
### Accidental Public Exposure *(✅ DELIVERED at Security Hardening S3, session 371 — #32c Path A. The audit found sensitive CRM data has no path to a public page (structural exclusion); delivery was: the write-time `allowedSlots` hole closed + reactive publish/collection warnings + a "Public-exposure controls" section in the permission-matrix runbook — **no field-level sensitivity tagging** (owner decision, liability/maintenance). C3a accountability half stays deferred until first customer. Stub retained for history.)*
...
### Stripe Test-Mode Detection & Random Data Generator Production Guard *(✅ RESOLVED at session 370 — Security S1. Shipped option (a) hard refuse-to-render: `App\Support\StripeMode::isLive()` detects `sk_live_`/`rk_live_`; the generator hides its generate form + the `store()` endpoint refuses on a live-Stripe install, while wipe stays available. See the 370 log. Original stub preserved below.)*
