...
- **Code Shape & Fit** — *between cycles* — Cycle 1 closed at session 313 (audit at 312 ✅, apply at 313 ✅). Track doc carries the Cycle 1 retrospective, status snapshot, and standing improvements: `sessions/tracks/code-shape-and-fit.md`. Next trigger ≈ session 379 per the +50 cadence rule. A004 review session queued out-of-band to evaluate Cycle 1 against stated goals.
- **Test Audit** — *active* — newly-lifted at A003 close. Recurring 50-session cadence; single audit session by default, carve-out shape if dimensions overflow. Four audit dimensions: D1 Playwright spec discipline, D2 Spec-claim integrity, D3 Mutation testing slice (existing `docs/testing/mutation-audits.md`), D4 Pest file relevance + fast/slow refresh. Cycle 1 = A005 (D1 + D2 only — biggest forcing function from A003's flake-chasing plus the s296 `[test-integrity]` backlog drain); D3 and D4 queue for later cycles. Prior partial audits noted in the track doc as precursors: s094 (Pest audit), s241 (first mutation slice). Track doc: `sessions/tracks/test-audit.md`.
