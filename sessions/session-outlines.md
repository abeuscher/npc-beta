...
Status as of 2026-04-26 (end of session 222):
...
- **Phase 4 — Retrofit existing widgets onto contracts** (5–8 sessions per migration doc; refined estimate after 218: ~5–7 sessions remaining for the ~11 genuine retrofit candidates, with the ~7 pure-config widgets requiring no contract). Campaign underway; ~16 page-builder widgets remaining after 218 (of which ~11 are genuine retrofit candidates):
  ...
  - **Phase 4g — Event single-row pair retrofit + PageContext::event retirement** (session 223, drafted): scoped at `sessions/223. Widget Primitive — Phase 4g (Event Single-Row Pair: EventDescription + EventRegistration + PageContext::event Retirement).md`. **First non-list-shaped contract** in the system — locks in the single-row DTO shape (`['item' => row | null]`) and the `cardinality` field on `DataContract` for every future single-row contract (ProductDisplay in 224, every Phase 5 record-detail-sidebar widget afterward). Aggregate-derived field pattern (`is_at_capacity` via `withCount` baked into `resolveEventOne()`) sets the chosen pattern for any future "derived field that needs a query" case. `PageContext::event()` and `eventCache` retire alongside (mirror of 217's `upcomingEvents()` and 222's `posts()` retirement patterns).
  - **Phase 4h — Product pair retrofit + WidgetDataResolver shell retirement** (session 224, planned): retrofits ProductCarousel + ProductDisplay onto a brand-new `'product'` arm in `ContractResolver` + `SystemModelProjector`. **First nested DTO projection** in the system (`prices` array of price-rows inside a product DTO row). Replaces the last `WidgetDataResolver::resolveProducts()` consumer; cleanup vertical retires the entire `WidgetDataResolver` shell (`resolve()`/`resolveBlogPosts`/`resolveEvents`/`resolveProducts`) plus `PageContext::collection()` and `PageContext::product()`. Closes Phase 4. Scope to be agreed with the user when 223 lands.
