...
Status as of 2026-04-24 (end of session 215):
...
- **Phase 4 — Retrofit existing widgets onto contracts** (5–8 sessions per migration doc; may come in closer to 3–5 in practice given 215 revealed much of the page-builder-widget plumbing is already in place). Sequence starts with BlogListing:
  - **Phase 4a — BlogListing retrofit** (session 216, drafted). Audit the already-substantially-retrofitted BlogListing widget, lock its contract-bound data path with two explicit load-bearing tests (fail-closed field whitelist; resolver-is-the-only-path query-count pin), and delete any now-dead direct-model code in its neighborhood. Scope is strict: no new contract filters, no projector field-map widening. The point is to prove the retrofit pattern is tractable — mostly audit + test + cleanup — before committing to the remaining ~19 widgets. Prompt at `sessions/216. Widget Primitive — Phase 4a (BlogListing Retrofit — Audit, Test-Lock, Cleanup).md`.
