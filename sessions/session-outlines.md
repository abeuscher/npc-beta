...
- **Security Hardening** — *active (opened at the 369 launch replan) — this track IS Gate 3 of the launch* — five sessions; **S1 ✅ (session 370)**, S2–S5 pending (schedule order S1 → S3 → S2 → S4 → S5): **S1 ✅** perimeter headers + enforced public CSP (admin Report-Only) + self-hosted Quill + prod-hardening defaults + the Stripe live-key generator guard + the demo-role seeding gate + an admin-editable CSP host allow-list, **S3** *(next — session 371, prompts drafted)* XSS write-path containment + the accidental-exposure audit (absorbs release-plan #32c), **S2** app-layer second lock on the five FM `/api/*` endpoints (the track's one boundary-touching session — additive contract bump planned), **S4** adversarial pass over the public write surface + the portal-scoping E2E suite (absorbs F2; the form-spam/CAPTCHA inbox item lands here), **S5** permission-matrix E2E + demo-server hardening (absorbs F3; assembles the Gate-3 findings register for the owner walkthrough). Bar: self-driven internal review — every enumerated item fixed or consciously accepted. **S1 residuals on the register:** admin CSP Report-Only (not enforced); analytics hosts managed via the admin allow-list. Evidence base: the session-368 security surface map, folded into the track doc as Appendix A. Track doc: `sessions/tracks/security-hardening.md`.
