...
Status as of 2026-04-24 (end of session 214):
...
- **Phase 3** (3–5 sessions, first two drafted ahead):
  ...
  - **Phase 3d — Dashboard config UI & storage** (session 215, drafted). Per-role admin dashboard arrangement. New `dashboard_configs` table as polymorphic owner of `page_widgets` (honors the migration doc's locked-in "polymorphic owner discipline" — no parallel `dashboard_widgets` table). Reuses the existing Vue page builder via a new Livewire shell + a `mode: 'dashboard'` field threaded through `BootstrapData`; picker enforces `allowedSlots()` for the first time; inspector honors `DashboardGridSlot::layoutConstraints()['allowed_appearance_fields']`. This is the first concrete step of Phase 6 (page-builder convergence) arriving naturally rather than as a later rewrite. Prompt at `sessions/215. Widget Primitive — Phase 3d (Dashboard Config UI & Storage).md`.
- **Phases 4–6** (retrofit remaining widgets, record-detail slot + `SlotContext` generalization, page-builder convergence): each detailed in `sessions/widget-primitive-migration.md`. Total ≈ 20–30 sessions, interleavable with feature work after 1.0 ships.
