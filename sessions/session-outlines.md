...
- **Code Shape & Fit** — *Cycle 1 audit closed at session 312 ✅; apply queued at session 313* — companion refactor track to Code Review & Cleanup; asks "is this the simplest expression of the intent it's trying to express?" Walks five workstreams (W-R1 Cardinality / W-R2 Locality / W-R3 Directness / W-R4 Necessary vs accidental / W-R5 Seam quality). Originally scheduled for ≈ session 324 (trailing pair after cleanup Cycle 3); pulled forward at user direction. **312 audit output:** 2 inline findings (W-R1-1 typography double-resolve, W-R4-1 source-context re-derive in `ImportSessionActions`), 3 Open Refactor Flags (W-R1/A `buildTree()` triple-key superset, W-R2/A `ImportContactsPage` Flag B successor, W-R4/A pattern-fix scale of W-R4-1), 2 intent-unrecoverable findings (both in `PageBuilderApiController` — `requiredForPage` re-derive, `collectLibs` parallel loops), 2 register-revision proposals for 313 walkthrough. W-R3 and W-R5 clean — no landing-grade findings. Permanent inter-cycle artifact `sessions/tracks/code-shape-and-fit-finding-template.md` shipped. Deliberately Kept register at `sessions/refactor-track-deliberately-kept.md` (three carve-outs: FieldMapper services, Page vs View, inline-eligibility roster) — citations all spot-checked clean; revisions deferred to 313 walkthrough. Track doc + workstream definitions: `sessions/tracks/code-shape-and-fit.md`.
