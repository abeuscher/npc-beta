...
2. **300 — Widget Color-Token Consumption Audit & Migration** *(prompts drafted; the arc's RISK session; renumbered 298→300 at session-297/2 — see the Numbering note below).* Audit/migrate all ~38 widgets' SCSS to consume `var(--np-color-*)` instead of hardcoded hex / `$color-*` literals — the thing that makes layers 297 & 301 actually reach widgets. Two classes (297's demotion makes `$color-*` users transitively token-backed; raw-hex users are the true silent-failure). **Size is unknown until a ~30-minute widget-hardcoding survey** at scoping; **may split per Rule 11.** This is the make-or-break session; without it the whole arc is decorative.
3. **301 — Schemes, Template Page-Shell & Chrome, Shared Resolver** *(detailed stub; renumbered 299→301 at session-297/2).* Theme-defined Default + Inverse schemes; per-template **scheme selector**; the header/footer slot + "No header"/"No footer" model; the **single shared resolver** wiring public + preview identically (request-time inline overrides, never bundled). Restores deliberate per-page deviation removed in 297.
4. **(Optional tail, ≤0.5 session, mostly absorbable)** — naming/IA polish (relabel the mislabeled CMS-Settings sections, page-vs-content-template split, move Stripe branding to Finance) + consolidate the published widget-dev token contract doc. Most doc work rides along in 297/300/301; the pure admin-reorg is deferrable and could fold into a later housekeeping batch.
