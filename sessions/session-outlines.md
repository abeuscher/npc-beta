...
> **Sample-showcase intent supplanted (demo-node only) — session A006.** The "one sample event + one sample blog post so all major features are demonstrated" goal is better realized by the demo node's curated public site (Mill Creek Conservancy, 6 authored pages over the `demo:reset` CRM baseline). A006 resolved the scope distinction **demo-node-only**: this stub's *fresh-install* seeding behavior is **unchanged** — the showcase moves to the demo node, not to every customer's fresh install. Design spec: `sessions/public website/demo-site-pages-spec.md`; build prompt: `sessions/demo-site-pages-build.PROMPT-DRAFT.md`.
