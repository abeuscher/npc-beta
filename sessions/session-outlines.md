...
**Next concern, queued as the content-import/export arc:** session 308 surfaced a write-conflict between the Theme/Typography editor and `ContentImporter::importDesign()` — both write to `SiteSetting::typography` with implicit-import-wins behaviour. Remediation scoped into three sessions: **309** (opt-in theme + interactive import prompts), **310** (unified Import Site / Export Site UI rollup), and **A001** (agentic backend extension to events, products, navigation, collections — first use of the `session-A###/N` async-agent branch convention). See `sessions/309. base-prompt.md`, `sessions/310. base-prompt.md`, `sessions/A001. base-prompt.md`.
