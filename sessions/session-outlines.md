...
4. **(Optional tail, ≤0.5 session, mostly absorbable)** — naming/IA polish (relabel the mislabeled CMS-Settings sections, page-vs-content-template split, move Stripe branding to Finance) + consolidate the published widget-dev token contract doc. Most doc work rides along in 297/300/301; the pure admin-reorg is deferrable and could fold into a later housekeeping batch. **Note (300/301 planning):** a *separate* theme/design export-import slice surfaced here is **not** this naming/IA tail and **not** part of 301 — it pairs with the Media Portability draft into its own combined post-301 session; see the **Theme + Media Portability** stub below.
