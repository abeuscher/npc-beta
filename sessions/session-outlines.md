...
- **Public Marketing Website** — *active — PMW1 closed at 284; CMS fixes closed at 285; home rebuild closed at 286; About layout rebuild queued at 287* — content + visual-build track building the five-page nonprofitcrm.com marketing site inside the product's own page builder. Lifted at session 283 mid-session from an outside-agent collaboration. The layout-spec-driven execution pattern was validated at 286 (zero importer warnings on first pass, eight bands in spec order, cross-section rhythm matched the spec's prediction) and now repeats per page. Sequence: PMW1 Audit + Home cleanup (closed at 284) → session 285 CMS Fixes Before Home Rebuild (lifted-gap session, outside the track's phase count) → session 286 Home Layout Rebuild (closed; home shipped against `sessions/public website/homepage-layout-spec.md` plus a small user-revision pass) → session 287 About Layout Rebuild (queued; same shape, drives off `sessions/public website/about-layout-spec.md`) → Pricing / Contact / Demo each as their own session, one page per session, following the spec-then-execute pattern → page-capture harness + close-out as the track's final session. 285 and 286 sit between PMW1 and E5 so their CMS-surface and home-rebuild work both inform the E-series tuning sessions (Mobile Type Scaling, Theme Colors, Column-Layout Collapse, UI/UX Sprint); remaining page sessions still sequenced after E8 so page polish lands against finalized type / color / collapse decisions. Working folder at `sessions/public website/` (brief, copy, references, JSON exports, screenshots, gap report, homepage-layout-spec, about-layout-spec). Track doc + premise + sequence + system conventions live in `sessions/tracks/public-marketing-website.md`. Gap-resolution lifts (new widget, new button style, new config option) become independent follow-on sessions, not absorbed into the track's phase count.
