...
- **Phase 4 follower — Query settings in the contract layer** *(stub — surfaced during session 219 manual testing; runs after Phase 4 retrofit campaign or interleaved with it once enough widgets are locked to make the UX gap visible)*. The QuerySettings inspector panel ([resources/js/page-builder-vue/components/QuerySettings.vue](resources/js/page-builder-vue/components/QuerySettings.vue)) supports five filter knobs (`limit`, `order_by`, `direction`, `include_tags`, `exclude_tags`) keyed per-slot. Today: it renders only when `widget_type_collections.length > 0` (legacy `WidgetDefinition::collections()` slots), and it works only because [WidgetRenderer.php:46-51](app/Services/WidgetRenderer.php#L46-L51) feeds `query_config` into [WidgetDataResolver::resolveCustom](app/Services/WidgetDataResolver.php) on the legacy path. Retrofit-locked widgets have empty `collections()` → panel hides → user can't apply `limit:4` to a Carousel that points at a 5-image collection. There is no architectural reason for this; query knobs are about *how many / which order / which subset* of rows, orthogonal to the contract pattern's *which fields* security boundary. Scope of the follower session:
    - Merge user-supplied `query_config` into `$contract->filters` at [WidgetRenderer.php:84](app/Services/WidgetRenderer.php#L84) before the resolver call. Decide precedence per knob: widget-default vs user-override (some widget defaults are load-bearing semantics — e.g. EventsListing's `date_range: from now` shouldn't be admin-override-able to show past events).
    - Extend [ContractResolver::resolveWidgetContentType](app/WidgetPrimitive/ContractResolver.php) to honor `direction`, `include_tags`, `exclude_tags` (port the `cmsTags` relation walk from `WidgetDataResolver::resolveCustom`). `limit` already supported on every source.
    - Flip the Vue panel render gate from `widget_type_collections.length > 0` to "this widget has a list-shaped contract" — true for `SOURCE_SYSTEM_MODEL` and `SOURCE_WIDGET_CONTENT_TYPE`, false for `SOURCE_PAGE_CONTEXT` (token map, not a list).
    - Decide the `query_config` keying convention for retrofit-locked widgets. Today it's slot-keyed (`{ data: { limit: ... } }`); contract-locked widgets have no slots. Options: (a) flatten to top-level (`{ limit: ... }`), (b) key by source/handle, (c) use a canonical default key per widget. Touches the API filter, the Vue panel, and migration of any persisted `query_config` rows.
    - Decide what becomes of [PageBuilderApiController::filterQueryConfig](app/Http/Controllers/Admin/PageBuilderApiController.php). The slot-allowlist gate breaks once `widget_type->collections` is empty for retrofit-locked widgets — either rederive the allowlist from the contract's accepted knob set or retire the strip in favor of validation against a closed knob constant set (`limit`, `order_by`, `direction`, `include_tags`, `exclude_tags`).
    - **UX rule for the rebuilt panel: fields display directly, no collapse-to-summary toggle.** The current panel hides its body behind a chevron toggle (`<button class="query-settings__toggle">` at [QuerySettings.vue:39-58](resources/js/page-builder-vue/components/QuerySettings.vue#L39-L58)); when this work lands the body should render directly without the disclosure pattern. Captured as a feedback memory.
    - **Latent seeder discipline gap surfaced.** Session 219 found that removing `WidgetDefinition::collections()` requires re-running `WidgetTypeSeeder` against the dev DB to sync `widget_types.collections`. Session 218 silently left `carousel`'s `collections` column at `["slides"]`; 219 left `bar_chart` at `["data"]` until the user surfaced the panel-still-renders bug. Neither test infra (Pest re-seeds in `beforeEach`) nor manual smoke (rendering tests don't exercise the slot list) caught this. The follower session should land a process rule (or hook) that re-seeds `WidgetTypeSeeder` after any retrofit-lock that removes definition methods affecting the row shape.
