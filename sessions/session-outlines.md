...
- **Phase 4 — Retrofit existing widgets onto contracts** (5–8 sessions per migration doc; refined estimate after 218: ~5–7 sessions remaining for the ~11 genuine retrofit candidates, with the ~7 pure-config widgets requiring no contract). Campaign underway; ~16 page-builder widgets remaining after 218 (of which ~11 are genuine retrofit candidates):
  ...
  - **Phase 4d — BarChart retrofit** (session 219, drafted). First true `SOURCE_WIDGET_CONTENT_TYPE` retrofit (Carousel was an audit) and the first widget where `dataContract()`'s `fields` list is dynamic per-instance — built from `$config['x_field']` and `$config['y_field']` filtered for non-empty. Confirms the contract language scales to per-instance shapes. Template rewires `$collectionData['data']` to `$widgetData['items']`; legacy `collections()` removed; sibling slot-strip tests will retarget from `'bar_chart'`/`'data'` to BoardMembers or LogoGarden. Prompt at `sessions/219. Widget Primitive — Phase 4d (BarChart Retrofit).md`.
