...
- ~~**Dev-environment orphan media cleanup.** Custom `php artisan app:reset` (refuses to run when `APP_ENV=production`) that wraps `migrate:fresh --seed` with `rm -rf storage/app/public/* storage/media-library/temp/*` so the on-disk media tree stays synchronized with the DB.~~ ✅ Shipped at session 247 in reduced shape — prod-refusal gate is the only safety mechanism (the originally-prompted confirmation prompt, `--force` flag, and Console-Kernel auto-discovery filter were dropped as gold-plating). Paired with daily `media-library:clean` on the scheduler (slow-drip orphan-conversion path); fires once the scheduler-runner gap closes. See `sessions/247. Housekeeping Batch 2 — app:reset Artisan + Orphan Media Cleanup — Log.md`.
