...
Status as of 2026-04-25 (end of session 218):
...
- **Phase 4 follower — Query settings in the contract layer** *(scoped as session 221; runs immediately after 220 since 220 retired the last slot-declaring widgets)*. The QuerySettings inspector panel ([resources/js/page-builder-vue/components/QuerySettings.vue](resources/js/page-builder-vue/components/QuerySettings.vue)) supports five filter knobs (`limit`, `order_by`, `direction`, `include_tags`, `exclude_tags`) keyed per-slot. Today: it renders only when `widget_type_collections.length > 0` (legacy `WidgetDefinition::collections()` slots), and it works only because [WidgetRenderer.php:46-51](app/Services/WidgetRenderer.php#L46-L51) feeds `query_config` into [WidgetDataResolver::resolveCustom](app/Services/WidgetDataResolver.php) on the legacy path. Retrofit-locked widgets have empty `collections()` → panel hides → user can't apply `limit:4` to a Carousel that points at a 5-image collection. There is no architectural reason for this; query knobs are about *how many / which order / which subset* of rows, orthogonal to the contract pattern's *which fields* security boundary. Scope of the follower session (drafted at `sessions/221. Widget Primitive — Phase 4 Follower (Query Settings in the Contract Layer).md`):
    - Merge user-supplied `query_config` into `$contract->filters` at [WidgetRenderer.php:84](app/Services/WidgetRenderer.php#L84) before the resolver call. Precedence rule resolved during 220 close: user values win for **honored** knobs (the universal five); non-honored knobs (e.g. EventsListing's `date_range: from now`) are immutable contract defaults — admins cannot override via `query_config`.
    - Extend [ContractResolver](app/WidgetPrimitive/ContractResolver.php) per source-arm to honor `order_by`, `direction`, `include_tags`, `exclude_tags`. **All five knobs uniformly across every list-shaped widget** — no per-source variance in the user surface. Per-source plumbing: `resolvePost` and `resolveEvent` add `order_by` validation against per-source allowlists (post: `published_at`/`created_at`/`updated_at`/`title`; event: `starts_at`/`ends_at`/`created_at`/`title`) plus tag filtering via the unified `tags()` relation. `resolveWidgetContentType` adds the same set; `order_by` allowlist is built dynamically from the contract's content-type text fields plus three CollectionItem system columns. `limit` already honored everywhere.
    - Flip the Vue panel render gate from `widget_type_collections.length > 0` to `widget.query_settings?.has_panel` — populated by a new `App\WidgetPrimitive\QuerySettings` value object exposed on the per-widget API JSON with `has_panel: bool`, `order_by_options: Record<string, string>`, `supports_tags: bool`.
    - **`query_config` keying flattens** from slot-keyed (`{ slides: { limit: 4 } }`) to top-level (`{ limit: 4 }`). One-time data migration flattens existing rows; pre-Beta acceptable, no rollback path needed.
    - **Deletions:** `WidgetDataResolver::resolveCustom` (and orphaned helpers `getCollectionFieldKeys` / `getCollectionImageFieldKeys`); the legacy `foreach ($widgetType->collections ?? [])` loop in `WidgetRenderer::render()` lines 44–51 plus the `$collectionData` template variable; `PageBuilderApiController::filterQueryConfig` (replaced with a closed-set knob allowlist via `array_intersect_key` against the five honored keys); the `widget_type_collections` field on the per-widget Vue API and TypeScript `Widget` type; the `<button class="query-settings__toggle">` chevron in `QuerySettings.vue` (lines 39–58); the misnamed `"Carousel Settings"` advanced-fields chevron heading in [InspectorFieldGroup.vue:99](resources/js/page-builder-vue/components/InspectorFieldGroup.vue#L99) (replaced with inline rendering under an "Advanced" heading — generic accordion fed by `advanced: true` field flag, mislabeled as carousel-specific for months).
    - **Side discovery folded into the deletion list:** the `cmsTags` relation referenced at [WidgetDataResolver.php:70](app/Services/WidgetDataResolver.php#L70) and [:74](app/Services/WidgetDataResolver.php#L74) was deleted in session 040; `include_tags`/`exclude_tags` on the legacy path has been silently broken for ~14 months. Tag uniformity across all five taggable models (Page, Event, CollectionItem, Organization, Contact) confirmed during 220 close — single `whereHas('tags', ...)` implementation works for every source-arm.
    - **No-collapse UX rule extends to QuerySettings.** Both inspector chevron toggles (QuerySettings + InspectorFieldGroup advanced-fields) removed in the same session per `feedback_no_collapse_ui` memory.
    - **Uniform UX over per-type plumbing** captured as a new feedback memory during 220 close (`feedback_uniform_ux_over_per_type_plumbing.md`): pay per-source-arm implementation cost to keep the user-facing surface uniform; only carve exceptions for genuine functional differences (date_range as identity-defining default is the one real exception).
    - **Latent seeder discipline gap (carry-forward from 219).** The follower session inherits the 219/220 process rule: re-run `WidgetTypeSeeder` after any retrofit-lock that removes definition methods affecting `widget_types` row shape.
