...
- **Phase 5** (record-detail slot + ambient context refactor): unchanged, 4–6 sessions per migration doc. Scheduled to follow the Date Display stub (session 226).
...
### Date Display — Format Hints + Projector-Owned Assembly *(stub — pre-Beta 1, follow-on to session 225)*

Add per-widget date format selection to the contract pattern, retire the `_label` projector-field-naming convention in favor of concept-named display fields (`event_date`, `event_time`, `post_date`), and pull every contract-locked widget's date formatting back into the projector — leaving the templates as dumb token renderers. The session 225 close-gate conversation surfaced the architectural rule this session enforces: *the projector is the view-model assembler; the template is a dumb renderer.* Templates that today contain `@php` blocks with imperative `Carbon::parse + DateFormat::format` calls (ThisWeeksEvents, EventDescription) are template-side anti-patterns that quietly opt out of the contract pattern's fail-closed promise. This session pulls them back in.

Mechanism: a new `formatHints` field on `DataContract` (associative array, keyed by projector field name, value is a `DateFormat` constant). Widget schemas declare a `date_format` select with `options` drawn from a curated per-widget option-set helper on `DateFormat` (`eventDateOptions()`, `postDateOptions()`). The projector validates each hint against the option-set and falls back to a known-safe default on unknown values — mirror of session 221's `orderByOptions` allowlist enforcement. The hint is a sibling axis to `querySettings`: both express per-instance, widget-curated knobs that travel via the contract.

Field rename: `_label` suffix retires. New rule — machine fields keep column names (`starts_at`, `published_at`); display fields use `<model>_<concept>` (`event_date`, `event_time`, `post_date`, `event_location`). The convention is uniform across models. EventDescription's same-day vs cross-day structural branching is removed entirely (multi-day events explicitly not supported); the template loses its 14-line `@php` block and becomes a straight token render. Tile cleanup (drop `Ends:` prefix, drop `{{item.slug}}` debug, drop `{{item.starts_at}}` ISO debug, reconcile two disagreeing default templates) lands in the same beat — the field rename forces every default template to update, so the cleanup is bundled.

Five widgets in scope: EventsListing, ThisWeeksEvents, EventDescription, BlogListing, BlogPager. Memos (JSONB key, different code path), PortalEventRegistrations (direct-query, not contract-locked), and EventCalendar (JS-driven) explicitly out of scope.

**Out of scope for this session:** `time_format` hint (only `date_format` lands; `event_time` always uses `TIME_SMART` for now), site-setting "default date format," multi-day event support, token modifier syntax (`{{item.event_date|long}}`), data migration to rewrite existing widget `content_template` strings (pre-existing custom templates referencing retired `_label` tokens render those tokens as empty strings until the user resets them; documented in the close-gate production notes).

**Scope:** one session. Phases: `DataContract::formatHints` field, `DateFormat` extensions (`EVENT_TILE_DATE` constant + option-set helpers), projector refactor (retire `_label` fields, emit concept-named fields, validate hints), per-widget contract + schema updates with bundled tile cleanup, test-lock, verification.
