...
- **Public Marketing Website** — *active — PMW1 closed at 284; session 285 (CMS Fixes) and session 286 (Home Layout Rebuild) queued next* — content + visual-build track building the five-page nonprofitcrm.com marketing site inside the product's own page builder. Lifted at session 283 mid-session from an outside-agent collaboration. Sequence: PMW1 Audit + Home cleanup (closed at 284) → session 285 CMS Fixes Before Home Rebuild (lifted-gap session, outside the track's phase count) → session 286 Home Layout Rebuild (rebuilds the home against `sessions/public website/homepage-layout-spec.md` using the CMS surface lifted at 285) → PMW2 Pricing + About → PMW3 Contact + Demo → PMW4 page-capture harness + close-out. 285 and 286 sit between PMW1 and E5 so their CMS-surface and home-rebuild work both inform the E-series tuning sessions (Mobile Type Scaling, Theme Colors, Column-Layout Collapse, UI/UX Sprint); PMW2–PMW4 still sequenced after E8 so page polish lands against finalized type / color / collapse decisions. Working folder at `sessions/public website/` (brief, copy, references, JSON exports, screenshots, gap report, homepage-layout-spec). Track doc + premise + sequence + system conventions live in `sessions/tracks/public-marketing-website.md`. Gap-resolution lifts (new widget, new button style, new config option) become independent follow-on sessions, not absorbed into the track's phase count.
