...
Status as of 2026-04-26 (end of session 223):
...
- **Phase 4 — Retrofit existing widgets onto contracts** (5–8 sessions per migration doc; refined estimate after 218: ~5–7 sessions remaining for the ~11 genuine retrofit candidates, with the ~7 pure-config widgets requiring no contract). Campaign underway; ~16 page-builder widgets remaining after 218 (of which ~11 are genuine retrofit candidates):
  ...
  - **Phase 4h — Product pair retrofit + WidgetDataResolver shell retirement** (session 224, drafted): scoped at `sessions/224. Widget Primitive — Phase 4h (Product Pair Retrofit + WidgetDataResolver Shell Retirement) — Phase 4 Close.md`. Retrofits ProductCarousel (list-shaped) + ProductDisplay (single-row by slug) onto a brand-new `'product'` arm in both `ContractResolver` and `SystemModelProjector`. **First nested DTO projection** in the system (`prices` as an array of `['id', 'label', 'amount']` sub-rows inside each product DTO row; nested-fields contract grammar deferred until a second case demands it — the projector locks the sub-row shape). **First session that introduces both list-shaped and single-row dispatch for the same model in one beat.** ProductCarousel declares `querySettings()` for consistency with every other list-shaped widget (`hasPanel: true`, four `orderByOptions`, `supportsTags: false` since Product is not in the taggable morphMap today). Cleanup vertical retires the entire `WidgetDataResolver` shell (`resolve()`/`resolveBlogPosts`/`resolveEvents`/`resolveProducts`) plus `PageContext::collection()` and `PageContext::product()`. **Closes Phase 4 of the Widget Primitive track** — every retrofit candidate in `app/Widgets/*` will carry a contract; `PageContext` will retain only `currentPage`, `currentUser`, and `form()` (Forms widget hasn't retrofitted, no scheduled session).
- **Phase 5** (record-detail slot + ambient context refactor): unchanged, 4–6 sessions per migration doc.
