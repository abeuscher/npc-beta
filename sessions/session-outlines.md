...
Status as of 2026-04-24 (end of session 217):
...
- **Phase 4 — Retrofit existing widgets onto contracts** (5–8 sessions per migration doc; refined estimate after 217: ~6–8 sessions remaining for the ~12 genuine retrofit candidates, with the ~7 pure-config widgets requiring no contract). Campaign underway; ~17 page-builder widgets remaining after 217 (of which ~12 are genuine retrofit candidates):
  ...
  - **Phase 4c — Carousel retrofit** (session 218, drafted). First `SOURCE_WIDGET_CONTENT_TYPE` retrofit-lock, audit-style (Carousel's contract is already declared and template already reads `$widgetData['items']` — the work is verification, test-lock, and cleanup of the legacy `collections()` declaration). Cleanup decision is audit-driven: the legacy renderer's `$fallbackCollectionData` half may still be load-bearing for demo data, in which case `collections()` stays; if not, it's removed. Test A's key-level invariant is asserted via direct `ContractResolver::resolve()` call rather than embedded JSON (Carousel renders Swiper slides directly without a `<script x-ref="listingData">` payload). Prompt at `sessions/218. Widget Primitive — Phase 4c (Carousel Retrofit — Audit, Test-Lock, Cleanup).md`.
