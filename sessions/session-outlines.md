...
Status as of 2026-04-23 (end of session 212):
...
- **Phase 3** (3–5 sessions, first two drafted ahead):
  ...
  - **Phase 3b — Data sink & source policy** (session 213, drafted). Write-side paradigm: `Source` identity primitive, model-local `HasSourcePolicy` trait, fail-closed `DataSink` singleton, `collections.accepted_sources` column with explicit checkboxes at Create Collection. `Donation::ACCEPTED_SOURCES` explicitly excludes `Source::DEMO` as the session's sharpest security assertion. Demo generators migrate to route through the sink. Folds data generation into the paradigm as a first-class declared source rather than an exception. Prompt at `sessions/213. Widget Primitive — Data Sink & Source Policy.md`.
  - **Phase 3c+** — dashboard-native widgets (recent donations, donor LTV, upcoming events), storage model for admin-arranged dashboards, admin configuration UI. Shape TBD.
