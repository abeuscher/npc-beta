...
- **Phase 4 — Retrofit existing widgets onto contracts** (5–8 sessions per migration doc; refined estimate after 218: ~5–7 sessions remaining for the ~11 genuine retrofit candidates, with the ~7 pure-config widgets requiring no contract). Campaign underway; ~16 page-builder widgets remaining after 218 (of which ~11 are genuine retrofit candidates):
  ...
  - **Phase 4f — BlogPager retrofit + PageContext::posts retirement** (session 222, drafted): scoped at `sessions/222. Widget Primitive — Phase 4f (BlogPager Retrofit + PageContext::posts Retirement).md`. Cheapest of the 5 remaining Phase 4 candidates (post projector exists; only addition is `author_name` to `SystemModelProjector::projectPost()`). Mirrors 217's retirement pattern: BlogPager is the sole production caller of `PageContext::posts()` — the helper and its `postsCache` property are deleted in the same session. After 222: 4 widgets remain in Phase 4 — the Event single-row pair (EventDescription + EventRegistration; both walk `$pageContext->event($slug)` and need a "find one by slug" pattern in `ContractResolver`) and the Product pair (ProductCarousel + ProductDisplay; require a brand-new `'product'` arm in both `ContractResolver` and `SystemModelProjector`).
