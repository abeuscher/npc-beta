...
- **Fleet Data Hygiene** — *active next — Phase 1 prompts drafted at the 351 close (session 352); SEO cluster cleared the queue ahead of it* — prevent + detect silent cruft accumulation on live instances (orphan event/post landing pages, leftover scrub data, media creep) under a **hard privacy boundary: FM is never *built* to read raw node data — counts only over the wire; a per-node FM "maintenance/auditable" toggle gates any data-touching op; deep audit is node-local + consent-gated.** Two halves: a CI keystone "scrub-wipe leaves no residue" test (the launch-gate prevention) + a node-local `app:data-hygiene` audit feeding a count-only `/api/health` subcheck (detection). Cross-repo (CRM + FM). Down-payment at session 349 (orphan-event-page cause fixed + `pages:prune-orphan-events` / `media:prune-orphans`). Full design + forward plan: `sessions/tracks/fleet-data-hygiene.md`.
...
- **Pending boundary changes:**
  - **Planned (not started): count-only `data_hygiene` health subcheck + per-node maintenance toggle — Fleet Data Hygiene track.** A future additive contract bump adds a `data_hygiene` subcheck to `/api/health` carrying **aggregate counts only** (orphan pages, residual scrub records, orphan media) — never raw node data, by deliberate privacy boundary: FM must not be *built* to read customer data even though mTLS makes it possible; deep audit stays node-local + consent-gated. Pairs with a manual, user-editable per-node "maintenance/auditable" toggle in FM gating any data-touching op. Pre-release priority. Design + boundary: `sessions/tracks/fleet-data-hygiene.md` (and agent memory `project-fleet-data-audit-privacy-boundary`).
