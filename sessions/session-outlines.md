...
- **Client Billing & Account** — *active — CB1 ✅ shipped (366, boundary-touching, v2.6.0); CB2/CB3 CRM-side + FM-B1…B5 FM-side pending* — how client organizations pay, see their account, and get suspended / offboarded. Stripe holds the money + risk; **Fleet Manager owns billing centrally** (polls Stripe daily, derives the suspension state machine + clocks, pushes state to nodes); each node gets a **read-only "My Account" window** + a hard **suspension gate**. **No vendor-Stripe anything CRM-side, by design** — money logic is FM-repo-only; the node enforces a flag and renders a pushed document. **CB1 (366, boundary-touching) shipped the node half:** contract **v2.5.0 → v2.6.0** (additive) — the `SUSPENSION_STATE` flag + one enforcement middleware (`admin_locked` → 403 admin lock with the public site / donations / member portal staying up; `site_off` → 503 public maintenance with FM `/api/*` up; absent = none; unrecognized fails safe to none), the display-only billing-state document reader (backup-excluded pushed JSON, null-object on missing/malformed), and a never-red / excluded-from-worst-of `suspension` `/api/health` subcheck. **Remaining CRM-side:** CB2 (the "My Account" Filament page + `manage_account` permission + convention-drift guards; prompts drafted as session 367) and CB3 (demo-conversion cleanup command, node half). **FM-side (separate repo):** FM-B1 Stripe sync → FM-B2 push + verify → FM-B3 operator UI → FM-B4 trial + conversion → FM-B5 first-conversion drill. Gate `first-customer` (commercial readiness — **not** Beta-1-blocking); CB2 depends on CB1 only, so the two repos run in parallel from here. Full design, session decomposition, open questions, and FM handoff: `sessions/tracks/client-billing-and-account.md`.
