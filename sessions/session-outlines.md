...
- **Phase 5** (record-detail slot + ambient context refactor): unchanged, 4–6 sessions per migration doc. Scheduled to follow the Date Handling stub (session 225).
...
### Date Handling — Centralized Vocabulary + Schema Audit *(stub — pre-Beta 1, follow-on to Widget Primitive Phase 4)*

Two related concerns to land in one session, scheduled **after Phase 4 of Widget Primitive completes** (i.e. after the Product pair retrofit closes the page-builder retrofit campaign and the legacy `WidgetDataResolver` shell retires). Doing this once the widget surface is stable means one sweep across all retrofitted projectors and templates rather than two.

**Concern 1 — Centralized format vocabulary.** As of session 222 the widget surface carries six distinct date-format strings with no shared vocabulary:

| Format | Output | Used by |
|---|---|---|
| `'F j, Y'` | "April 25, 2026" | `projectPost::published_at_label`, `PageContextTokens::date`, Memos template, PortalEventRegistrations |
| `'F j, Y g:i a'` | "April 25, 2026 5:00 pm" | `PageContextTokens::starts_at` |
| `'D, F j, Y \a\t g:i A'` | "Sat, April 25, 2026 at 5:00 PM" | `projectEvent::starts_at_label` |
| `'g:i A'` | "5:00 PM" | `projectEvent::ends_at_label` |
| `'D M j · g:i a'` | "Sat Apr 25 · 5:00 pm" | `ThisWeeksEvents/template.blade.php` (overrides projector) |
| `'F jS'` + conditional `'ga'`/`'g:ia'` | "April 25th" + "5pm" or "5:00pm" | EventDescription (lands in 223) |

The de facto convention (projector emits `<col>` ISO + `<col>_label` canonical presentation) holds, but the canonical format varies by projector author's taste, and `PageContextTokens` formats post dates differently from how `projectPost` formats them. No locale awareness, no 12/24-hour preference, no central way to change "all event dates" globally. Templates that need a different format than the projector emitted drop to `Carbon::parse(ISO)` ad-hoc (ThisWeeksEvents already does; EventDescription will after 223).

The fix: a `DateFormat` const class / enum with named formats (`LONG_DATE`, `EVENT_LIST_DATE`, `EVENT_TIME_OF_DAY` with the conditional `g:ia`/`ga` baked in, etc.). Projectors and templates call `DateFormat::format($carbon, DateFormat::LONG_DATE)`. ~10–15 migration call sites in the widget surface, plus a tail in Filament/controllers (~15 more). Leaves the door open for site-settings-driven locale or 12/24-hour overrides without a re-walk later.

**Concern 2 — Schema audit for missing date fields.** Surveyed 2026-04-26:

- **`created_at` / `updated_at`**: present everywhere. No work.
- **`deleted_at` (soft deletes)**: present on `pages`, `collections`, `collection_items`, `contacts`, `organizations`, `memberships`. **Missing on `events` and `products`** — those use `status='cancelled'` (events) or `is_archived` (products) for tombstoning. Decision: leave as-is, or backfill `deleted_at` for symmetry. Probably leave — the existing tombstone columns work and migrating off them is a bigger refactor than this session covers.
- **`published_at` (timestamp)**: present only on `pages`. **Missing on `events` and `products`** — they rely on `status` enum without a publish-time. Real consequence: 221's QuerySettings allowlist for events is `['starts_at', 'ends_at', 'created_at', 'title']` — no `published_at` because the column doesn't exist; the natural "newest first" sort on events falls back to `created_at` (which is when the admin row was created, not when it went live). Proposal: add `published_at` to events and products with backfill from `created_at` for already-published rows; update `EventObserver` and `ProductResource` save paths to set `published_at = now()` on first publish (mirror of `PageObserver`'s pattern).
- **Memos `posted_at`**: lives as a JSONB key inside `collection_items.data` (the SWCT JSONB-key sort path 221 added). Not a candidate for promotion to a column — it's content-type-specific.
- **`memberships.starts_on` / `expires_on`**: `date` (not timestamp). Intentional — membership periods are calendar days, not points in time. Leave as-is.

**Scope:** one session. Phase 1 lands `DateFormat` and migrates the widget surface. Phase 2 (optional same-session if cheap) lands the `published_at` migration on events and products plus observer/resource wiring. If Phase 2 turns out thornier than expected, split it off — the format-vocabulary concern is the more time-sensitive one because every Phase 5 sidebar widget will format dates.

**Sequencing note:** can be slotted before Phase 5 (record-detail slot) or right after Phase 4 closes. Doing it after Phase 4 means one sweep across a stable retrofit surface; doing it before Phase 5 means sidebar widgets ship onto the centralized vocabulary from day one. Either order works.
