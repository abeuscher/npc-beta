...
### Page Builder Inline Editing *(arc — pre-Beta 1; critical-path, Beta-1-blocking; supersedes the retired "UI / UX Sprint" stub; A=304 / B=305 (partial) / C=306 / D=307; non-boundary v2.3.0; no schema)*
...
- **Session D — 307: Inline-Editing Arc Close (Builder↔Public Parity + Gate Widening).** The two deliverables originally on 305 that didn't run: regular automated builder-preview vs public-render appearance parity tests (now crisper to write since the 305 conflation split removed leaked markers from public; complementary to the session-296 stale-stylesheet drift guard); widen the code-declared eligibility gate to the remaining safe display-prose nodes (DonationForm heading, EventsListing static heading, others) while keeping the Tier-B guard green. Arc-closing.

Canonical: `sessions/304. *` (archived), `sessions/305. *`, `sessions/306. *`, `sessions/307. *` + `docs/inline-formatting-toolbar-spec.md` + `release-plan.md`'s retired-§ E8 entry. Non-boundary; CRM stays v2.3.0; no DB schema (the gate is declared in widget code).
