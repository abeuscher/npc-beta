...
Status as of 2026-04-24 (end of session 216):
...
- **Phase 4 — Retrofit existing widgets onto contracts** (5–8 sessions per migration doc; may come in closer to 3–5 in practice given 215 revealed much of the page-builder-widget plumbing is already in place). Campaign underway; ~19 page-builder widgets remaining after 216:
  - **Phase 4a — BlogListing retrofit** (session 216): complete. Audit confirmed the retrofit was already substantially landed (contract declared, projector in place, template reading `$widgetData['items']`, no `collections()` routing through `WidgetDataResolver`). Two load-bearing tests added at `tests/Feature/BlogListingContractRetrofitTest.php`: (a) fail-closed field whitelist — template tokens referencing undeclared fields render as empty strings, `listingData` JSON's `items[0]` keys canonically equal the seven-field contract; (b) resolver-is-the-only-path — one `pages` select matching `ContractResolver::resolvePost()`'s shape (type + published + `COALESCE(published_at, created_at)` order), one eager `media` load, no N+1. Phase 3 cleanup produced an empty diff — audit confirmed no dead code. Notable finding: the fail-closed invariant is about **contract keys**, not source-column content — the projector legitimately derives whitelisted fields from non-whitelisted source columns (e.g. `excerpt` from `meta_description`). Leak-test sentinels must live in columns without a derived whitelisted counterpart. Fast Pest: 1399/0 single-pass with `-d memory_limit=2G` (sidesteps the pre-existing mid-suite OOM; latent, not fixed). Log at `sessions/216. Widget Primitive — Phase 4a (BlogListing Retrofit — Audit, Test-Lock, Cleanup) — Log.md`.
  - **Phase 4b — EventsListing retrofit** (session 217, drafted). True retrofit rather than audit — EventsListing has no `dataContract()` declaration today, its template walks `$pageContext->upcomingEvents()` and composes `url`/`location`/`price_badge`/`image`/`date`/`date_iso` imperatively. Session lands the contract (`SOURCE_SYSTEM_MODEL` on Event, nine-field whitelist), extends `SystemModelProjector::projectEvent()` with the derived fields (including `date_iso`/`date` key migration that also affects `ThisWeeksEvents`), adds `landingPage` eager-load to `ContractResolver::resolveEvent()`, rewrites the template to read from `$widgetData['items']`, deletes `PageContext::upcomingEvents()` (grep-confirmed sole caller), updates `resources/docs/widget-types.md` accordingly, and adds the test-lock pair mirroring BlogListing's. Prompt at `sessions/217. Widget Primitive — Phase 4b (EventsListing Retrofit).md`.
