...
### Demo Guided Tour — driver.js *(stub — scoped at the unnumbered demo-polish session, 2026-05-24)*

A guided walkthrough that fires when a demo user lands in the admin (`/demo/enter` → shared `demo@demo.local`), introducing several key views. **Library decision settled: `driver.js`** (MIT — permissive, tiny, no deps; chosen over Shepherd.js, whose current licensing flags AGPL-or-commercial, and over Intro.js, which is AGPL/commercial). Greenfield — no tour/onboarding infra exists today.

Lift is dominated by *content*, not plumbing. Wiring (~½ session): add `driver.js`, an init module imported into `resources/js/admin.js`, started on `alpine:init`, injected via the existing `AdminPanelProvider` render hook (`PanelsRenderHook::HEAD_END`, already in use). Per-view tour content (~¼–½ session/view) is the variable cost: each step anchors to a CSS selector in Filament's generated DOM, so some steps will need `data-tour="…"` hooks added where selectors aren't stable. Two decisions that swing the estimate: **self-contained per-view tours vs. one tour that walks across views** (cross-page continuation needs progress persistence + resume); and **progress tracking** — the demo user is a *shared* row, so per-user DB "seen" state is useless → use `localStorage` for the demo (a per-user DB pref only if real users get tours later). Rough total ~1.5–2.5 sessions for ~4–6 self-contained per-view tours; +0.5–1 for a single cross-view walk. No schema. SaaS alternatives (Product Fruits free tier, Usetiful) were considered and set aside in favour of self-owned OSS — no third-party script on admin.

### Page Builder — Preview & Publish (Versioned Pages) *(stub — scoped at the unnumbered demo-polish session, 2026-05-24; full spec at `sessions/page-builder-preview-publish-spec.md`)*

Real editorial preview-then-publish for pages/posts, which also closes the demo security hole (demo users currently mutate the live public demo instantly, because builder writes are live; a publish *permission alone does not fix this* — writes are already live). **Shape A chosen:** leave the reactive builder untouched (it keeps autosaving to the working rows); the public site stops reading the working rows and instead reads a **published artifact** — a JSON snapshot stored in a new `page_versions` table, written on publish via the existing, production-proven `ContentExporter`/`ContentImporter` JSON engine. Round-trip fidelity (content + media, content-addressed since 320) is empirically proven from cross-site moves, so **no de-risking spike is needed**. Gives version history + `?version=N` / `?preview=working` querystring preview + portability for free. Demo gets a new `publish_page` permission denied → edit + preview only, public demo frozen at last published version (a real boundary, not demo-only scaffolding). One in-code fork: render published from JSON-hydrated-transient (A1, recommended) vs materialize into twin rows (A2). **Lift ~1.5–2.5 sessions** — migration + `page_versions` + publish/preview/revert + render branch + permission/policy + dirty indicator + Pest/Playwright; posts covered by the same model. CRM-internal, non-boundary (contract stays v2.3.0). Full data model, flows, render branch points, and open questions in the spec doc.
