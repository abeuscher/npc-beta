...
### Theme & Template Re-Taxonomy *(stub — pre-Beta 1; supersedes & absorbs `release-plan.md` § E6; arc, 3–4 sessions)*
...
1. **297 — Theme Color Foundation** *(prompts written; full base + session prompt exist).* The `--np-color-*` contract + concrete defaults; Colors editor on the Theme page; `.np-site` bundle delivery (mirrors the 295 typography/button correction); relocate `primary_color` + header/footer/nav colours off `templates` into the Theme (byte-faithful data migration — the G1/285-shape data-preservation guard); kill "default-template = site brand"; publish the widget-dev token doc. Interim consequence (accepted): no per-template colour 297→299 until schemes land. **Adds a DB column-drop migration** (unlike 295/296). *Not* widgets, *not* schemes/chrome.
2. **298 — Widget Color-Token Consumption Audit & Migration** *(detailed stub; the arc's RISK session).* Audit/migrate all ~38 widgets' SCSS to consume `var(--np-color-*)` instead of hardcoded hex / `$color-*` literals — the thing that makes layers 297 & 299 actually reach widgets. **Size is unknown until a ~30-minute widget-hardcoding survey** at scoping; **may split per Rule 11.** This is the make-or-break session; without it the whole arc is decorative.
3. **299 — Schemes, Template Page-Shell & Chrome, Shared Resolver** *(detailed stub).* Theme-defined Default + Inverse schemes; per-template **scheme selector**; the header/footer slot + "No header"/"No footer" model; the **single shared resolver** wiring public + preview identically (request-time inline overrides, never bundled). Restores deliberate per-page deviation removed in 297.
4. **(Optional tail, ≤0.5 session, mostly absorbable)** — naming/IA polish (relabel the mislabeled CMS-Settings sections, page-vs-content-template split, move Stripe branding to Finance) + consolidate the published widget-dev token contract doc. Most doc work rides along in 297–299; the pure admin-reorg is deferrable and could fold into a later housekeeping batch.

**Discipline carried from session 295 (rules, not guidelines):** verify in a *real browser by computed values* across public + admin + preview — never from emitted CSS/`curl`; Theme defaults in the bundle (296-guarded), per-page overrides request-time inline (drift-proof); the consumption audit (298) is the real work and the real risk; the token list is the agreed contract — do not re-derive it.
