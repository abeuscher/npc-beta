...
*Preceded by **333 — Page Builder ↔ Public Widget Parity** (editor↔public fidelity; promoted ahead of this push after the 332 container-query rollback surfaced an editor Swiper-load race, an uneven-grid defect, and a possible layout-column drop — see the 332 log). The items below are **334+**.*
...
- **Mobile Nav — hamburger + footer columns *(own session)*.** The `nav` hamburger menu is broken and the footer column-layout collapse renders poorly. Carved out of the ruleset above as its own session because it's nav-chrome, not widget-interior styling. Distinct from (and a prerequisite feel for) the post-Beta "Nav widget variants — separate mobile/desktop presets" backlog item.
