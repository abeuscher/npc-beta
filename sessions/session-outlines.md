...
4. **(Optional tail, ≤0.5 session)** — **drafted at 301 close as session 302** (`sessions/302. base-prompt.md` + `sessions/302. Naming & IA Polish — Re-Taxonomy Tail.md`), arc-closing: relabel/regroup the CMS-Settings surface, rename the mislabeled "Header & Footer Code Snippets" section (favicon + OG image), move Stripe-checkout branding into the existing `FinanceSettingsPage`, give page-vs-content templates IA clarity, consolidate `docs/theme-color-tokens.md` (stale 298/299 → 300/301; the now-resolved interim note). Pure relabel/regroup/move/doc — no behaviour, schema, or contract change. Items the catalogue listed that 297/300/301 already resolved are explicitly not re-done. **On 302 close the arc is complete and `release-plan.md` § E6 is fully retired.** **Note:** the *separate* theme/design export-import slice is **not** this tail — it pairs with the Media Portability draft into its own post-arc session; see the **Theme + Media Portability** stub below.
