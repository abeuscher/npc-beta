...
- **Phase 3** (3–5 sessions, first two drafted ahead):
  - **Phase 3a — Dashboard grid: slot plumbing & hosted render** (session 212, drafted). Replace `DashboardGridSlot::ambientContext()` throw with a minimum-viable body; add optional `$slotHandle` parameter to `WidgetRenderer::render()`; mount a new `DashboardSlotGridWidget` Filament widget on the existing Dashboard page, rendering two existing widget-primitive widgets (Blog Listing + Carousel). No new widgets, no storage model, no config UI. Prompt at `sessions/212. Widget Primitive — Phase 3a (Dashboard Grid — Slot Plumbing & Hosted Render).md`.
