...
- **Public Marketing Website** — *active — PMW1 closed at 284; CMS fixes closed at 285; home rebuild closed at 286; About layout rebuild closed at 287; Pricing chart widget build closed at 288; Pricing layout rebuild closed at 289; Contact layout rebuild closed at 290 (pivoted mid-session to an embedded contact form in the hero; shipped the `text.link_color` widget appearance feature as an in-scope code expansion resolving G22; G21 open with proven path; G23 form-notifications lifted to 291); form-submission notifications closed at 291 (lifted gap — G23 ✅ resolved, shipped as an operator-editable `form_submission` System Email + `{{contact_email}}` reuse + graceful mail-failure handling + FM pre-Beta pseudo-versioning; CRM contract stays v2.3.0); Demo built at 292 (form-less single-button `/demo/enter` auto-login + tightly-scoped `demo` role + per-IP throttle + demo-server guard + the split page per `demo-page-spec.md`; CRM contract stays v2.3.0; Phase 3 Contact+Demo complete); page-capture harness/track-close → 293)* — content + visual-build track building the five-page nonprofitcrm.com marketing site inside the product's own page builder. Lifted at session 283 mid-session from an outside-agent collaboration. The layout-spec-driven execution pattern validated at 286 was re-validated at 287 and 289 (zero importer warnings on first pass, bands in spec order, cross-section rhythm matching each spec's prediction; 289 added the first real-page placement of the new `pricing_chart` widget as Band 2's carrier and it rendered acceptably with subgrid row-alignment, mobile collapse, and the v1 background-tint emphasis on Monthly). Sequence: PMW1 Audit + Home cleanup (closed at 284) → session 285 CMS Fixes Before Home Rebuild (lifted-gap session) → session 286 Home Layout Rebuild (closed) → session 287 About Layout Rebuild (closed; one structural workaround applied for stacked text widgets in single-column bands per new gap G20, one new pull-quote-border gap surfaced as G19) → session 288 Pricing Chart Widget Build (closed; lifted-gap session shipping the comparison-chart widget end-to-end + a generic `repeater` field type as the system extension; v1 background-tint emphasis with a built-but-unfilled border slot for the v1.1 appearance-config border-knob upgrade) → session 289 Pricing Layout Rebuild (closed; five bands shipped with Band 2 owned end-to-end by the new widget configured via `marketingSiteTiers()`; Band 3's 2×2 grid avoided G20 via a 4-slot/2-column-CSS-wrapped shape; no revision rounds requested; fast suite at 2416 / 0; Demo-page architecture conversation during close locked in single-button auto-login + 24h wipe model with Fleet Manager shared-contract updates lifted to a cross-session arc — captured in project memory) → Contact (290; closed — greenfield, pivoted mid-session from the 3-band `copy.md` shape to a single hero with an embedded `web_form` contact form + 3:4 portrait; `text.link_color` feature shipped resolving G22; G23 lifted) → Form Submission Notifications (291; ✅ closed — lifted-gap, resolved G23 as an operator-editable `form_submission` System Email; recipient is the existing `{{contact_email}}` CMS setting; graceful mail-failure handling added; FM pre-Beta pseudo-versioning folded in — CRM contract stays v2.3.0) → Demo (292; ✅ closed — form-less single-button `/demo/enter` auto-login on the demo server, tightly-scoped `demo` role (never `super_admin`), per-IP `throttle:10,1`, demo-server-only guard, shared "Demo User" seeded demo-only, + the split page per `demo-page-spec.md` built into the CMS with zero importer warnings and user-approved; does NOT consume 291's notification surface — the earlier "Demo builds on the notification primitive" linkage was superseded by the s289-close form-less architecture; Phase 3 Contact+Demo complete) → page-capture harness + track close-out (293, track's final session). 285 and 286 sit between PMW1 and E5 so their CMS-surface and home-rebuild work both inform the E-series tuning sessions (Mobile Type Scaling, Theme Colors, Column-Layout Collapse, UI/UX Sprint); remaining page sessions still sequenced after E8 so page polish lands against finalized type / color / collapse decisions. Working folder at `sessions/public website/` (brief, copy, references, JSON exports, screenshots, gap report, layout specs, pricing-chart-widget spec, demo-page spec). Track doc + premise + sequence + system conventions live in `sessions/tracks/public-marketing-website.md`. Gap-resolution lifts (new widget, new button style, new config option) become independent follow-on sessions, not absorbed into the track's phase count.
