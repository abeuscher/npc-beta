...
Status as of 2026-04-23 (end of session 213):
...
- **Phase 3** (3–5 sessions, first two drafted ahead):
  ...
  - **Phase 3c — Dashboard-native widgets** (session 214, drafted). Three widgets — Memos (`SOURCE_WIDGET_CONTENT_TYPE`), Quick Actions (no contract), This Week's Events (`SOURCE_SYSTEM_MODEL` on Event) — covering all three contract sources to prove the dashboard slot end-to-end. Re-enables `DashboardSlotGridWidget::widgets()` registrations, retires the legacy Filament-native quick-actions widget, extends `ContractResolver` to handle `model: 'event'`. `memos` collection regular-seeded (not demo). Prompt at `sessions/214. Widget Primitive — Phase 3c (Dashboard-Native Widgets).md`.
  - **Phase 3d+** — admin configuration UI for arranging dashboard widgets, storage model for admin-arranged dashboards (`page_widgets` with new owner type, or new table). Shape TBD; plausibly session 215.
