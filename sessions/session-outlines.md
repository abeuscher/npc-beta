...
- **Public Marketing Website** — *active, not yet started* — four-phase content + visual-build track building the five-page nonprofitcrm.com marketing site inside the product's own page builder. Lifted at session 283 mid-session from an outside-agent collaboration. Phases: PMW1 Audit + Home cleanup → PMW2 Pricing + About → PMW3 Contact + Demo → PMW4 page-capture harness + close-out. PMW1 sequenced before E5 / E6 / E7 / E8 so the audit output informs their design choices; PMW2–PMW4 sequenced after E8 so page polish lands against finalized type / color / collapse decisions. Working folder at `sessions/public website/` (brief, copy, references, JSON exports, screenshots, gap report). Track doc + premise + sequence + system conventions live in `sessions/tracks/public-marketing-website.md`. Gap-resolution lifts (new widget, new button style, new config option) become independent follow-on sessions, not absorbed into the track.
