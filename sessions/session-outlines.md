...
- **Last boundary-touching session in this repo:** session 253 (documentation revision under v2.1.0 — no contract surface change, no version bump). Adds Recovery posture + FM-side trust assumptions sub-section to Security Posture: break-glass recovery path (shipped — CRM-side rotation script + recovery runbook ride alongside), FM's off-filesystem-key posture (FM-side intended posture, Beta-1 scope), audit-sink discipline (FM-side intended posture, Beta-1 scope). Section-header preface labels item-by-item shipped vs intended-posture status; future revisions promote intended items to shipped without a contract bump. Prior boundary-touching session: 251 (additive `/api/logs` endpoint; bumped 2.0.0 → 2.1.0; mTLS gate extended to the new location; `?lines=N` tail-only against the `single`-channel `laravel.log`; JSON envelope carrying `lines` / `lines_returned` / `lines_truncated` / `source`).
- **Last boundary-touching session in Fleet Manager repo:** FM session 011 (refreshed local cache from v1.1.0 to v1.2.0; verified no FM-side consumer-code change required — `ContractValidator` already accepts `unknown`, `StatusInterpreter` re-derives `last_backup_at` against FM-side thresholds matching the CRM's `[24, 36]`). FM 012 (drafted: mTLS Migration — Absorb v2.0.0) and FM 013+ (A2 affordance work, including the log-reading slice) absorb v2.0.0 + v2.1.0 together at next FM-side cycle. The 253 documentation revision rides along — FM-side WebFetch picks up the new sub-section + revision CHANGELOG entry on the next refresh; no FM-side consumer-code change required.
...
### Organizations Model Overhaul *(B1a closed at 255; B1c queued for 256; B1b post-B2)*
...
- **B1a — Organizations Model Overhaul (Min)** ✅ *(closed at session 255).* Five nullable transactional FKs (`donations.organization_id`, `memberships.organization_id`, `event_registrations.organization_id`, `events.sponsor_organization_id`, `transactions.organization_id`), all `ON DELETE SET NULL`. Four importer Org-as-source sentinels reusing the `__org_contact__` strategy-radio UX. Org admin form rebuilt: single section, type values `nonprofit/for_profit/government/other`, email field, address inline. Sponsor field on Event admin form. Notes lifted to a shared polymorphic Timeline pattern via abstract `app/Filament/Concerns/RecordTimelinePage.php`; ContactNotes refactored, OrganizationNotes added; `organizations.notes` text column dropped. Final bottom-of-record shape on Org: **Events Sponsored panel only**, plus an ellipsis-menu "View affiliated contacts →" link to a filtered Contacts list (financial transactions are referenced via filtered links to Finance, not displayed inside records). Block-with-counts deletion guard with force-delete branch. See `sessions/255. Organizations Model Overhaul (Min) — Log.md`.
- **B1c — Organizations Importer** — *next session per release-plan execution order; queued for session 256.* Top-level CSV importer for Organizations under the Tools group, mirroring the namespaced importer pattern. Three new sentinels (`__custom_organization__`, `__tag_organization__`, `__note_organization__`). Schema additions on `organizations` for source-policy + import-trace (`source`, `custom_fields`, `import_source_id`, `import_session_id`, `external_id`). Dedup match-key dropdown with three-option duplicate strategy. Coexistence with the Org-as-source sentinels is structural — both orderings work via the same `LOWER(TRIM(...))` match logic.
