...
*Supplanted (showcase intent only, demo-node-only) — session A006.* The "sample event / sample blog post so all major features are demonstrated" showcase intent is better realized by the curated **demo-site pages** (a whole pretend-nonprofit site over the `demo:reset` data), specced in `sessions/public website/demo-site-pages-spec.md` and built by `sessions/demo-site-pages-build.PROMPT-DRAFT.md`. **Scope:** A006 decided this is **demo-node only** — the fresh-install default-content behavior above (every customer's install) is **unchanged**; only the demo server gets the showcase site. The remaining items here (all-five-published, the skip-default-content install option, and the Privacy/Terms starter pages below) stand as-is.
