...
Copy this file to `178. base-prompt.md` at the start of each session. Replace `178` with the session number and update the session title.

---

We are about to begin a new session: **178. Code Review & Cleanup (Audit)**.
...
3. Open `sessions/178. Code Review & Cleanup (Audit).md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/178. Preset Thumbnails — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
