...
#### E8. UI/UX Sprint *(page builder full-screen + Quill height handle)* — **SUPERSEDED — retired in favour of the Page Builder Inline Editing arc**

- **status:** **RETIRED at session-303 close.** The two E8 items were band-aids on the *current* form-based / fixed-editor authoring model: the Quill drag-resize handle solves "the side editor is too small" — a problem that disappears under inline editing (you edit text at its rendered height in place); the page-builder full-screen toggle's premise is undercut by the inline/iframe model and was already substantially covered by session 156's admin-wide topbar fullscreen toggle. Polishing the editing model the builder is replacing is throwaway work.
- **superseded by:** the **Page Builder Inline Editing arc** — originally a two-session split (A=304 / B=305 terminal), **extended at the 305 close to four sessions (A=304 / B=305 partial / C=306 / D=307)** because the formatting-toolbar deliverable proved under-specified on first build. Occupies this execution-order slot (position 38). **Session A — 304** (Inline Editing Foundation: real drag handle + explicit Edit affordance, whole-panel select removed; in-page contenteditable text on a safe set behind a code-declared per-node capability gate; discoverability; Phase 2b column/repeater control deferred, later replaced in 305 by a count-dropdown design). **Session B — 305 (PARTIAL)** (first-cut toolbar built and discarded as under-specified + lifecycle-buggy; user produced the visual + behaviour design contract at `docs/inline-formatting-toolbar-spec.md`; foundation laid: simplified pricing column control replacing the deferred 304 Phase 2b, Inspector cleanup, public-vs-builder render conflation split, four spec §6 companion changes). **Session C — 306** (Inline Formatting Toolbar Rebuild — build to the spec; §A handle refactor + §A14 MutationObserver in useInlineEdit; eighteen controls + popovers + accessibility per spec §4/§5). **Session D — 307** (Inline-Editing Arc Close — standing builder↔public render-parity tests + widen the eligibility gate to remaining safe display-prose; arc-closing). Non-boundary, v2.3.0, **no schema** (capability gate declared in widget code, not the DB). Critical-path, Beta-1-blocking.
- **canonical scope:** the `session-outlines.md` **"Page Builder Inline Editing" arc stub** + the session prompts `sessions/305. *` / `sessions/306. *` / `sessions/307. *` + the toolbar design contract at `docs/inline-formatting-toolbar-spec.md` (304 is archived).
- **estimated time cost:** 4 sessions in total (A shipped; B partial-closed at 305; C + D remaining).
...
38. **Page Builder Inline Editing arc** *(supersedes the retired § E8 "UI/UX Sprint"; lifted at session-303 close — critical-path, Beta-1-blocking)* — **Session A = 304** (Inline Editing Foundation) → **Session B = 305** (Inline Formatting Toolbar & Builder↔Public Parity). A independently shippable + hard prereq for B; non-boundary v2.3.0, no schema. Canonical: `session-outlines.md` "Page Builder Inline Editing" stub + `sessions/304.*` / `sessions/305.*`.
