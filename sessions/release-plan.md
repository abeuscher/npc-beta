...
38a. **Content Import/Export — Theme Write-Conflict Remediation arc** *(lifted at session-308 close — integration-seam fix surfaced during 308 typography debugging)* — three-piece arc. **309** (Opt-in Theme + Interactive Import Prompts): exporter opt flags, `ContentImporter::analyze()` pre-flight, `merge_design` default → OFF, two-step import UI with three checkboxes. **310** (Unified Import Site / Export Site UI): the rollup, button-grade v1 (no diff preview), Tools-menu surface. **A001** (agentic): backend-only extension of bundle coverage to events, products, navigation, collections — first use of `session-A###/N` async-agent branch convention; runs autonomously after 309 + 310. Non-boundary; CRM stays v2.3.0; no schema. Drafted in `sessions/309. *`, `sessions/310. *`, `sessions/A001. *`.
