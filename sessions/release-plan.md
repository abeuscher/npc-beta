...
- **superseded by:** the **Page Builder Inline Editing arc** — a deliberate two-session split, lifted ahead of the rest of the order at session-303 close, occupying this execution-order slot (position 38). **Session A — 304** (Inline Editing Foundation: interaction-model rework — real drag handle + explicit Edit affordance, whole-panel select removed; in-page contenteditable text on a safe set behind a code-declared per-node capability gate; discoverability spec; simplified structural repeater control). **Session B — 305** (Inline Formatting Toolbar & Builder↔Public Parity: single shared custom Vue toolbar over the Quill API; standing builder↔public render-parity tests; widen the eligibility gate). A is independently shippable and a hard prerequisite for B. Non-boundary, v2.3.0, **no schema** (capability gate declared in widget code, not the DB). Critical-path, Beta-1-blocking (beta testers cannot author pages with the whole-panel-select + Inspector-only model).
- **canonical scope:** the `session-outlines.md` **"Page Builder Inline Editing" arc stub** + the session prompts `sessions/304. *` / `sessions/305. *`.
- **estimated time cost:** 2 sessions (A + B; A may split per Rule 11 if it inflates).
