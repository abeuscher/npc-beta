...
- **success criterion:** `sessions/public website/contact.json` built against the Contact page layout spec (or against `copy.md` directly if no spec lands). Two or three bands per `copy.md`: Hero / Email / What I respond to fastest *(optional — designer's call)*. Email band is a `mailto:` link, not a form. Re-imported with zero importer warnings; screenshot reviewed by user. Gap report extended (continued-manifestation notes on G15–G20 / G-pricing-1 as applicable).
- **artifact:** new `contact.json` + screenshot + extended gap report.
- **estimated time cost:** 1 session.
