...
45. **E19.** Accessibility pass — Aria + Usability + Lighthouse *(promoted from housekeeping inbox at s313 close; combined session covering: text-editor toolbar button `title` attributes pulled in from the inbox, a full Aria sweep, a usability pass on operator-facing controls, and a Lighthouse audit using the locally-installed app. Surfaces fixes in one consolidated pass rather than spreading across housekeeping batches.)*
