...
#### E12. Housekeeping Batch 2 *(split per Rule 11 into two batches: Batch 2A = session 314 ✅, Batch 2B = session 315)*

- **gate:** release
- **prerequisites:** none
- **success criterion:** Walk `sessions/housekeeping-inbox.md` and work through the public-website-blocking subset across two batch sessions. **Batch 2A (session 314) closed** — shipped the Hero widget's button-alignment + text-block-max-width controls, a fresh-install placeholder logo image, and the logo wordmark responding to a full-bleed hero's text colour; dropped three proposed items as no-longer-valid (paragraph/list padding default, button-style refresh, editor text-colour legibility). Also shipped the mid-session bug-capture flow (`npm run logbug` → `housekeeping-incoming.md`, digested at close) and the verify-the-premise-at-walk staleness guard. **Batch 2B (session 315)** works through the inbox remainder; the entry closes when Batch 2B closes. May split further per Rule 11 if the remainder needs a third batch.
- **estimated time cost:** 2 sessions (Batch 2A done; Batch 2B pending).
...
42. **E12.** Housekeeping Batch 2 *(absorbs the public-website-blocking subset of `sessions/housekeeping-inbox.md`; split into two batches — Batch 2A = session 314 ✅, Batch 2B = session 315. Downstream feature sessions bumped one slot: record-duplicate = session 316, accessibility = 317, media finder = 318. Positions below unchanged — these are execution-order, not session numbers.)*
