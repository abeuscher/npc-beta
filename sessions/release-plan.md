...
- **estimated time cost:** 1 session.
- **post-290 sequence note:** G23 lifted as **session 291 — Public Marketing Website — Form Submission Notifications** (lifted-gap session, outside the track's phase count, lands before Demo so the Demo submit flow builds on the notification primitive). Demo Page Build → **292**; page-capture harness + track close → **293**. The full `§ 291` plan entry + execution-order renumber land at 291 close per `sessions/291. base-prompt.md`; scope canonical meanwhile at `sessions/scoping-form-notifications.md`.
