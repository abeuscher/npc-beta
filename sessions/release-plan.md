...
##### 288. Public Marketing Website — Pricing Chart Widget Build *(lifted-gap session, outside the track's phase count)*
...
- **success criterion:** new `pricing_chart` widget built end-to-end against the widget spec. Schema: widget-level eyebrow_label / heading / subheading / footnote (all rich text or text), columns repeater (in-widget config repeater, not collection-backed), each column carrying eyebrow / title / price / lead_content / attribute_rows / CTAs / per-column `emphasize` toggle. Rendering: cards as a parent CSS Grid with each card a subgrid that inherits the parent's row tracks, so the Nth row of each card lines up automatically; mobile collapses to one column; CTA pinned at card-bottom via subgrid; emphasized columns get background-tint treatment (border slot built into the template but unfilled until G-pricing-1's `appearance_config` border-knob extension lands). Demo path: `demoConfig()` returning a hardcoded three-tier example. Ships one v1 preset ("Marketing site tiers") matching the Pricing page's Band 2 verbatim — doubles as session 289's one-click setup AND as the agent's correctness check. New tests: schema-shape Pest test, render Pest test, manifest test auto-coverage. Widget count assertion in `WidgetPickerSession119Test` updated. `php artisan build:public` regenerates the widget asset bundle with the new entry. Thumbnail at `app/Widgets/PricingChart/thumbnails/static.png`. Surfaces a small system extension (a generic `repeater` field type — the only existing repeater is the specialized `buttons` field).
- **artifact:** new `app/Widgets/PricingChart/` folder (definition / template / SCSS / `DemoSeeder` if needed / thumbnail) + `WidgetTypeSeeder` registration + new generic `repeater` field type (Vue inspector component + config-schema entry) + new Pest tests + `resources/docs/widget-pricing-chart.md` help doc + 288's session log + gap-report annotations (G-pricing-1 v1 partial, G19 noted as related surface).
- **estimated time cost:** 1 session.
