...
34a. **285.** Public Marketing Website — CMS Fixes Before Home Rebuild *(closed at session 285; shipped G1 layout `appearance_config` round-trip + G2 TextBlock CTAs + cta_alignment + G3-interim `secondary-dark` button variant + G13/G14 typography defaults pushed via SiteSetting + Image widget aspect_ratio + max_width as an in-scope addition surfaced by the Image audit; fast Pest 2408 / 0 sequential, +18; outside the track's phase count per gap-resolution discipline)* ✅
34b. **286.** Public Marketing Website — Home Layout Rebuild *(closed at session 286; home.json rebuilt against `sessions/public website/homepage-layout-spec.md` as nine sibling layout blocks delivering the eight spec bands; user-revision pass landed five small visual changes; four new gap rows G15–G18 surfaced; layout-spec-driven execution pattern validated and adopted for the remaining four pages, one page per session; fast Pest 2407 / 1 sequential, with the 1 failure pre-dating 286)* ✅
34c. **287.** Public Marketing Website — About Layout Rebuild *(rebuilds about.json against `sessions/public website/about-layout-spec.md`; spacer-cell pattern for Band 2 reading-width constraint; multi-widget-per-cell for the pull-quote treatment)*
