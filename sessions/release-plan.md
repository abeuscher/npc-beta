...
- **gate:** release
- **prerequisites:** 286; user-supplied `sessions/public website/about-layout-spec.md` (authored at 286 close, user-revised to absorb spec-side feedback) + `sessions/public website/existing-about-page-export.json` (user exports from admin before session start)
- **success criterion:** `sessions/public website/about.json` built against the About layout spec — three bands (hero / business case / how-it-built), spacer-cell pattern for inner-content-width constraint in Band 2, multi-widget-per-cell pattern for the pull-quote treatment, Band 3 dark-band three-CTA close with `secondary-dark` for both non-primary CTAs. Re-imported with zero importer warnings; informal screenshot reviewed by the user. Gap report extended (G15 / G16 / G17 / G18 noted as continued, not re-flagged).
- **artifact:** new `about.json` + screenshot + extended gap report.
- **estimated time cost:** 1 session.
