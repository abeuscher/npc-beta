...
We are about to begin a new session: **148. Editor Canvas in Vue**.
...
3. Read `docs/editor-migration-plan.md` **in full** — this is the architectural plan for the Vue migration. Every design decision, API endpoint, store shape, and component structure is documented there. This session implements the canvas phase.
4. Open `sessions/148. Editor Canvas in Vue.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/148. Editor Canvas in Vue — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit on the current `session-148/N` branch, and notify the user. Do not push — the user will push and merge when ready.
