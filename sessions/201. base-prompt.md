# Base Prompt — Session 201

We are about to begin a new session: **201. Widget Template Tokens — Per-Item Namespace Split**.
...
2. Read `docs/app-reference.md` for environment names, container names, view-to-file mappings, and key dependencies. Read `docs/schema/README.md` for the table index, then `docs/schema/page_widgets.md` — the only table touched this session (config-JSONB contents rewritten by a data migration; no column changes).
3. Read the three files at the heart of the bug, in order:
   - `app/Services/PageContextTokens.php` in full — the page-level token list (`TOKENS`) and the substitute-in-place pass.
   - `app/Services/WidgetRenderer.php`, focusing on lines ~58–77 — the call site that runs `PageContextTokens::substitute` over every `richtext`/`text` config field before the widget's template ever executes.
   - `app/Widgets/EventsListing/EventsListingDefinition.php` and `app/Widgets/EventsListing/template.blade.php` — the canonical listing widget whose per-item token grammar collides with page-level tokens.
4. Skim other listing-widget templates to assess blast radius — do NOT edit each one up front. Grep for `content_template` defaults across `app/Widgets/*/*Definition.php`, and for per-item token replacement loops (`str_replace('{{'` / `preg_replace('/\{\{[^}]+\}\}/'`) inside `app/Widgets/*/template.blade.php`. Record which widgets produce per-item cards; those are the Phase 1 punch-list.
5. Read `sessions/200. Admin CSS Consolidation — Tightening Pass — Log.md` briefly for the most recent state of the widget preview surface (admin CSS only — no direct bearing, but confirms the last files touched).
6. Open `sessions/201. Widget Template Tokens — Per-Item Namespace Split.md` and read the session prompt carefully.
