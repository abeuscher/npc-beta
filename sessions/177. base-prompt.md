...
Copy this file to `177. base-prompt.md` at the start of each session. Replace `177` with the session number and update the session title.

---

We are about to begin a new session: **177. Preset Thumbnails**.
...
3. Open `sessions/177. Preset Thumbnails.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/177. Preset Thumbnails — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
