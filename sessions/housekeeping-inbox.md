...
- Organization importer auto-mapping helper file (s313 close): the other six importers (Contacts, Events, Donations, Memberships, Invoices, Notes) each have a small per-type helper that drives auto-mapping of inbound CSV columns. Organizations importing landed at session 256 but no matching helper was ever built — most likely an oversight. Add an `OrganizationFieldMapper` modeled on the existing five sibling files, slot it into the importer registration the same way, and the existing test patterns should carry. Low-risk, scope-fenced. Cross-ref: `sessions/refactor-track-deliberately-kept.md` § "Per-namespace mapper helpers" notes the gap.
- General settings — logo cannot be removed (s313 close): the operator can change the uploaded logo but there is no path to reset it to blank/unset once one has been uploaded. Add a Remove / Reset affordance next to the logo control in the general-settings panel so a fresh-install-style "no logo" state is reachable from the UI.
- Backup download artifact cleanup (s313 close): the export-artifact TTL sweeper item already in this inbox covers queued bundle exports, but the user noted backups are also "getting stored somewhere when I download them" — verify whether downloaded backup files are being cleared on the same schedule, and if not, fold them into the same TTL sweeper. Small janitor task once the sweeper item is picked up.
- Text-editor toolbar button accessibility (s313 close): toolbar buttons in the rich-text editor need `title` attributes for hover tooltips and screen-reader labels. Pairs naturally with the broader accessibility pass (E19 in `release-plan.md`) but is small enough to land alone in a housekeeping batch if it gets there first.
- Ellipsis menu missing on main page editor view (s313 close): the page record's edit-detail view exposes an ellipsis-menu of secondary actions, but the main page-builder editor view does not. Surface the same menu (or its current equivalent) on the page-builder view so operators don't have to navigate to detail just to reach those actions. Pattern is already established on detail; small port.
- Columns-primitive alignment tools — labels or presets (s313 close): the vertical / horizontal alignment controls inside the columns primitive are powerful but obtuse to non-technical users. Either improve the labels and inline hints, or add a preset dropdown alongside the raw controls (e.g. "Center content / Top-align / Distribute evenly") that resolves to the same underlying values. UX polish; the underlying mechanism stays as-is.
