...
7. **Migrations vs. the squash discipline** — mechanics understood; the work is (a) redrawing the squash boundary so core's schema dump excludes plugin tables while each plugin owns its own migration list, (b) deterministic install order: core schema → enabled plugins' migrations → seeders (enabling a plugin later runs its migrations then), (c) a per-composition fresh-install identity check replacing today's single fixed-schema assumption. Cost class: about a session, landing with the squash redraw (P6/P7). Discipline problem, not a danger problem.
   ...
   **Events-owned schema annotation (recorded at session 381 — P5's deliverable in lieu of moving schema; the input for the P6/P7 squash redraw).** Tables that move out of core's dump when the boundary is redrawn: `events`, `event_registrations`, `ticket_tiers` (docs at `docs/schema/{events,event_registrations,ticket_tiers}.md`). All three live in the squashed dump `database/schema/pgsql-schema.sql`; the only live events migration is `2026_07_23_000001_add_is_complimentary_to_ticket_tiers_table.php`. Factories staying core until P7: `EventFactory`, `EventRegistrationFactory`, `TicketTierFactory`. Core columns/relations that reference these tables and stay core: `pages.type = 'event'` landing pages (`events.landing_page_id`), `contacts`/`organizations` event relations, `transactions` polymorphic subject rows (`EventRegistration` subjects), import bookkeeping columns (`import_source_id` / `import_session_id` on both events tables). The models (`Event`, `EventRegistration`, `TicketTier`) and the engine seams that read them (resolver arms, page-builder sources, content import/export, random-data generator) are core code by decision until P7.
...
- **P6 (382)** — composer path-repository mechanics: the pilot widget becomes the first true package with its own `composer.json` (`config/plugins.php` stays the activation + ordering authority — no Laravel auto-discovery, per the 381 P6 recommendation); the **plain T1 squash** lands here (the parked launch item absorbed — three live migrations collapse into the regenerated dump, 348-precedent identity check). *(Scope split settled at the 381 close, superseding this line's earlier wording: the squash-boundary redraw moved to P7.)*
