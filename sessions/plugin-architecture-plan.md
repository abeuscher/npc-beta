...
**Arc progress (last update: session 381 close, 2026-08-18):** **P5 ✅ complete** (381) — the first domain vertical: the Events behavioral surface lives in `plugins/Events/` (both registration controllers + routes, quantities service, both observers, three mailables, reminders command + schedule, policy, `EventResource` + four pages, both import wizard pages, seven widgets nested under `Widgets/`, the events importer). Surfaces **3/4 PROVEN** (first shipping admin-page plugin; the extracted-vertical permission nuance recorded — `event` permissions stay core-seeded); **surface 8 PROVEN** (core `ImporterContribution` + `ImporterRegistry`; every core import surface resolves plugin importers through it); **surface 10 PROVEN, events slice** (core `App\Payments\Events\CheckoutSettled` dispatched by the Payments webhook's events branch; the plugin's `PromotePaidRegistrations` listener owns fulfillment; no listener = 200, rows stay pending, data kept). Route parity 272/272 after every phase; the Events×Payments presence matrix is executable (enabled twin, `EventsRemovedTestCase` vanish mirror, Events-without-Payments); standing guard `EventModuleBoundaryTest` (zero allowlist); landing-page factory lifted to core (`App\Services\EventLandingPageFactory` — models are core, so core callers survive the plugin's absence); models/schema stay core with the events-owned annotation in § 6.7 (the P7 redraw input); contract doc **v0.5.0**; fast Pest 3226/0. **P6 next (382 — First Composer Package & Migration Squash, Stage B entry):** LogoGarden alone becomes the first composer path-repo package; `config/plugins.php` stays the activation + ordering authority (no Laravel auto-discovery — amends surface 1's Stage B sentence; provider ordering is load-bearing and every remove-the-line test keys on the list); the **plain T1 squash** lands at P6 (three live migrations collapse into a regenerated dump, 348-precedent identity check) with the **squash-boundary redraw at P7** per the owner-approved 381 scope statement; Dockerfile manifest-copy fix for the manifests-before-source composer layers. Earlier: **P4 ✅** (380) — the Stripe rails in `plugins/Payments/`, the first shipping foundation plugin; core `CapabilityRegistry` (surface 13 **PROVEN**) with core-owned `CheckoutProvider`/`PaymentModeProvider` contracts; `PaymentModuleBoundaryTest` (zero allowlist); route parity 274/274; vanish proven; Cashier removed; contract doc v0.4.0; see the 380 log.  **P1 ✅** (377 — LogoGarden pilot + contract v0.1.0; see the 377 log). **P2 ✅** (378 — widget boundary real across all 41; `dataContracts()` multi-contract seam; standing guard `WidgetTemplateBoundaryTest`, zero allowlist; contract v0.2.0; see the 378 log). **P3 ✅ complete** (379) — the admin shell is a socket: `AdminPanelProvider`'s inline `->routes()` closure decomposed into eleven per-feature `routes/admin/` files with **exact route-list parity** (274 routes byte-identical, verified twice); core `app/Plugins/` registry (`AdminContribution` + `PluginAdminRegistry`, declared at plugin `register()`, consumed by the panel provider) carrying Filament discovery paths, an in-panel route file (force-wrapped in `Authenticate` by core — no opt-out), and permission names (swept idempotently by `PermissionSeeder`, granted to no shipped role; super-admin via the gate bypass); the three session-376 Filament conventions mechanical, convention 3 enforced by standing guard `PluginAdminCssGuardTest`; proven by the SocketProbe test fixture with enabled + remove-the-line mirror tests; contract doc **v0.3.0** (surfaces 3/4 DECLARED-with-mechanics per the honesty rule — promoted at 381 by the first shipping admin-page plugin); fast Pest 3156/0; see the 379 log.
