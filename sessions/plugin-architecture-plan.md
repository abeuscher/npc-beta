...
**Arc progress (last update: session 387 close, 2026-08-19):** **P1–P10 + D1 ✅ complete (sessions 377–387) — Stages A, B, C-lite closed; Stage D open.** D1 (387): Payments packaged and extracted in one session — the manifest walk `folder` → `path` → `vcs`, ending at `github.com/abeuscher/crm-plugin--payments` (tag `v0.1.0`); the first extracted foundation plugin (the surface-13 capability seam held — nothing outside the plugin edited beyond three test file-read re-points) and the first dependency-carrying package (`stripe/stripe-php` package-owned, root's direct line dropped, lock resolution identical); contract doc **v0.11.0**; fast Pest 3263/0; route parity 272/272. **Next — D2 (Events extracted)**; the 387 log's D2 kickoff input section carries the known Events edges. **The 385 reframe (owner ruling):** no launch — the finish line is the end-state architecture via **§ Stage D — full decomposition** below (~15–21 sessions, positions D1–D12), then **§ Stage E** (niche distributions) as the follow-on arc. Per-stage detail lives in § 7; per-session detail in the session logs (the pre-385 session-by-session narrative formerly held here is compressed into the § 7 stage ladder — the logs are canonical).
...
- **D2 — Events extracted** (~1). The known-edges runbook run: fixture `basePath('plugins/…')` migration-path reaches, the `Events` testsuite location, the EventsListing→BlogPager cross-plugin asset input, thumbnail shipping.
