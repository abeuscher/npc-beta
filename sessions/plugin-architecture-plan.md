...
**Arc progress (last update: session 390, 2026-08-19):** **P1–P10 + D1–D2 ✅ complete (sessions 377–388) — Stages A, B, C-lite closed; Stage D running.** D2 (388): Events extracted to `github.com/abeuscher/crm-plugin--events` (tag `v0.1.0`, manifest `path` → `vcs`) — the known-edges runbook run: first migration-owning extraction (the disabled-≠-uninstalled fixtures re-pointed to the vendor'd migrations, assertions unchanged), first testsuite-carrying extraction (the 29-file `Events` suite travels with the package; phpunit re-points to `vendor/nonprofitcrm/events/tests`, identical counts in core CI), gate 4 live at scale (five vendor-relative asset paths, bundle content + the BlogPager cross-input proven), and `plugins/` emptied until D3's carve (scoped guard relaxations; root PSR-4 stays until D12); contract doc **v0.12.0**; fast Pest 3263/0; route parity 272/272. **D3 running — carve (389) and package (390) done:** `plugins/Donations/` refilled and now the fourth path-repo package `nonprofitcrm/donations` owning its four-table schema (the second squash-boundary redraw, both identity checks clean); surfaces 7 + 10-donations PROVEN; contract 0.14.0; Payments v0.2.0 + Events v0.1.1 released; next in the block: extraction (391) via the runbook. **The 385 reframe (owner ruling):** no launch — the finish line is the end-state architecture via **§ Stage D — full decomposition** below (~15–21 sessions, positions D1–D12), then **§ Stage E** (niche distributions) as the follow-on arc. Per-stage detail lives in § 7; per-session detail in the session logs (the pre-385 session-by-session narrative formerly held here is compressed into the § 7 stage ladder — the logs are canonical).
...
- **D3 — Donations vertical** (~2–3). The hardest remaining entanglement (funds, receipts, transactions); expected to force surface 7 (email-template registry) and surface 9 (contact-extension socket) into PROVEN. **Block opened at 389 — the carve is done:** the behavioral surface lives in `plugins/Donations/` (checkout controller + route, observers, policies, mailables, the four admin surfaces, the importer contribution, both widgets, three listeners); surface 7 PROVEN (core `EmailTemplateRegistry`, the two donation handles its first contribution); surface 10's donation slice PROVEN via the **first cross-repo inversion** — `crm-plugin--payments` v0.2.0 (all three donation-reaching webhook branches → `CheckoutSettled` / new core `InvoiceSettled` / `InvoicePaymentFailed` dispatches) plus a rider `crm-plugin--events` v0.1.1 (listener metadata guard + a removal-mirror assertion generalized for sibling importers); surface 9 not forced (no contact-screen socket needed — honesty rule). Contract 0.13.0. **Package done (390):** `nonprofitcrm/donations` is the fourth path-repo package and the second squash-redraw consumer — the four donation tables left core's dump for plugin-owned byte-faithful migrations, both per-composition identity checks verified clean, the setup checklist's fund row gated on the funds admin route (the one composition-safety find); contract 0.14.0. Remaining in the block: extraction via the runbook (391).
