...
**Arc progress (last update: session 386 close, 2026-08-19):** **P1–P10 ✅ complete (sessions 377–386) — Stages A, B, and C-lite all closed.** P10 (386): the distribution manifest is real — `distribution.json` is the plugin-set + ordering authority, `config/plugins.php` its generated artifact (`plugins:manifest-sync`), the standing alignment guard pins every remaining hand-maintained plugin surface to it, and the composition-by-manifest verified run passed (committed manifest = full composition; Events-entry-removed manifest = the 384 events-disabled facts, by the manifest edit alone); contract doc **v0.10.0**; fast Pest 3263/0; route parity 272/272. **Stage D next — D1 (Payments packaged + extracted)**; the 386 log's Stage D kickoff input section carries what D1 needs. **The 385 reframe (owner ruling):** no launch — the finish line is the end-state architecture via **§ Stage D — full decomposition** below (~15–21 sessions, positions D1–D12), then **§ Stage E** (niche distributions) as the follow-on arc. Per-stage detail lives in § 7; per-session detail in the session logs (the pre-385 session-by-session narrative formerly held here is compressed into the § 7 stage ladder — the logs are canonical).
...
- **D1 — Payments packaged + extracted** (~1). Stage B packaging (P6 mechanics) + runbook run. Retires part of the root `"Plugins\\"` PSR-4 need.
