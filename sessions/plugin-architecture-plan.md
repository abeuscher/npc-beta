...
**Arc progress (last update: session 385 close, 2026-08-19):** **P1–P9 ✅ complete (sessions 377–385)** — Stage A (in-repo boundaries), Stage B (packages, one workspace, per-install activation, superset image), and the Stage C-lite opener are done. P9 (385): LogoGarden lives in its own public repo (`abeuscher/crm-plugin--logo-garden`, tag `v0.1.0`, composer VCS resolution, guards extended over the vendor path) and **`docs/plugin-extraction-runbook.md` shipped** as the repeatable procedure; contract doc **v0.9.0**; fast Pest 3249/0; route parity 272/272. **P10 next (386 — Distribution Manifest & Central Build**, the closing Stage C-lite session; prompts drafted at 385 close). **The 385 reframe (owner ruling):** no launch — the finish line is the end-state architecture via **§ Stage D — full decomposition** below (~15–21 sessions, positions D1–D12), then **§ Stage E** (niche distributions) as the follow-on arc. Per-stage detail lives in § 7; per-session detail in the session logs (the pre-385 session-by-session narrative formerly held here is compressed into the § 7 stage ladder — the logs are canonical).
