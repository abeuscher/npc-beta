...
We are about to begin a new session: **151. Widget Delete & Livewire Inspector Removal**.
...
3. Read `docs/editor-migration-plan.md` **in full** — this is the architectural plan for the Vue migration. Every design decision, API endpoint, store shape, and component structure is documented there. This session removes the Livewire inspector and adds widget deletion.
4. Open `sessions/151. Widget Delete & Livewire Inspector Removal.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/151. Widget Delete & Livewire Inspector Removal — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit on the current `session-151/N` branch, and notify the user. Do not push — the user will push and merge when ready.
