We are about to begin a new session: **088. Image Optimization & SVG Support**.
...
3. Open `sessions/088. Image Optimization & SVG Support.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/088. Image Optimization & SVG Support — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit to a feature branch named `session-088`, and notify the user. Do not push — the user will push and merge when ready.
