...
- **Session 291 — Form Submission Notifications.** Queued. Lifted-gap session (outside the track's phase count, like 285 / 288) resolving G23. Wires form submissions to operator-configurable site-owner email via the existing mail infra (`Form.settings.notifications` + a Mailable + an observer + a `site_owner_email` SiteSetting + a FormResource UI), then populates the seeded `contact-page` form so `/contact` notifies on submit. Lands before Demo so the Demo submit flow builds on it. Scope canonical at `sessions/scoping-form-notifications.md`; security constraints binding per `docs/security-forms.md` rows 4–9.
- **Session 292 — Demo Page Build.** Queued. Shape change vs the content sessions: page itself is short (one band, two cells, one button per `demo-page-spec.md` — already accepted at 289 close), but the conversion-action wiring is a code-architecture deliverable (single-button auto-login endpoint on the demo server with per-IP rate-limit, shared "Demo User" account, demo page hosted on the demo server) and now builds on 291's notification primitive. Fleet Manager shared-contract updates (demo-server identity, abuse alerting, wipe coordination) flagged as cross-session work, not absorbed into 292.
