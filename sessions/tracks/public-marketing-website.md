...
- **Session 290 — Contact Layout Rebuild.** Queued. Greenfield page; two or three bands per `copy.md` (Hero / Email / What I respond to fastest — last band optional, designer's-call). Optional `sessions/public website/contact-page-spec.md` pre-pass; same-session sketch from `copy.md` is the fallback if no pre-pass lands. Email band is a `mailto:` link, not a form.
- **Session 291 — Demo Page Build.** Queued. Shape change vs the content sessions: page itself is short (one band, two cells, one button per `demo-page-spec.md` — already accepted at 289 close), but the conversion-action wiring is a code-architecture deliverable (single-button auto-login endpoint on the demo server with per-IP rate-limit, shared "Demo User" account, demo page hosted on the demo server). Fleet Manager shared-contract updates (demo-server identity, abuse alerting, wipe coordination) flagged as cross-session work, not absorbed into 291.
