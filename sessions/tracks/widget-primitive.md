...
**Last update:** 2026-04-28 (Phase 5d closed, session 234 shipped Recent Donations).

**Complete:** Phases 1, 2, 3 (3a–3d), 4 (4a–4h + follower), 5a, 5b, 5c, 5c.5, **5d (5d-1/5d-2/5d-3 — closed and compressed)**. Sessions 209–224, 227, 228, 229, 230, 231, 232, 234.

**Active:** Phase 5e (Permissions in the Contract Layer) is **queued as session 235** — lifts the per-arm `Gate::denies` line that landed on each of the three 5d resolver arms into a declared `requiredPermission` field on `DataContract`, enforced uniformly at the resolver dispatch boundary. The centralization is unblocked because all three arms reduced to "single permission name → fail-closed return that's a function of cardinality alone" — clean compression possible at the close-gate analysis.

**Remaining in track:** ~2–4 sessions.

- Phase 5e — Permissions in the contract layer (1 session, queued as 235)
- Phase 5d-4 — Recent Activity widget if pursued (1–2 sessions; aggregates across multiple system models — its own design surface and a fourth permission-gate evidence point that 5e's grammar can be stress-tested against)
...
Three concrete consumers of the record-detail Slot primitive landed in succession, each adding a new arm to `ContractResolver::resolveSystemModel()` plus a `projectXxx()` method on `SystemModelProjector` plus a server-rendered Blade widget plus a seeder attachment to the `contact_overview` View.

- **5d-1 (231) Recent Notes.** First concrete consumer; `note` arm in cardinality-many; six concept-named fields; body `Str::limit(strip_tags, 140)`; `EditContactView` sub-page wired up. Iteration-2 introduced two UX refinements driven by manual testing: the `HasRecordDetailSubNavigation` trait's threshold now ignores the entry page (sub-nav stays hidden when only the seeded primary View exists), and `RecordDetailViewResource::canDelete()` plus `DeleteBulkAction::before()` protect seeded-primary Views via a `primaryHandles()` registry. Architectural alignment surfaced: most record types have one View; multi-View is for record types with genuinely-different sub-areas (Templates/Themes is canonical).
- **5d-2 (232) Membership Status.** First single-row record-detail contract; `membership` arm in cardinality-one (sibling to `event` and `product`); seven concept-named fields; ambient gate narrows to Contact records; `RecordDetailViewSeeder` refactored from count-based early-return to per-handle `attachWidgetIfMissing()` so 5d-1 installs gain new widgets idempotently. **Pivot at close-gate**: prompt-drafting for 5d-3 surfaced a structural gap (financial rows conflated origin and lifecycle under `status`) → spawned the *Financial Data Origin & Lifecycle Discipline* track. Phase A jumped the queue as session 233, pushing 5d-3 to 234.
- **5d-3 (234) Recent Donations.** Third concrete consumer; `donation` arm in cardinality-many; **first read-side consumer of session 233's `source` column** via a `donation_origin` projector field (Stripe / Imported / Manual). Status filter `IN ('active', 'cancelled', 'past_due')` excludes only `pending` (checkout-in-flight); `started_at`/`created_at` order-by whitelist. Surfaced an importer↔widget status mismatch — donations importer's `mapDonationStatus()` writes `'completed'` for `active`/`paid`/`succeeded`/blank, which falls outside the canonical four-value enum and outside the widget filter; carried forward as a 235 candidate or Phase B work. **Origin verification ergonomics**: post-implementation, added a Source badge column to `/admin/donations` and an "Origin: X" line to the Recent Donations widget template — the column was previously back-end-discipline-only with no admin display surface.

**Carry-forward into Phase 5e.** Each arm gained a single inline `auth()->user()?->can($permission)` gate at its head, returning the cardinality-derived empty shape on deny. Three structurally identical gates accumulated by 234 close: clean compression possible into a `requiredPermission` field on `DataContract` enforced at the resolver dispatch boundary. Phase 5e (session 235) is the lift.

**Cumulative widget count and test footprint.** Phase 5d added `recent_notes` / `membership_status` / `recent_donations` to the registry (`widget_types` count: 34 → 37). Fast Pest grew from 1543 (post-5c.5) to 1641 (post-5d-3) — net +98 across the phase. Recent Donations was the first record-detail widget pinning a per-row visible affordance (origin caption) on a column from a separate track.

**Deferred at Phase 5d close.** Membership history widget, projector-side label assembly for Membership Status, `show_amount` config knob, placeholder widget removal, DashboardConfig→DashboardView retrofit, Themes/Templates nav merge, importer→widget status mismatch fix.
...
- **5d** (sessions 231, 232, 234, complete): three concrete record-detail widgets — Recent Notes, Membership Status, Recent Donations — each contributing a per-arm permission gate that 5e centralizes.
- **5e** (queued as session 235): permissions in the contract layer — lift the three per-arm `Gate::denies` lines into a `requiredPermission` field on `DataContract` with resolver-dispatch-level enforcement.
- **5d-4 if pursued**: Recent Activity widget aggregating across multiple system models; not currently queued.
- **Cheap follow-up session** (post-5d, scheduled ad-hoc): convert `DashboardConfig` (session 215) to a `DashboardView` model implementing `IsView`. Brings dashboards into the unified View vocabulary. Small surface, no schema migration if Eloquent overlay is sufficient; small migration if rename-to-`dashboard_views` is preferred.
...
- **Permissions in the contract layer.** ~~Pending~~ **Queued as session 235.** The 5d arc landed three structurally-identical inline `Gate::denies` lines at the head of the note / membership / donation resolver arms; close-gate analysis at 234 confirmed clean compression is possible into a `requiredPermission` field on `DataContract` enforced at the resolver dispatch boundary. Spatie permission-name-string single-check coverage; instance-bound `Gate::denies('view', $model)` shape stays as a forward hook for a later arm if needed. Moves into `Phase Retrospectives` at 235 close.
