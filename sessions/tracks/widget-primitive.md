...
**Last update:** 2026-04-27 (Phase 5c.5 shipped, session 230).

**Complete:** Phases 1, 2, 3 (3a–3d), 4 (4a–4h + follower), 5a, 5b, 5c, 5c.5. Sessions 209–224, 227, 228, 229, 230.

**Active:** Phase 5d-1 — first concrete record-detail widget (Recent Notes for the current contact). "Earn the primitive" milestone — contract design + projector + test-lock pair, mirror of Phase 4a. Forces a minimal Phase 5e decision (literal `Gate::denies('view_note')` check at the resolver's note arm, fail-closed) without committing to the full perm-in-contract architecture. Also wires `EditContact::recordDetailViewSubPageClass()` to a concrete `EditContactView` page, resolving the "first View wins on EditContact" gap surfaced during 5c.5 manual testing.

**Remaining in track:** ~4–7 sessions.

- Phase 5d-1 (1 session, the next one — Recent Notes)
- Phase 5d-2+ — remaining concrete record-detail widgets: Recent Donations (forces full Phase 5e), Membership Status, Recent Activity (~3–5 sessions, one widget per session)
