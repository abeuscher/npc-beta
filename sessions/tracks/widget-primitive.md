...
### Phase 5d-1 — First concrete record-detail widget: Recent Notes (session 231)

First concrete consumer of the record-detail Slot primitive. Contract + projector + test-lock pair on the `note` system model; per-arm `Gate::denies('view_note')` permission gate at the resolver, fail-closed; ambient-record scoping derived from `RecordDetailAmbientContext::$record` rather than per-instance contract filters. Six-field concept-named projector map (`note_id`, `note_subject`, `note_body_excerpt`, `note_type`, `note_occurred_at`, `note_author_name`) per the 226 convention; body is `Str::limit(strip_tags(...), 140)` so full body never crosses the contract boundary. Limit clamped 1–50 (default 5) inside the widget's `dataContract()`; `order_by` whitelisted to `occurred_at` (default) + `created_at` with a fallback for unknown values. SOURCE_SYSTEM_MODEL retrofit shape — `resolveSystemModel()` gained a `SlotContext` parameter (small upstream signature change; only the dispatch site needed updating, the post / event / product arms unchanged). RecentNotes widget itself is a server-rendered Blade with no JS / SCSS / declared assets — `npm run build` and `build:public` both unnecessary. EditContact gained a concrete `EditContactView` sub-page wiring (extends `RecordDetailViewPage`, route `contacts/{record}/edit/view/{view}`, access gate mirrors EditContact's view/edit policy), and the seeder (`RecordDetailViewSeeder::seedContactOverview()`) swapped the placeholder widget out for `recent_notes`. Fast Pest 1574/0 (+31 net from 230's 1543).

**Iteration 2 — sub-nav threshold + primary-View deletion protection.** User manual testing surfaced two emergent UX bugs from how the trait threshold and Resource canDelete behaviors composed: (a) sub-nav rendered with the seeded primary View alone because the `HasRecordDetailSubNavigation` threshold counted the entry page toward total, and (b) the user could delete the seeded primary `contact_overview` through `/admin/record-detail-views`, leaving Contact with no foundation View and the only forward path being to *create* a new View (which then re-triggered sub-nav). Iteration-2 fixed both: the trait threshold now gates on `$views->count() + count($additionalPages)` only (the entry is the implicit primary destination and doesn't count); `RecordDetailViewResource::canDelete()` returns false for primaries per a new `primaryHandles(): array<class-string, array<int, string>>` static helper, and `DeleteBulkAction::before()` cancels with a notification if any selected row is primary. Templates still surface their sub-nav (3+ navigable destinations easily clear the threshold). Fresh installs with `(Contact, contact_overview)` registered as primary cannot accidentally orphan the foundation View through the UI; future record types append to `primaryHandles()` to gain the same protection.
...
- **5b** (session 228): Filament mount point for record-detail + `RecordDetailSidebarSlot::ambientContext($record)` body opens + `SOURCE_RECORD_CONTEXT` source/projector/tokens + `IsView` interface + first concrete `RecordDetailView` (hardcoded array, table-backing in 5c) + placeholder widget that confirms the ambient flows on `EditContact`. The named risk for the Phase 5 arc — "low probability the contract layer cannot coexist with Filament's Livewire model" — gets answered here.
- **5c**: `record_detail_views` table + `IsView` registry + sub-nav rendering primitive. Templates/Themes record sub-navigation refactor folds in as a consumer of the same primitive (`EditPageTemplate` and `EditContentTemplate` adopt `getRecordSubNavigation()` driven by Views bound to template type). Admin UI for authoring per-record-type View sets.
- **5d+**: Concrete record-detail widgets — Recent Notes for current contact, Recent Donations, Membership Status, etc. Each is a contract design + projector + test-lock pair, mirror of Phase 4a–4h.
