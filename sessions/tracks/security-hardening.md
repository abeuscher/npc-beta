...
**Follow-on (session 375) — the public CSP is now proven compatible with the shipped front end.** S1's enforced public policy was correct but untested against the built public bundle: `script-src` carries no `'unsafe-eval'`, and standard Alpine evaluates every directive through `new Function()`, so from the S1 deploy until the session-374 recon **every public page threw `EvalError` at startup and no Alpine component initialized** — nav dropdowns, custom selects and the theme store were dead in production, and the console errors were the only outward sign. **The policy was not weakened to fix it** (the `'unsafe-eval'` carve-out was considered and rejected by the owner): the public bundle moved to `@alpinejs/csp`, whose parser accepts a strict JavaScript subset, and the twelve affected widgets moved their JS into registered components. The admin panel's `'unsafe-eval'` carve-out and Report-Only posture are untouched, so residual (a) below stands unchanged. Two standing guards now cover the class — `tests/Feature/PublicAlpineCspGuardTest.php` (static, browser-free) and `tests/e2e/public-console-errors.spec.ts` (asserts the pages are console-clean **and** the components still respond). **Lesson for the remaining track sessions: a header-level security control is not verified until something exercises the surface it governs in a real browser.**
