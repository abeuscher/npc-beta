...
- **Consult the Deliberately Kept register.** Findings that match a registered carve-out get skipped, not landed. **Retirement check:** if a carve-out's original justification looks stale (the architectural reason no longer holds, the cited session's decision has been superseded, the code the carve-out protects has materially changed), surface the entry for retirement at the 313 walkthrough rather than silently skipping the finding. The retirement decision belongs to the walkthrough, not the audit; the audit's job is to flag the staleness with citation.
- **New carve-outs surfaced during the audit get proposed for register addition;** the addition itself happens at the 313 walkthrough, not unilaterally during the audit.
- **Open Refactor Flag captures over-scope.** If a finding is too big for a single 313 iteration (e.g. "restructure the importer subsystem around polymorphism instead of `match()`"), capture as an Open Refactor Flag with risk + scope + proposed paths. 313 decides whether to absorb a slice or punt to a dedicated session.
...
3. **Propose additions for the 313 walkthrough.** If the workstream walks below surface patterns that look intentional but aren't in the register, capture them in a *Proposed additions* sub-section of the audit log — with citation. The user ratifies additions at 313; do not edit the register file during the audit.
4. **Propose retirements for the 313 walkthrough.** Stale entries (from step 2) get a *Proposed retirements* sub-section in the audit log, with the staleness rationale. Same discipline: user ratifies at 313; no unilateral edits.
...
**Skip if matches Deliberately Kept register.** Otherwise land on backlog for 313.
...
- **Suggested next step:** [usually "document the intent before refactoring"; sometimes "ask the user during 313 walkthrough"]
```

These feed the **next cycle's W-R5 walk** as documentation-gap pre-loaded findings. They are not in scope for 313's apply work.
...
- **Risk + scope:** [why it's bigger than a single 313 iteration]
...
- **Existing suite confirmation:** run `docker compose exec app php artisan test --exclude-group=slow` once at session start to confirm green baseline. Note the count for the 313 prompt. No suite runs expected during the audit itself.
- **Baseline carry-in:** confirm baseline from cleanup Cycle 3's squash close; that's what 313 will measure against.
...
- **Apply phase** — backlog walkthrough, ratification, iteration-sliced refactors. **Deferred to session 313.**
...
- The new artifact (`sessions/tracks/code-shape-and-fit-finding-template.md`) is a doc file under `sessions/tracks/`. It ships with the session log commit. The Deliberately Kept register is not edited during this session (validate-and-extend; ratification happens at 313).
...
- **Log file:** `sessions/312. Code Shape & Fit (Audit) — Cycle 1 — Log.md`. Include:
  - Per-workstream finding count + summary
  - Intent-unrecoverable findings section (separate)
  - Open Refactor Flags with full context for 313
...
- **Branch:** `session-312/N` (final iteration). Commits: the finding-template artifact + the audit log. No code changes. **No edits to the Deliberately Kept register file** — register changes are 313 walkthrough work.
- **Draft session 313 documents at close.** Copy `sessions/template-base-prompt.md` to `sessions/313. base-prompt.md`; write `sessions/313. Code Shape & Fit (Apply) — Cycle 1.md` per the 313 draft (separate file). The 313 prompt's *Open Refactor Flags* + *W-R1 through W-R5 backlog* sections carry forward from this session's log; *proposed register additions/retirements* land in 313's walkthrough block.
- **Update the track doc.** `sessions/tracks/code-shape-and-fit.md` § *Cycle Retrospectives* gets a stub entry for Cycle 1 (window covered, sessions list pointing at 312/313). The full retrospective lands at 313's close, not here.
- **Stub closeout:** `sessions/session-outlines.md` § "Code Shape & Fit — 2-session cycle" — mark the 312 child stub as ✅ closed; preserve 313 stub untouched.
