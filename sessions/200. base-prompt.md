# Base Prompt — Session 200

We are about to begin a new session: **200. Admin CSS Consolidation — Tightening Pass**.
...
2. Read `docs/app-reference.md` for environment names, container names, view-to-file mappings, and key dependencies. No schema tables are touched this session.
3. Read the three files whose shape the session is rearranging, in order:
   - `public/css/admin.css` in full — the 1000+ line override file loaded via render hook from `AdminPanelProvider`. Note the three distinct zones: Filament chrome overrides, Vue page-builder dark-mode overrides, and the help-system Blade chrome.
   - `resources/js/page-builder-vue/styles/inspector.css` — the shared light-mode inspector primitives.
   - `resources/js/page-builder-vue/App.vue` (just the `<style>` block) — the component-scoped style convention the editor follows, and the entry point for `inspector.css` import.
4. Skim the list of Vue files whose dark-mode rules currently live in `admin.css`. They share a predictable naming convention (`.editor-toolbar*`, `.preview-canvas*`, `.inspector-*`, `.color-picker__*`, `.gradient-picker__*`, `.bg-panel__*`, `.nine-point*`, `.image-upload*`, `.button-list*`, `.spacing-control`, `.query-settings`, `.widget-appearance`, `.layout-inspector`, `.block-list*`, `.apply-changes`, `.confirm-delete-*`, `.add-block-dropdown*`, `.inspector-header*`, `.inspector-tabs*`). Each maps to a corresponding Vue component under `resources/js/page-builder-vue/`. Do not exhaustively read every Vue file up front — read each one immediately before migrating its block.
5. Read the `--np-control-*` token block at the bottom of `admin.css` and the Quill toolbar rules that consume it, so the token vocabulary is clear before raw-RGB values are migrated.
6. Read `app/Providers/Filament/AdminPanelProvider.php` briefly — confirm the render-hook that injects `/css/admin.css` and the `viteTheme('resources/css/filament/admin/theme.css')` entry. This session does **not** move overrides to the Vite theme; it only tightens what already lives in `admin.css`.
7. Open `sessions/200. Admin CSS Consolidation — Tightening Pass.md` and read the session prompt carefully.
