We are about to begin a new session: **182. Design System Editor — Typography**.
...
3. Open `sessions/182. Design System Editor — Typography.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/182. Design System Editor — Typography — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Archive the previous session**: move all files matching `sessions/181. *.md` into `sessions/archived/`. This includes the previous session's base prompt, session prompt, and log. Skip silently if those files don't exist or have already been archived (e.g., the user moved them manually).
- **Commit**: stage all changed files (including the log, updated outlines, and any files moved into `sessions/archived/`), commit on the current `session-182/N` branch, and notify the user. Do not push — the user will push and merge when ready.
