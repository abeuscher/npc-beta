We are about to begin a new session: **089. Image & Carousel Widgets**.
...
3. Open `sessions/089. Image & Carousel Widgets.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/089. Image & Carousel Widgets — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit to a feature branch named `session-089`, and notify the user. Do not push — the user will push and merge when ready.
