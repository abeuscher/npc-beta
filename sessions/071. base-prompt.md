We are about to begin a new session: **071. Portal Household Invitations**.
...
2. Open `sessions/071. Portal Household Invitations.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/071. Portal Household Invitations — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
- **Update session-outlines.md**: move this session's title into the Completed Sessions table. Remove "Gated Pages" from the upcoming stubs (already built). Review remaining stubs and update if this session's work affects them.
- **Commit**: stage all changed files (including the log and updated outlines), commit to a feature branch named `session-071`, and notify the user. Do not push — the user will push and merge when ready.
