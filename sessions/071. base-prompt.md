We are about to begin a new session: **071. Household Linking**.
...
2. Open `sessions/071. Household Linking.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/071. Household Linking — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
