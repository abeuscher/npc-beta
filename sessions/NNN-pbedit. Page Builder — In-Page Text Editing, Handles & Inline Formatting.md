...
## Decided — capability-gate location (settled; do not relitigate in-session)

The inline-eligibility rule lives **in the widget's code (a declaration on the widget definition / PHP class), not the database**. No `widget_types` column, no migration, no `docs/schema/` change — this stays non-boundary and schema-neutral. Eligibility is a developer/design property of the widget, so it ships with the widget. The eligibility *model* is **per-node, path-addressed** (validated against PricingChart): a node is editable iff it is a text/richtext field reachable by a config path (including inside nested repeaters), token-free in this instance, and not data-driven/templated — combined with the **opt-in template annotation** rule and the **Tier-B exempt set** from the 40-widget pass. If, at implementation, the only clean place turns out to require a schema column, **stop and surface** — the decision is no-migration.

---

## Open questions to resolve at session start (the genuinely-open decisions)

1. **Scope split (Rule 11).** Recommended split if it inflates: **Session A = Phases 1–2 (+2b)** (interaction model + first safe inline slice + preview-refresh/lifecycle + the simplified column/repeater control) and **Session B = Phases 3–4** (custom toolbar + lock-step parity tests + widen the gate). Confirm the split with the user rather than running one mega-session.
...
Activate a Vue contenteditable layer in `PreviewRegion.vue` on `[data-config-key]` nodes **of the selected, inline-eligible widget only**, where `data-config-key` is a **config path** (supports nesting, e.g. `columns.2.title`). Bind edits to the value at that **raw config path**, route through `updateLocalConfig()` → existing debounced save, on blur + debounce. Implement the **capability gate** (decided: declared in widget code, per-node path-addressed — see "Decided — capability-gate location") and the **token/data-driven exclusions**. **Refresh-suppression is text-only:** a text-content edit suppresses the preview refresh for the active node (mirror the `RichTextField.vue` dirty-guard); a *structural* edit (Phase 2b) always re-renders. Inline-eligible fields must render an **empty editable placeholder wrapper even when blank** (templates currently omit empty nodes — this is a cross-widget template-pattern change; see the discoverability spec below for what fills it). Confirm sanitization + allow-list cover every inline path (`content`, `heading_N`, `body_N`, and nested `columns.*.…`). Start with the unambiguous targets: **TextBlock**, **Hero** (token-free), **ThreeBuckets** bodies; then prove the nested case on **PricingChart**. Playwright: type-in-place → blur → reload → persisted (incl. a nested `columns[i].attribute_rows[j].value`); a `{{token}}`-bearing node is **not** inline-editable; a data-driven widget is **not** inline-editable; a plaintext node shows **no** formatting toolbar.

### Phase 2 — discoverability spec (how the user knows a region is editable)

**Principle: progressive disclosure scoped to the selected widget.** Editability is *never* revealed page-wide; selecting a widget (Phase 1 model: body-click / handle / Edit affordance) is what *arms* its editable regions. The **unselected preview stays pixel-clean** — no outlines, no placeholders, identical to the published page (this preserves WYSIWYG and keeps visual review / screenshots honest). All cues are gated behind selection (or hover within the selected widget). This is the deliberate trade: a first-time user learns "select a widget to edit it" once (carried anyway by the Phase 1 handle + Edit affordance) in exchange for an honest preview.

- **Empty slots → schema-`label` ghost text.** The empty placeholder wrapper renders muted placeholder text derived from the field's existing schema `label` (e.g. `"Add heading…"`, `"Add body text…"`), styled as the real text would render. This is the concrete content of the empty-field placeholder-wrapper accommodation — a *positive* affordance (says what goes there), not a highlight box. Shown only within the selected widget.
- **Non-empty regions → the dormant `.inline-editable` CSS.** Activate `public/css/admin.css:677–692` (dashed outline on hover, solid on focus), scoped to the selected widget, plus `cursor: text` on hover. Nothing new to author — it already exists.
- **Generous hit target.** The clickable zone is the whole slot including its padding/whitespace; clicking anywhere in the region focuses the editor and drops the caret. No pixel-perfect hit on tiny or empty text required (this is what makes the empty case forgiving — placeholder gives an aim point, the padded slot makes it generous).
- **Confirmation signal.** Focus reveals the Phase 3 formatting toolbar — that *is* the "you are now editing" confirmation; no separate indicator needed.
- **Playwright:** the unselected preview exposes **zero** inline affordances/placeholders (assert clean); selecting a widget reveals schema-label placeholders on its empty inline slots; hovering an editable region in the selected widget shows the outline + text cursor; clicking the whitespace of an empty slot focuses the editor and places the caret.
...
- **Next session:** the deferred phase-slice if split per Open Q1, drafted only when the user names it.
