...
Copy this file to `176. base-prompt.md` at the start of each session. Replace `176` with the session number and update the session title.

---

We are about to begin a new session: **176. Designer Preset Drafts & Export**.
...
3. Open `sessions/176. Designer Preset Drafts & Export.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/176. Designer Preset Drafts & Export — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
