# Base Prompt — Session 298 (Test Feedback Loop — Scoped Loop + Async Verification)

> **Reshuffled into the numbered sequence at session-297/2: this is session 298.** Emergent, **iteration-cost forcing function** (the fast suite is ≈11 min/run, run ~hourly — dead time on scoped design work). **Session 1 of a two-session split**: this session (298) = scoped inner loop + async full-suite verification + slow-group hygiene; **Session 2 (299)** = parallelization + test-isolation cleanup. Independent in code but 298 should land first (it provides the CI that 299's parallelized suite runs in). Slot order: 298 → 299 → 300 (Widget Color-Token Audit, the arc RISK session). Warrants a `release-plan.md` note linking it to § D4.

**Start now.** Execute the reading list, open `sessions/298. Test Feedback Loop — Scoped Loop & Async Verification.md`, then begin. No confirmation needed.

---

We are about to begin a new session: **298. Test Feedback Loop — Scoped Inner Loop & Async Full-Suite Verification**.
...
**Sequencing note (ROI):** this session is independent of the Theme/Template Re-Taxonomy arc, but its payoff is highest **run immediately before the arc's RISK session (300 — Widget Color-Token Consumption Audit)**: that is a ~38-widget SCSS migration with many build/verify cycles over exactly the drift cluster the scoped loop targets. Recommend slotting 298 right before 300; 299 can follow or interleave. (Reflected in the numbering: 298 → 299 → 300.)
...
7. Open `sessions/298. Test Feedback Loop — Scoped Loop & Async Verification.md` and read it carefully.
...
Successor is **Session 2 (`299`, parallelization + isolation)** — its draft exists; refresh its inherited-state against what this session shipped (esp. the CI workflow it will run in). Draft nothing else unless the user names it.

### Phase 2 — Close

When the user says to close: log `sessions/298. Test Feedback Loop — Scoped Loop & Async Verification — Log.md` per the template (scoped-selection durable form chosen + rationale; the CI workflow; the `CLAUDE.md`/template close-gate rewording verbatim; the slow-group re-audit results; `VERSION` bump; drift). Update `completed-sessions.md`, archive the previous session's files, commit on the final `session-298/N` branch, notify, do not push. Never push to main / force-push / merge to main. Do not begin Session 2 until the user starts it.
