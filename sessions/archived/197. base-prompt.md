# Base Prompt — Session 197

We are about to begin a new session: **197. Quill Toolbar Visual Language — Reference-Image Pilot**.
...
2. Read `docs/app-reference.md` for environment names, container names, view-to-file mappings, and key dependencies. No schema tables are touched this session (pure CSS).
3. Read the reference image at `/mnt/e/Clients/nonprofitcrm/test_import_data/wysiwyg toolbar.jpg` using the Read tool. This is the visual target — button chrome, group separators, spacing, dropdown chip shape, two-row layout.
4. Read the current Quill surfaces end to end — the session styles both, and they share CSS:
   - `app/Forms/Components/QuillEditor.php` — Filament form field class (one-liner wrapper).
   - `resources/views/forms/components/quill-editor.blade.php` — field blade.
   - `resources/js/admin/quill-editor.js` — Alpine instance + toolbar config (used by the Filament form field).
   - `resources/js/page-builder-vue/components/fields/RichTextField.vue` — Vue component used by the page builder for `richtext` widget fields. Reuses the same global Quill.
5. Read `app/Providers/Filament/AdminPanelProvider.php` — specifically the `renderHook('panels::head.end', …)` at line 192–202 that loads Quill v2 + `quill.snow.css` from CDN on every admin page. This is where Quill CSS enters the admin shell.
6. Read the existing Quill overrides:
   - `public/css/admin.css` lines 844–862 — current dark-mode Quill overrides. Any new styles should sit near these or in a clearly separated section.
   - `resources/scss/_custom.scss` lines ~100–107 — public-side `.ql-align-*` rules (content-rendering only, unrelated to toolbar chrome, but worth being aware of).
7. Read `resources/docs/widget-development.md` — confirm that `richtext` widget fields use `RichTextField.vue` (Quill) and that styling changes propagate to every widget with a richtext field.
8. Grep for `QuillEditor::make` to confirm the admin-side usage sites: `app/Filament/Pages/Settings/GeneralSettingsPage.php`, `app/Filament/Resources/EmailTemplateResource.php`, `app/Filament/Resources/EventResource.php`. These are the visual test surfaces on the Filament side.
9. Open `sessions/197. Quill Toolbar Visual Language — Reference-Image Pilot.md` and read the session prompt carefully.
