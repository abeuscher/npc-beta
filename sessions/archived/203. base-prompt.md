# Base Prompt — Session 203

We are about to begin a new session: **203. CRM Importer — Notes & Interactions (Standalone Path)**.
...
2. Read `docs/app-reference.md` for environment names, container names, view-to-file mappings, and key dependencies. Read `docs/schema/README.md` for the table index, then read the relevant table files: `notes.md` (central — the destination table), `contacts.md` (lookup target), `import_sources.md`, `import_sessions.md`, `import_id_maps.md` (for constituent-ID → contact UUID resolution), `import_staged_updates.md` (for update-strategy semantics).
3. Read the files at the heart of this session:
   - `app/Models/Note.php` in full — fillable + casts + any scopes. The importer writes directly to this model.
   - `sessions/archived/198. Notes — Structured Interactions Schema & Timeline Surfaces — Log.md` — the canonical record of what the notes schema settled into, what the form surfaces do, and what the Timeline renders. The importer must produce notes that round-trip cleanly through the Timeline.
   - `app/Filament/Pages/ImportDonationsPage.php` and `app/Filament/Pages/ImportDonationsProgressPage.php` — the closest shape to what the Notes importer will become (relational: row → lookup contact → create child record → stage updates). Use as the structural template.
   - `app/Filament/Pages/Concerns/InteractsWithImportWizard.php` and `app/Filament/Pages/Concerns/InteractsWithImportProgress.php` — the shared traits. The new Notes page consumes both.
   - `app/Importers/DonationImportFieldRegistry.php` + `app/Importers/DonationFieldRegistry.php` + `app/Importers/TransactionFieldRegistry.php` — the field-registry trio pattern. `NoteImportFieldRegistry` and `NoteFieldRegistry` follow this shape.
   - `app/Filament/Pages/ImportContactsPage.php` — specifically the `__note_contact__` path (the existing **appendage** for a notes column on a contacts CSV). This is NOT being removed or enhanced by session 203. Read just enough to confirm the two paths stay cleanly separated: appendage for the degenerate "notes column on contacts.csv" case, standalone importer for separate activity files.
   - `app/Services/Import/CsvTemplateService.php` — template headers; session 203 adds `noteHeaders()`.
   - `app/Services/Import/FakeCsvComposer.php` and `app/Console/Commands/GenerateFakeImportCsvsCommand.php` — session 202's generator. Session 203 extends both to emit `notes.csv`.
   - `app/Console/Commands/SeedFakeImportSourcesCommand.php` — session 203 adds the sixth "Demo Fake Data — Notes" source.
4. Open `sessions/203. CRM Importer — Notes & Interactions (Standalone Path).md` and read the session prompt carefully.
...
- **Update session-outlines.md**: move this session's title into the Completed Sessions table. Review upcoming stubs and update them if this session's work affects their scope or dependencies. In particular, remove the "Importer — Custom Field Mapping for Notes / Interactions" stub that this session satisfies.
