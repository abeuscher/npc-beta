...
We are about to begin a new session: **164. Appearance Controls — Background Panel**.
...
3. Open `sessions/164. Appearance Controls — Background Panel.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/164. Appearance Controls — Background Panel — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
- **Update session-outlines.md**: move this session's title into the Completed Sessions table. Review upcoming stubs and update them if this session's work affects their scope or dependencies. This session is the first half of a two-iteration split of the original 164 outline — update the upcoming stubs accordingly: replace the original 164 stub with a new 165 stub ("Appearance Controls — Text & Section Layout Panels"), and renumber the existing 165 (Docs Finalization & Widget Spacing Harmonization) → 166.
- **Archive the previous session**: move all files matching `sessions/163. *.md` into `sessions/archived/`. This includes session 163's log (and base prompt / session prompt if present). Skip silently if those files don't exist or have already been archived. Session 164's own files stay in `sessions/` until session 165's close.
- **Commit**: stage all changed files (including the log, updated outlines, and any files moved into `sessions/archived/`), commit on the current `session-164/N` branch, and notify the user. Do not push — the user will push and merge when ready.
