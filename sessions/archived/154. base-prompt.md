...
We are about to begin a new session: **154. Nav Widget & Footer**.
...
3. Open `sessions/154. Nav Widget & Footer.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/154. Nav Widget & Footer — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit on the current `session-154/N` branch, and notify the user. Do not push — the user will push and merge when ready.
