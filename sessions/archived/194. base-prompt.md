We are about to begin a new session: **194. CRM Importer — Update-Existing Strategy for All Content Types**.
...
2. Read `docs/app-reference.md` for environment names, container names, view-to-file mappings, and key dependencies. Read `docs/schema/README.md` for the table index, then read the table files for every entity this session touches: `contacts.md`, `events.md`, `event_registrations.md`, `donations.md`, `memberships.md`, `transactions.md`, `import_sessions.md`, `import_logs.md`, `import_sources.md`, `import_staged_updates.md`, `import_id_maps.md`.
3. Read `app/Filament/Pages/Concerns/InteractsWithImportProgress.php` end to end — the progress trait is where `runDryRun()` / `tick()` / `finaliseSession()` live and is where the new update path will be plugged in for the four non-contact importers.
4. Read the four non-contact progress pages (`ImportEventsProgressPage.php`, `ImportDonationsProgressPage.php`, `ImportMembershipsProgressPage.php`, `ImportInvoiceDetailsProgressPage.php`) and their wizard counterparts — note exactly how each identifies existing records today (external_id, invoice_number, etc.) and what "reuse" currently means.
5. Read `ImportContactsPage.php`'s `processRow()` method and `ImporterPage.php`'s `approve`/`rollback`/`delete` actions to understand the existing contacts update-and-approve path — that's the template we're generalising.
6. Read `app/Models/ImportStagedUpdate.php` — existing polymorphic-candidate staging model.
7. Open `sessions/194. CRM Importer — Update-Existing Strategy for All Content Types.md` and read the session prompt carefully.
