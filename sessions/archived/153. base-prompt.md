...
We are about to begin a new session: **153. Column Layout System**.
...
3. Read `docs/editor-migration-plan.md` **in full** — this is the architectural plan for the Vue migration. Note that the session 153 section in this document is outdated — the session prompt takes precedence.
4. Open `sessions/153. Column Layout System.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/153. Column Layout System — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit on the current `session-153/N` branch, and notify the user. Do not push — the user will push and merge when ready.
