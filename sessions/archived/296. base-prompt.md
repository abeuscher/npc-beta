# Base Prompt — Session 296 (Post-Incident Test Integrity)

> **Number assigned: 296.** Emergent, post-incident session with no `release-plan.md` execution-order entry (it does not get one — see the qualifier below). Authored unnumbered on `claude/investigate-stale-stylesheet-ANZJt`; numbered to 296 at session-295 close-prep and scheduled as **the next session run, immediately after session 295** — ahead of the Theme/Template Re-Taxonomy arc (297+), which it deliberately de-risks (see the "Mobile/Theme — Theme & Template Re-Taxonomy" stub in `session-outlines.md`). Run on branch `session-296/1`.

**Start now.** Execute the reading list below, open the session prompt at `sessions/296. Post-Incident Test Integrity — Stale-Stylesheet Drift Guard.md`, then begin the work. No confirmation needed.

---

We are about to begin a new session: **296. Post-Incident Test Integrity — Stale-Stylesheet Drift Guard & Fixture-Test Remediation**.
...
3. There is **no `release-plan.md` entry** for this session. Read `release-plan.md` § D4 *only* for context on the project's test-audit artifact convention (`sessions/296-test-audit-findings.md`) — this session does not execute D4 and must not absorb D4's breadth.
...
7. Open `sessions/296. Post-Incident Test Integrity — Stale-Stylesheet Drift Guard.md` and read it carefully — it carries the canonical findings table (scope is fixed there), the drift-guard design, the open questions, the explicit deferred set, and a **Forward-awareness** subsection on an incoming taxonomy/tooling reshuffle that materially shapes how the guard must be written (design-settings group resolved dynamically not hardcoded; bundle-only by intent; do not over-invest in finding 5). Read that subsection *before* designing the guard.
...
- **Build/version discipline (carry-forward — load-bearing):** the deploy pipeline hard-fails a forgotten/duplicate repo-root `VERSION` bump. Bump `VERSION` to the assigned session's `0.296.<iteration>` as part of the iteration, exactly as every recent session does.
...
- **Commit:** stage all changed files (the service, the new guard + its test, every rewritten/deleted test file, the touched Playwright spec, `VERSION`, the log, `completed-sessions.md`, the housekeeping-inbox update, archived previous-session files) on the final `session-296/N` branch and notify the user. Do not push — the user pushes on their own cadence. Never push to main, never force-push, never merge to main yourself (see `CLAUDE.md` Git Workflow for the full forbidden list).
