...
Copy this file to `175. base-prompt.md` at the start of each session. Replace `175` with the session number and update the session title.

---

We are about to begin a new session: **175. Preset Inspector UI & Hero Preset Pilot**.
...
3. Open `sessions/175. Preset Inspector UI & Hero Preset Pilot.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/175. Preset Inspector UI & Hero Preset Pilot — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
