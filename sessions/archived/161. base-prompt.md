...
We are about to begin a new session: **161. Appearance Controls for Widgets — Docs Refresh & Shared Primitives**.
...
3. Open `sessions/161. Appearance Controls for Widgets — Docs Refresh & Shared Primitives.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/161. Appearance Controls for Widgets — Docs Refresh & Shared Primitives — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Archive the previous session**: move all files matching `sessions/160. *.md` into `sessions/archived/`. This includes session 160's log (and base prompt / session prompt if present). Skip silently if those files don't exist or have already been archived. Session 161's own files stay in `sessions/` until session 162's close.
- **Commit**: stage all changed files (including the log, updated outlines, and any files moved into `sessions/archived/`), commit on the current `session-161/N` branch, and notify the user. Do not push — the user will push and merge when ready.
