We are about to begin a new session: **193. CRM Importer — Commit Progress, Loading States & Large-Import Handling**.
...
2. Read `docs/app-reference.md` for environment names, container names, view-to-file mappings, and key dependencies. Read `docs/schema/README.md` for the table index; this session does not add or alter schema.
3. Read `app/Filament/Pages/Concerns/InteractsWithImportProgress.php` in full — this is where `runCommit()`, `tick()`, `finaliseSession()`, and `saveMapping()` live and is the trait all five progress pages use.
4. Read the blade views for every progress page. `resources/views/filament/pages/import-progress.blade.php` (contacts) plus `import-events-progress.blade.php`, `import-donations-progress.blade.php`, `import-memberships-progress.blade.php`, `import-invoice-details-progress.blade.php`. Note how each handles `phase === 'committing'` (`wire:poll`) and `phase === 'done'` (Save Mapping + action buttons).
5. Read `app/Filament/Pages/ImporterPage.php` and `resources/views/filament/pages/importer.blade.php` — the "Go to review queue" destination. Understand what queries run on render so we can gate the slow ones behind a loading state too.
6. Read `sessions/192. CRM Importer — Data Review Step.md` and `sessions/192. CRM Importer — Data Review Step — Log.md` (if present) for UAT context that motivated this session.
7. Open `sessions/193. CRM Importer — Commit Progress, Loading States & Large-Import Handling.md` and read the session prompt carefully.
