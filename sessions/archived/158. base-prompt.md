...
We are about to begin a new session: **158. Inspector Panel Reactivity**.
...
3. Open `sessions/158. Inspector Panel Reactivity.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/158. Inspector Panel Reactivity — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit on the current `session-158/N` branch, and notify the user. Do not push — the user will push and merge when ready.
