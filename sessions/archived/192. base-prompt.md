We are about to begin a new session: **192. CRM Importer — Data Review Step**.
...
2. Read `docs/app-reference.md` for environment names, container names, view-to-file mappings, and key dependencies. Read `docs/schema/README.md` for the table index, then read the individual table files under `docs/schema/` relevant to this session's scope — at minimum `import_sessions.md`, `import_sources.md`, `import_logs.md`.
3. Read the session 191 prompt (`sessions/191. CRM Importer — Polish, Shared Code & Source Templates.md`) for the trait structure and noise-detection groundwork this session builds on.
4. Read `app/Filament/Pages/Concerns/InteractsWithImportWizard.php` in full — this trait is where the new step will be wired. Read all five wizard pages under `app/Filament/Pages/Import*.php` (not the progress pages) to understand how each consumes the trait today.
5. Read `app/Services/Import/NoiseDetector.php` — it's the closest pattern for the new `DuplicateHeaderDetector` service. Read `app/Services/Import/FieldMapper.php` for the per-source skip-header machinery.
6. Open `sessions/192. CRM Importer — Data Review Step.md` and read the session prompt carefully.
