...
Copy this file to `181. base-prompt.md` at the start of each session. Replace `181` with the session number and update the session title.

---

We are about to begin a new session: **181. Migration Squash & Inline JS/CSS Audit**.
...
3. Open `sessions/181. Migration Squash & Inline JS&CSS Audit.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/181. Migration Squash & Inline JS/CSS Audit — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Archive the previous session**: move all files matching `sessions/180. *.md` into `sessions/archived/`. This includes the previous session's base prompt, session prompt, and log. Skip silently if those files don't exist or have already been archived (e.g., the user moved them manually).
- **Commit**: stage all changed files (including the log, updated outlines, and any files moved into `sessions/archived/`), commit on the current `session-181/N` branch, and notify the user. Do not push — the user will push and merge when ready.
