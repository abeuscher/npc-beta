...
### G23 — form submissions don't email the site owner ✅ resolved at 291
...
- **Resolution (291):** lifted-gap session 291 shipped it — but **as an operator-editable System Email**, not the originally-recommended fixed built-in template. `FormSubmissionMailable` renders via `EmailTemplate::forHandle('form_submission')`; `FormSubmissionObserver` (`#[ObservedBy]`) dispatches per `Form.settings.notifications` entry; the recipient token is the existing **`{{contact_email}}`** CMS setting (the proposed new `site_owner_email` key was added then removed on user direction — reuse over new surface); submission data enters only via an `e()`-escaped `{{submission}}` token with the subject rendered from a submission-data-free token set. Graceful mail-failure handling added (`FormNotificationDeliveryException` → form inline error, no page break, no leak; admin test-email hardened too). `contact-page` wired to `{{contact_email}}`. Security register `docs/security-forms.md` rows 4–9 moved to as-shipped. The earlier "Demo submit flow builds on this primitive" rationale was found stale at close — Demo (292) is form-less/no-email per the s289 architecture and does **not** consume this surface; the notification consumer is the Contact page. Fast Pest 2441 / 0. See `sessions/291. … — Log.md`.
