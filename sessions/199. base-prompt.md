# Base Prompt — Session 199

We are about to begin a new session: **199. Admin Secondary Button Colour & Header-Action Polish**.
...
2. Read `docs/app-reference.md` for environment names, container names, view-to-file mappings, and key dependencies. Read `docs/schema/README.md` for the table index, then read `docs/schema/site_settings.md` if present (the settings table this session extends).
3. Read the current admin-theme plumbing so the secondary colour is wired the same way the primary is:
   - `app/Providers/Filament/AdminPanelProvider.php` — where `admin_primary_color` is loaded from `SiteSetting` and passed to `->colors(['primary' => Color::hex(...)])`.
   - `app/Providers/AppServiceProvider.php` — where `site.admin_primary_color` is shared with views.
   - `app/Filament/Pages/Settings/GeneralSettingsPage.php` — the ColorPicker for `admin_primary_color`, and the corresponding `SiteSetting::set(...)` on save.
4. Read `app/Filament/Resources/ContactResource/Pages/EditContact.php` and `app/Filament/Resources/ProductResource/Pages/EditProduct.php` — these are the two edit pages whose `view_transactions` header action moves into the ellipsis overflow menu.
5. Grep the admin for `->color('gray')` on `Actions\Action::make(...)` header actions. These are the candidates for the secondary-colour swap. Note which surfaces currently render that white/outline style.
6. Open `sessions/199. Admin Secondary Button Colour & Header-Action Polish.md` and read the session prompt carefully.
