# Base Prompt — Session 210

We are about to begin a new session: **210. Widget Primitive — Contract Refinements**.
...
3. Read the two framing documents for this architectural track. **These are the north star; everything in the session prompt assumes you have read them.**
   ...
   - `sessions/widget-primitive-migration.md` — the migration sketch. Phase 1 landed in session 209; this session resolves the four refinements named in the Phase 1 findings before Phase 2 (slot taxonomy) locks the contract shape.
4. Read session 209's log and findings — they are the mandate for this session.
   - `sessions/209. Widget Primitive — Data Contract Prototype — Log.md`
   - `sessions/209. Widget Primitive — Data Contract Prototype — Findings.md` — the four refinements this session resolves are enumerated here with the rationale for each.
5. Read the widget-primitive namespace and its direct consumers. These are the files this session touches:
   - `app/WidgetPrimitive/DataContract.php`, `ContentType.php`, `SlotContext.php`, `ContractResolver.php` — the contract primitives delivered in session 209.
   - `app/Services/WidgetRenderer.php` — hosts the fallback-data shim that moves out of here in Phase 2 of this session.
   - `app/Services/PageContextTokens.php` — becomes a singleton with per-page memoization in Phase 3 of this session.
   - `app/Widgets/Contracts/WidgetDefinition.php` — base class for `dataContract()`. Docstring updates in Phase 4.
   - `app/Widgets/BlogListing/BlogListingDefinition.php`, `TextBlock/TextBlockDefinition.php`, `Carousel/CarouselDefinition.php` — the three migrated widgets. No contract-declaration changes expected; read to confirm the existing shape.
   - `tests/Unit/WidgetPrimitive/ContractResolverTest.php`, `DataContractTest.php`, `tests/Feature/WidgetPrimitiveIntegrationTest.php` — the existing test baseline. All must continue to pass after each refinement.
6. Open `sessions/210. Widget Primitive — Contract Refinements.md` and read the session prompt carefully. The **Design decisions (resolved before starting)** block settles several calls that are not open for rediscussion.
...
- **Next session base prompt**: copy `sessions/template-base-prompt.md` to `sessions/211. base-prompt.md`, updating the session number, title, and any session-specific read-list items.
- **Next session prompt**: draft `sessions/211. Session Title.md` using `sessions/template-session-prompt.md` as the format reference. The expected next session is **Widget Primitive — Phase 2 (Slot Taxonomy)** per `widget-primitive-migration.md`. This may take iteration with the user.

### Phase 2 — Close

When the user says to close (after the next session's prompt is agreed):

- **Session log**: write a log file at `sessions/210. Widget Primitive — Contract Refinements — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
- **Update session-outlines.md**: move this session's title into the Completed Sessions table. If findings affect the Phase 2 scope, update the Phase 2 stub accordingly.
- **Archive the previous session**: move all files matching `sessions/209. *.md` into `sessions/archived/`. This includes the previous session's base prompt, session prompt, log, and findings. Skip silently if those files don't exist or have already been archived.
- **Commit**: stage all changed files (including the log, updated outlines, and any files moved into `sessions/archived/`), commit on the current `session-210/N` branch, and notify the user. Do not push — the user will push and merge when ready.
