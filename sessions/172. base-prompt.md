...
We are about to begin a new session: **172. Per-Widget File Colocation**.
...
3. Open `sessions/172. Per-Widget File Colocation.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/172. Per-Widget File Colocation — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Archive the previous session**: move all files matching `sessions/171. *.md` into `sessions/archived/`. This includes session 171's base prompt, session prompt, and log. Skip silently if those files don't exist or have already been archived. Session 172's own files stay in `sessions/` until the next session's close.
- **Commit**: stage all changed files (including the log, updated outlines, and any files moved into `sessions/archived/`), commit on the current `session-172/N` branch, and notify the user. Do not push — the user will push and merge when ready.
