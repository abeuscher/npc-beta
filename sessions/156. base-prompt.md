...
We are about to begin a new session: **156. Form Polish, Image Fields & Chrome Toggle**.
...
3. Open `sessions/156. Form Polish, Image Fields & Chrome Toggle.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/156. Form Polish, Image Fields & Chrome Toggle — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit on the current `session-156/N` branch, and notify the user. Do not push — the user will push and merge when ready.
