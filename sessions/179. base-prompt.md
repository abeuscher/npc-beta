...
Copy this file to `179. base-prompt.md` at the start of each session. Replace `179` with the session number and update the session title.

---

We are about to begin a new session: **179. Code Review & Cleanup (Apply)**.
...
3. Open `sessions/179. Code Review & Cleanup (Apply).md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/179. Code Review & Cleanup (Apply) — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
