We are about to begin a new session: **082. Codebase Audit & Migration Squash**.
...
2. Open `sessions/082. Codebase Audit & Migration Squash.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/082. Codebase Audit & Migration Squash — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit to a feature branch named `session-082`, and notify the user. Do not push — the user will push and merge when ready.
