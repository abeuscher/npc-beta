...
We are about to begin a new session: **147. Editor API & Vue Scaffold**.
...
3. Read `docs/editor-migration-plan.md` **in full** — this is the architectural plan for the Vue migration. Every design decision, API endpoint, store shape, and component structure is documented there. This session implements the first phase.
4. Open `sessions/147. Editor API & Vue Scaffold.md` and read the session prompt carefully.
...
- **Session log**: write a log file at `sessions/147. Editor API & Vue Scaffold — Log.md`. Use `sessions/template-session-log.md` as the format reference — copy its structure exactly, do not base it on previous logs.
...
- **Commit**: stage all changed files (including the log and updated outlines), commit on the current `session-147/N` branch, and notify the user. Do not push — the user will push and merge when ready.
