    ...
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        @if ($phase === 'rejected')
            <div class="rounded-xl border border-red-200 bg-red-50 p-6 dark:border-red-800 dark:bg-red-950">
                ...
                <p class="mt-2 text-sm text-red-700 dark:text-red-400">
                    This memberships CSV was rejected due to sensitive data. Remove the flagged data and try again.
                </p>
            </div>
        ...
        @elseif ($phase === 'awaitingDecision')
            ...
            @php
                ...
                $membershipsPreview = $entities['memberships'] ?? ['would_create' => 0];
                $tiersPreview       = $entities['tiers'] ?? ['would_create' => 0, 'would_match' => 0];
                $contactsPreview    = $entities['contacts'] ?? ['would_create' => 0];
                $hasEntityPreview = ($membershipsPreview['would_create']
                    + $tiersPreview['would_create'] + $tiersPreview['would_match']
            ...
            @if ($hasEntityPreview)
                <div class="rounded-xl border border-gray-200 bg-white p-5 text-sm shadow-sm dark:border-gray-700 dark:bg-gray-900">
                    ...
                    <dl class="grid grid-cols-2 gap-3 sm:grid-cols-3 text-xs">
                        <div>
                            <dt class="text-gray-500 uppercase">Memberships</dt>
                            <dd class="mt-1 text-gray-700 dark:text-gray-300">
                                <strong>{{ number_format($membershipsPreview['would_create']) }}</strong> new
                            </dd>
                        </div>
                        <div>
                            <dt class="text-gray-500 uppercase">Tiers</dt>
                            <dd class="mt-1 text-gray-700 dark:text-gray-300">
                                <strong>{{ number_format($tiersPreview['would_create']) }}</strong> new,
                                <strong>{{ number_format($tiersPreview['would_match']) }}</strong> reused
                            </dd>
                        </div>
        ...
        @elseif ($phase === 'done')
            @if ($importSessionId)
                <div class="rounded-xl border border-amber-200 bg-amber-50 p-6 dark:border-amber-800 dark:bg-amber-950">
                    ...
                    <p class="mt-2 text-sm text-amber-700 dark:text-amber-400">
                        Imported memberships are awaiting reviewer approval.
                    </p>
                </div>
            @else
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                Store the memberships mapping on <strong>{{ $sourceName }}</strong>.
                            </p>
                        </div>
                        <button type="button"
            ...
            @if ($mappingSaved)
                <div class="rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-300">
                    Memberships mapping saved to <strong>{{ $sourceName }}</strong>.
                </div>
            @endif
