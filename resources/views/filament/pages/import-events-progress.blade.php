    ...
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        @if ($phase === 'rejected')
            <div class="rounded-xl border border-red-200 bg-red-50 p-6 dark:border-red-800 dark:bg-red-950">
                ...
                <p class="mt-2 text-sm text-red-700 dark:text-red-400">
                    This events CSV was rejected because it contains data that may include sensitive financial or personal identifiers. Remove the flagged data and try again.
                </p>
                @if ($piiHeaderBlocked)
        ...
        @elseif ($phase === 'awaitingDecision')
            ...
            @php
                ...
                $eventsPreview = $entities['events'] ?? ['would_create' => 0, 'would_match' => 0];
                $regsPreview   = $entities['registrations'] ?? ['would_create' => 0];
                $txPreview     = $entities['transactions'] ?? ['would_create' => 0, 'would_match' => 0];
                $hasEntityPreview = ($eventsPreview['would_create'] + $eventsPreview['would_match']
                    + $regsPreview['would_create']
                    + $txPreview['would_create'] + $txPreview['would_match']) > 0;
            @endphp
            ...
            @if ($hasEntityPreview)
                <div class="rounded-xl border border-gray-200 bg-white p-5 text-sm shadow-sm dark:border-gray-700 dark:bg-gray-900">
                    ...
                    <dl class="grid grid-cols-2 gap-3 sm:grid-cols-3 text-xs">
                        <div>
                            <dt class="text-gray-500 uppercase">Events</dt>
                            <dd class="mt-1 text-gray-700 dark:text-gray-300">
                                <strong>{{ number_format($eventsPreview['would_create']) }}</strong> new,
                                <strong>{{ number_format($eventsPreview['would_match']) }}</strong> reused
                            </dd>
                        </div>
                        <div>
                            <dt class="text-gray-500 uppercase">Registrations</dt>
                            <dd class="mt-1 text-gray-700 dark:text-gray-300">
                                <strong>{{ number_format($regsPreview['would_create']) }}</strong> new
                            </dd>
                        </div>
                        <div>
            ...
            @if ($dryRunReport['skipped'] > 0)
                ...
                <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-800 dark:bg-amber-950">
                    ...
                    <ul class="space-y-1 text-xs text-amber-700 dark:text-amber-400">
                        @if (($skipReasons['blank_event_id'] ?? 0) > 0)
                            <li class="flex items-start gap-2">
                                <span class="font-mono font-semibold tabular-nums">{{ number_format($skipReasons['blank_event_id']) }}</span>
                                <span>— row has no value in the <strong>Event External ID</strong> column. The importer cannot identify or create an event without an external ID.</span>
                            </li>
                        @endif
                        @if (($skipReasons['blank_contact_key'] ?? 0) > 0)
                            <li class="flex items-start gap-2">
                                ...
                                <span>— row has a blank value in the <strong>Contact match key</strong> column (email or external ID). Without a contact identifier, the row can't be linked to a person.</span>
                            </li>
                        @endif
                        @if (($skipReasons['contact_not_found'] ?? 0) > 0)
                            <li class="flex items-start gap-2">
                                ...
                                <span>— the contact match key had a value but <strong>no matching contact</strong> was found. These people are in the events export but not in your contacts. Import them via Import Contacts first, then re-run.</span>
                            </li>
                        @endif
                    </ul>
                </div>
                        ...
                        @foreach ($customFieldLog as $entry)
                            <li class="flex items-center gap-2">
                                @if ($entry['action'] === 'created')
                                    ...
                                    <span><strong>{{ $entry['label'] }}</strong> <span class="font-mono text-xs text-gray-500">({{ $entry['handle'] }}, on {{ $entry['target'] }})</span> — will create new field on commit</span>
                                @else
                                    <x-heroicon-o-arrow-path class="h-4 w-4 text-blue-500 shrink-0" />
                                    <span><strong>{{ $entry['label'] }}</strong> <span class="font-mono text-xs text-gray-500">({{ $entry['handle'] }}, on {{ $entry['target'] }})</span> — will reuse existing field</span>
                                @endif
                            </li>
                        @endforeach
                    </ul>
                            ...
                            @foreach (array_slice($dryRunReport['errors'], 0, 50) as $err)
                                <tr class="border-t border-gray-100 dark:border-gray-800">
                                    ...
                                    <td class="px-4 py-2 text-xs">
                                        @php
                                            $bits = array_filter([
                                                $err['identity']['event_id']    ?? null,
                                                $err['identity']['event_title'] ?? null,
        ...
        @elseif ($phase === 'done')
            @if ($importSessionId)
                <div class="rounded-xl border border-amber-200 bg-amber-50 p-6 dark:border-amber-800 dark:bg-amber-950">
                    ...
                    <p class="mt-2 text-sm text-amber-700 dark:text-amber-400">
                        Imported events and registrations are awaiting reviewer approval.
                    </p>
                </div>
            @else
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                Store the events mapping on <strong>{{ $sourceName }}</strong> so the next events import from this source starts pre-configured.
                            </p>
                        </div>
                        <button type="button"
            ...
            @if ($mappingSaved)
                <div class="rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-300">
                    ...
                    Events mapping saved to <strong>{{ $sourceName }}</strong>.
                </div>
            @endif
