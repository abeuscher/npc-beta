    ...
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        @if ($phase === 'rejected')
            <div data-testid="import-progress-phase-rejected" class="rounded-xl border border-red-200 bg-red-50 p-6 dark:border-red-800 dark:bg-red-950">
                ...
                <p class="mt-2 text-sm text-red-700 dark:text-red-400">
                    This notes CSV was rejected because it contains data that may include sensitive financial or personal identifiers. Remove the flagged data and try again.
                </p>
                @if ($piiHeaderBlocked)
        ...
        @elseif ($phase === 'awaitingDecision')
            ...
            @php
                ...
                $notesPreview = $entities['notes'] ?? ['would_create' => 0, 'would_update' => 0];
                $hasEntityPreview = ($notesPreview['would_create'] + ($notesPreview['would_update'] ?? 0)) > 0;
            @endphp
                    ...
                    <dl class="grid grid-cols-2 gap-3 sm:grid-cols-3 text-xs">
                        <div>
                            ...
                            <dd class="mt-1 text-gray-700 dark:text-gray-300">
                                ...
                                <strong>{{ number_format($notesPreview['would_update'] ?? 0) }}</strong> update{{ ($notesPreview['would_update'] ?? 0) === 1 ? '' : 's' }}
                            </dd>
                        </div>
                    </dl>
                </div>
                    ...
                    <ul class="space-y-1 text-xs text-amber-700 dark:text-amber-400">
                        @if (($skipReasons['duplicate_skipped'] ?? 0) > 0)
                            <li class="flex items-start gap-2">
                                ...
                                <span>&mdash; row matched an existing note (by External ID) and the <strong>Skip</strong> strategy was chosen.</span>
                            </li>
                        @endif
                    </ul>
                </div>
                                    ...
                                    <td class="px-4 py-2 text-xs">
                                        @php
                                            $bits = array_filter([
                                                ...
                                                $err['identity']['external_id'] ?? null,
                                                $err['identity']['subject']     ?? null,
                                            ]);
                                        @endphp
                                        {{ $bits ? implode(' / ', $bits) : '—' }}
                                    </td>
                            ...
                            @if (count($dryRunReport['errors']) > 50)
                                <tr>
                                    <td colspan="3" class="px-4 py-2 text-center text-xs text-gray-500">
                                        &hellip; {{ count($dryRunReport['errors']) - 50 }} more (download the CSV to see all)
                                    </td>
                                </tr>
                            @endif
                        </tbody>
        ...
        @elseif ($phase === 'done')
            ...
            @if ($importSessionId)
                <div class="rounded-xl border border-amber-200 bg-amber-50 p-6 dark:border-amber-800 dark:bg-amber-950">
                    ...
                    <p class="mt-2 text-sm text-amber-700 dark:text-amber-400">
                        Imported notes are awaiting reviewer approval.
                    </p>
                </div>
            @else
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                Store the notes mapping on <strong>{{ $sourceName }}</strong> so the next notes import from this source starts pre-configured.
                            </p>
                        </div>
                        <button type="button"
            ...
            @if ($mappingSaved)
                <div class="rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-800 dark:border-green-800 dark:bg-green-950 dark:text-green-300">
                    ...
                    Notes mapping saved to <strong>{{ $sourceName }}</strong>.
                </div>
            @endif
